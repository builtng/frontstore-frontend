...
const MOCK_PRODUCT_FAQS = [
  ...
  ["Are the tools and supplies genuine?", "Yes. We stock only genuine, professional-grade tools and materials, the same ones our tradespeople use on the job."],
...
const MOCK_SERVICE_FAQS = [
  ["Do you bring your own tools and materials?", "Yes. We arrive with our own professional tools and the materials needed for the job. If a specialist material is required, we will confirm this when you book."],
  ["How do deposits work?", "A deposit secures your slot when you book, and the balance is paid after the work is complete. The deposit amount is always shown before you confirm."],
  ["Can I reschedule or cancel?", "Yes, up to 24 hours before your booking for a full deposit refund. Inside 24 hours the deposit is held against the booking."],
  ["Do I need to be home?", "Yes, for most home service jobs we need access to the property and a responsible adult present during the work."],
...
const MOCK_CATS = ["Plumbing", "Electrical", "Painting", "Tiling", "Carpentry", "General", "Plans"];
              ...
              <div className="pd-listing">
                ...
                <div className="ab-wrap">
                  <div className="ab-main">
                    ...
                    <p className="ab-para">{store.about_intro_text || "What began in 2018 with one van and one cleaner is now a vetted team across Lagos. The promise has stayed the same throughout: turn up on time, do the job properly, and treat every home like your own."}</p>
                    <p className="ab-para">{store.about_intro_text || "Today the studio looks after brides, busy professionals and regulars who keep coming back for the calm of a private room and results they can count on, whether that is a wedding morning, a big shoot or an everyday refresh."}</p>
      ...
      {search && (
        <div className="ps-overlay" onClick={() => setSearch(false)}>
          <div className="ps-search-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ps-chips">{["Plumbing Repair", "Electrical Install", "Painting", "Tiling", "Carpentry"].map((c: any) => (<button key={c} onClick={() => { setSearch(false); ping("Searching " + c); }}>{c}</button>))}</div>
          </div>
        </div>
      )}
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#fff7ed;padding:7px 12px;border-radius:9px;margin-top:11px;}
...
.ps-notify{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:700;color:#fff;background:var(--brand);padding:8px 13px;border-radius:9px;box-shadow:0 4px 12px rgba(249,115,22,.28);cursor:pointer;}
...
.ct-map-pin{position:absolute;left:50%;top:44%;transform:translate(-50%,-50%) rotate(-45deg);display:grid;place-items:center;width:34px;height:34px;border-radius:50% 50% 50% 0;background:var(--brand);color:#fff;box-shadow:0 6px 13px rgba(194,65,12,.4);}
