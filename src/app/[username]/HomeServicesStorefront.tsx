...
interface HomeServicesStorefrontProps {
  username: string;
  store: StoreType;
  categories: Category[];
  products: Product[];
...
const MOCK_SERVICE_INFO = {
  s1: { includes: ["Diagnosis of leak, blockage or pressure issue", "Replacement of worn or faulty components", "Pipe repairs and joint sealing", "Tap, valve and fitting replacement", "Testing and sign-off on completion", "Advice on preventing the issue from returning"] },
  s2: { includes: ["Full wiring assessment before work begins", "Safe installation of sockets, switches and fittings", "Fuse board and consumer unit upgrades", "Certified testing and sign-off", "Earthing and bonding checked and confirmed", "Certificate issued on completion where applicable"] },
  s3: { includes: ["Surface preparation, filling and sanding", "Primer coat applied before finish coats", "Two finish coats for a long-lasting result", "Clean, sharp edges to ceilings and skirting", "Protection of floors and furniture throughout", "Full clean up and removal of materials on completion"] },
  s4: { includes: ["Floor preparation and levelling where needed", "Adhesive laid to a professional standard", "Tiles cut and set to agreed pattern", "Grouting with colour-matched or specified grout", "Sealant applied to wet areas and edges", "Finished joints cleaned down and inspected"] },
  s5: { includes: ["Design and measurement for fitted pieces", "Material selection guidance if needed", "Cutting, assembly and installation on site", "Hinges, handles and fixtures fitted and adjusted", "Structural fixing to walls and floors where required", "Site left clear with all debris removed"] },
  s6: { includes: ["Inspection of plumbing, electrics and carpentry", "Checking of windows, doors and locks", "Minor repairs carried out on the day", "Report of any issues needing further work", "Priority scheduling for follow-up jobs", "Pause or adjust the plan as your needs change"] },
  s7: { includes: ["Full property walkthrough and condition report", "Assessment of structural elements and finishes", "Plumbing pressure and drainage check", "Electrical safety assessment", "Written report with findings and recommendations", "Follow-up call to talk through the report"] },
  s8: { includes: ["Monthly visit from a dedicated technician", "Plumbing, electrical and carpentry checks included", "Minor repairs carried out on each visit", "Priority response for urgent issues between visits", "The same technician where possible", "Pause or cancel with one month's notice"] },
};
...
const MOCK_NOTIFY_TOPICS = [["plans", "Maintenance plans"], ["offers", "Offers and slots"], ["tips", "Home care tips"], ["news", "Announcements"]];
const MOCK_HOURS = [
...
export default function HomeServicesStorefront({
  username,
  store,
  categories,
  products,
  ...
  const customBrandColor = store.primary_color || '#f97316';
  const customBrandDeep = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 80%, black)` : '#c2410c';
  const customTint = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 14%, white)` : '#fff7ed';
  ...
  const DUMMY_AUTHOR = {
    ...
    initial: (store.founder_name || "S")[0].toUpperCase(),
    ...
    socials: store.founder_socials || { instagram: "", tiktok: "" },
  ...
  const primaryCta = ({
    ...
    book: { label: "Book a service", Icon: Calendar, run: () => openBooking() },
  ...
  } as Record<string, any>)[DUMMY_STORE.primaryCta as any] || { label: "Book a service", Icon: Calendar, run: () => openBooking() };
...
.sv-card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:18px;box-shadow:0 8px 30px rgba(249,115,22,.08);}
...
`;
  ...
  const serviceView = () => {
    ...
    const steps = [
      ...
      ["We arrive ready", "Your tradesperson arrives with professional tools and the standard materials for the job, so you do not have to source a thing."],
      ["We clean to a checklist", "The work follows a set scope for this service, and we will not leave until the job is done properly."],
      ["You check, then pay the balance", "Walk through the work with us at the end. The balance is only paid once you are happy with the result."],
    ];
    const brings = ["Professional-grade tools for the job", "Standard materials and fixings included", "A licensed, background checked tradesperson"];
    return (
      <div className="sv-wrap">
        ...
        <div className="sv-grid">
          <div className="sv-main">
            <div className="sv-hero"><Wrench size={34} /></div>
          ...
          <aside className="sv-side">
            <div className="sv-card">
              ...
              <button className="sv-book" onClick={() => openBooking(s)}><Calendar size={17} /> Book now</button>
              <a className="sv-enquire" href={waHref} target="_blank" rel="noreferrer"><MessageCircle size={16} /> Ask a question</a>
              <p className="sv-note">A deposit of about {money(deposit)} secures your slot. The balance is paid after the work, once you are happy.</p>
    ...
    return (
      <Sheet onClose={() => setBookOpen(false)} onBack={onBack} title={(titles as any)[bookStep]}>
          ...
          <div className="bk-svclist">
            {SERVICES.map((s: any) => (
              <button key={s.id} className="bk-svc" onClick={() => { setBookSvc(s); setBookStep("date"); }}>
                <span className={`bk-thumb ${catColor(s.cat)}`}><Wrench size={18} /></span>
        ...
        {bookStep === "review" && (<>
          ...
          <p className="ps-deposit">Frontstore holds your deposit until the work is complete, then the {money(bookSvc.price - deposit)} balance is due. Covered by buyer protection.</p>
  ...
  const aboutWork = () => {
    if (!displayPortfolio.length) return null;
    return (
  ...
  const aboutJournal = () => {
    ...
    <div className="ab-section">
      ...
      <p className="ab-journal-note">Written by {DUMMY_AUTHOR.name}, drawing on a decade of work across Lagos homes.</p>
    </div>
  );
  };
  ...
  const aboutBody = () => (<>
    ...
    <button className="ab-book-m" onClick={() => openBooking()}><Calendar size={16} /> Book now</button></>);
  const faqSections = () => (
  ...
  const portfolioBody = () => (<>
    ...
    {portfolioChips()}
    {portfolioGrid()}
    <button className="ab-book-m" onClick={() => openBooking()}><Calendar size={16} /> Book a service</button>
    {portfolioFollow()}
  </>);
  const articleView = () => {
    if (!post) return null;
        ...
        <div className="art-wrap">
          ...
          <aside className="art-rail">
            ...
            <div className="blog-convert">
              ...
              <p>Book a service, or browse the full list of what we offer.</p>
              <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book a service</button>
    ...
    <div className="ps-root" style={storeStyleTheme}>
              ...
              <div className="pd-listing">
                ...
                {page === "services" && (
                  <div className="svc-page">
                    <p className="svc-intro">Book a service with our team. A deposit secures your slot, with the balance paid after the service.</p>
                    ...
                    <div className="blogp-body">
                      ...
                      <aside className="blogp-rail">
                        <div className="author-card">
                          ...
                          <p>{DUMMY_AUTHOR.bio}</p>
                          <div className="author-cred"><Sparkles size={13} /> Over 8 years fixing homes across Lagos</div>
                          <button className="author-link" onClick={() => go("about")}>More about {DUMMY_AUTHOR.name.split(" ")[0]} <ChevronRight size={14} /></button>
                        </div>
                        <div className="blog-convert">
                          <b>Enjoyed the read?</b>
              ...
              <div className="pd-listing">
                ...
                <div className="ab-wrap">
                  <div className="ab-main">
                    ...
                    <h2 className="ab-headline">Home services done right, by people who show up when they say.</h2>
                    ...
                    <div className="ab-approach">
                      ...
                      <div className="ab-approach-grid">
                        <div><span className="ab-num">01</span><b>We learn the job</b><p>Every booking starts by understanding what needs doing, so the fix suits your home and how you live in it.</p></div>
                        <div><span className="ab-num">02</span><b>The right approach</b><p>We bring the right tools and tradespeople for your home, not a one size fits all routine.</p></div>
                        <div><span className="ab-num">03</span><b>Made to last</b><p>From a quick fix to a full renovation, the job is done properly, not just quickly.</p></div>
                      </div>
                    </div>
                  ...
                  <aside className="ab-rail">
                    <div className="pd-railcard">
                      <div className="ab-rail-h"><MapPin size={15} /> Visit us</div>
                    ...
                    <div className="blog-convert">
                      ...
                      <p>Book now, or browse the full list of services.</p>
                      <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book now</button>
                      {(store.storefront_sections || []).includes("services") && (
          <button className="blog-convert-ghost" onClick={() => go("services")}>Browse services</button>
        )}
                    </div>
              ...
              <div className="pd-listing">
                ...
                <div className="ct-wrap">
                  ...
                  <aside className="ct-rail">
                    ...
                    <div className="blog-convert">
                      <b>Prefer to book?</b>
                      <p>Skip the message and book a service in a couple of taps.</p>
                      <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book now</button>
                      {(store.storefront_sections || []).includes("services") && (
          <button className="blog-convert-ghost" onClick={() => go("services")}>Browse services</button>
        )}
      ...
      {!isDesktop && (
        <nav className="ps-bottom">
          ...
          {(store.storefront_sections || []).includes("services") && (
          <button className={page === "services" ? "on" : ""} onClick={() => go("services")}><Wrench size={21} /><span>Services</span></button>
        )}
      ...
      {search && (
        <div className="ps-overlay" onClick={() => setSearch(false)}>
          <div className="ps-search-panel" onClick={(e) => e.stopPropagation()}>
            <div className="ps-search-top"><Search size={18} /><input autoFocus placeholder={`Search home services`} /><button onClick={() => setSearch(false)}><X size={20} /></button></div>
...
function ServiceCard({ s, onBook }: { s: any, onBook: () => void }) {
  return (<div className="ps-card"><div className="ps-card-thumb svc"><Wrench size={22} /></div>
  ...
  return (
    <div className="feat-card">
      <div className={`feat-thumb ${f.type}`}>
        {f.type === "service" ? <Wrench size={26} /> : <ShoppingBag size={26} />}
...
const css = `
.ps-root{--brand:#f97316;--brand-deep:#c2410c;--gold:#c79a4b;--bg:#f4f8f8;--card:#fff;--ink:#18272a;
  --muted:#6f8480;--line:#e2eceb;--ok:#1f9d57;--wa:#25d366;
  --shadow-sm:0 1px 2px rgba(24,39,42,.04),0 2px 8px rgba(24,39,42,.05);
  --shadow-md:0 4px 12px rgba(24,39,42,.06),0 10px 28px rgba(24,39,42,.08);
  --shadow-lg:0 8px 24px rgba(24,39,42,.09),0 20px 48px rgba(24,39,42,.11);
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#fff7ed;padding:7px 12px;border-radius:9px;margin-top:11px;max-width:100%;min-width:0}
...
.ps-notify{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:700;color:#fff;background:var(--brand);padding:8px 13px;border-radius:9px;box-shadow:0 4px 12px rgba(249,115,22,.28);cursor:pointer;flex-shrink:0}
...
.nt-topic.on{background:#fff7ed;border-color:var(--brand);color:var(--brand-deep);}
...
.ps-signature{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:14px;margin-bottom:18px;box-shadow:0 8px 22px rgba(194,65,12,.07);}
.ps-sig-tag{display:inline-flex;align-items:center;gap:5px;font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:var(--brand-deep);background:#fff7ed;padding:4px 9px;border-radius:7px;}
...
.ps-book-btn,.ps-sheet-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:15px;padding:13px;border-radius:12px;box-shadow:0 6px 16px rgba(249,115,22,.3);}
.ps-book-btn:hover,.ps-sheet-cta:hover{box-shadow:0 8px 22px rgba(249,115,22,.4);transform:translateY(-1px);}
.ps-book-btn:active,.ps-sheet-cta:active{transform:translateY(2px);box-shadow:0 3px 10px rgba(249,115,22,.26);}
...
.ps-mini{font-size:13px;font-weight:700;padding:7px 15px;border-radius:9px;color:#fff;background:var(--brand);box-shadow:0 3px 8px rgba(249,115,22,.28);}
.ps-mini:hover{box-shadow:0 5px 14px rgba(249,115,22,.36);transform:translateY(-1px);}
.ps-mini:active{transform:translateY(1px);box-shadow:0 2px 6px rgba(249,115,22,.24);}
...
.ps-seeall:hover{background:#fff7ed;border-color:var(--brand);}
...
.ps-dir:hover{background:#fff7ed;border-color:var(--brand);}
...
.ps-locked{background:#fff7ed;border:1px solid #fed7aa;border-radius:14px;padding:14px;margin:16px 0 8px;}
...
.ps-locked p{font-size:12.5px;line-height:1.55;color:#6b5344;margin-top:8px;}
...
.ps-foot-links button{font-size:12px;font-weight:600;color:#6b5344;}
...
.ps-nav button.on{background:#fff7ed;color:var(--brand-deep);font-weight:700;}
.ps-panel-actions{display:flex;flex-direction:column;gap:9px;margin-top:auto;}
.ps-act-book{display:flex;align-items:center;justify-content:center;gap:8px;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:13px;border-radius:12px;box-shadow:0 6px 16px rgba(249,115,22,.3);}
...
.ps-fab svg{position:relative;z-index:1;width:54px;height:54px;background:var(--brand);color:#fff;padding:16px;border-radius:50%;box-shadow:0 8px 20px rgba(249,115,22,.4);margin-top:-26px;}
...
.bk-svcbar{display:flex;align-items:center;justify-content:space-between;gap:10px;background:#fff7ed;border:1px solid #fed7aa;border-radius:12px;padding:10px 13px;margin-bottom:16px;}
...
.bk-deposit-row{display:flex;align-items:center;justify-content:space-between;margin-top:16px;padding:12px 14px;background:#fff7ed;border:1px solid #fed7aa;border-radius:12px;}
...
.pd-head-book{display:flex;align-items:center;gap:7px;background:var(--brand);color:#fff;font-weight:700;font-size:14px;padding:11px 18px;border-radius:12px;box-shadow:0 5px 14px rgba(249,115,22,.3);}
...
.pd-avatar{width:128px;height:128px;border-radius:30px;flex:0 0 auto;background:linear-gradient(150deg,var(--brand),var(--brand-deep));color:#fff;font-family:'Fraunces';font-weight:700;font-size:56px;display:grid;place-items:center;border:6px solid var(--bg);box-shadow:0 10px 28px rgba(194,65,12,.22);}
...
.pd-book{display:flex;align-items:center;gap:7px;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px 22px;border-radius:12px;box-shadow:0 6px 16px rgba(249,115,22,.3);}
...
.pd-tabs button.on{background:#fff7ed;color:var(--brand-deep);font-weight:700;}
...
.pd-railbtns button:hover{background:#fff7ed;border-color:var(--brand);}
.pd-railcard.trust{background:#fff7ed;border-color:#fed7aa;}
.pd-trust-h{display:flex;align-items:center;gap:7px;font-size:13.5px;font-weight:700;color:var(--brand-deep);}
.pd-railcard.trust p{font-size:12px;line-height:1.55;color:#6b5344;margin-top:8px;}
...
.feat-tag{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--brand-deep);background:#fff7ed;padding:5px 11px;border-radius:8px;}
...
.feat-card{flex:0 0 76%;max-width:280px;background:var(--card);border:1px solid var(--line);border-radius:16px;overflow:hidden;box-shadow:0 8px 22px rgba(194,65,12,.08);display:flex;flex-direction:column;}
...
.feat-cta{font-size:13px;font-weight:700;padding:8px 18px;border-radius:10px;color:#fff;background:var(--brand);box-shadow:0 4px 10px rgba(249,115,22,.3);}
...
.blog-card:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(194,65,12,.1);}
...
.blog-body p{font-size:13px;color:#6b5344;line-height:1.5;}
...
.blog-hero:hover{transform:translateY(-2px);box-shadow:0 14px 32px rgba(194,65,12,.1);}
...
.blog-convert{background:#fff7ed;border:1px solid #fed7aa;border-radius:16px;padding:17px;}
.blog-convert>b{font-family:'Fraunces';font-weight:600;font-size:16px;}
.blog-convert>p{font-size:12.5px;color:#6b5344;line-height:1.5;margin:6px 0 13px;}
.blog-convert-cta{display:flex;align-items:center;justify-content:center;gap:7px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:13.5px;padding:11px;border-radius:11px;box-shadow:0 6px 14px rgba(249,115,22,.28);}
...
.ab-chips span{font-size:11.5px;font-weight:600;color:var(--brand-deep);background:#fff7ed;border:1px solid #ecd2dd;padding:5px 11px;border-radius:16px;}
...
.ab-follow-icons button{width:40px;height:40px;border-radius:11px;display:grid;place-items:center;background:#fff7ed;border:1px solid #ecd2dd;color:var(--brand-deep);transition:background .12s,color .12s;}
...
.ab-book-m{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;margin-top:18px;background:var(--brand);color:#fff;font-weight:700;font-size:15px;padding:14px;border-radius:13px;box-shadow:0 8px 18px rgba(249,115,22,.3);}
...
.ab-offer-card:hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(194,65,12,.09);border-color:#fed7aa;}
...
.ab-verified{display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:700;color:var(--brand-deep);background:#fff7ed;border:1px solid #ecd2dd;padding:4px 10px;border-radius:16px;margin-top:8px;}
...
.ab-shot:hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(194,65,12,.14);}
...
.ab-review{position:relative;background:#fff7ed;border:1px solid #fed7aa;border-radius:18px;padding:24px;margin:28px 0;}
...
.ab-journal-item:hover{transform:translateY(-2px);box-shadow:0 10px 22px rgba(194,65,12,.09);border-color:#fed7aa;}
...
.faq-cats button:hover{background:#fff7ed;color:var(--brand-deep);}
.faq-cats button svg{color:var(--brand);flex:0 0 auto;}
.faq-help{background:#fff7ed;border:1px solid #fed7aa;border-radius:16px;padding:17px;}
.faq-help>b{font-family:'Fraunces';font-weight:600;font-size:16px;}
.faq-help>p{font-size:12.5px;color:#6b5344;line-height:1.5;margin:6px 0 13px;}
.faq-help-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:13.5px;padding:11px;border-radius:11px;box-shadow:0 6px 14px rgba(249,115,22,.28);}
...
.ct-wa{display:flex;align-items:center;justify-content:center;gap:9px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:15.5px;padding:15px;border-radius:13px;box-shadow:0 8px 18px rgba(249,115,22,.3);}
...
.ct-send{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14px;padding:13px;border-radius:11px;box-shadow:0 6px 14px rgba(249,115,22,.28);}
...
.ct-map-pin{position:absolute;left:50%;top:44%;transform:translate(-50%,-50%) rotate(-45deg);display:grid;place-items:center;width:34px;height:34px;border-radius:50% 50% 50% 0;background:var(--brand);color:#fff;box-shadow:0 6px 13px rgba(194,65,12,.4);z-index:2}
...
.ct-hours-list li.today{background:#fff7ed;color:var(--brand-deep);}
...
.rf-related button:hover{background:#fff7ed;color:var(--brand-deep);}
...
.tm-contents button:hover{background:#fff7ed;color:var(--brand-deep);}
...
.pf-shot:hover{transform:translateY(-2px);box-shadow:0 14px 30px rgba(194,65,12,.16);}
...
.svc-intro{font-size:14px;color:#6b5344;line-height:1.5;margin:-4px 0 24px;max-width:620px;}
...
.svc-radios button.on{background:#fff7ed;color:var(--brand-deep);font-weight:700;}
.svc-book-card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:16px;box-shadow:0 8px 22px rgba(194,65,12,.06);}
...
.svc-book-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px;border-radius:12px;box-shadow:0 6px 16px rgba(249,115,22,.3);}
...
.svc-card:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(194,65,12,.1);}
...
.svc-card-desc{font-size:12.5px;color:#6b5344;line-height:1.5;margin-top:8px;flex:1;}
...
.svc-card-book{display:inline-flex;align-items:center;gap:3px;font-size:13px;font-weight:700;color:#fff;background:var(--brand);padding:8px 14px;border-radius:10px;box-shadow:0 4px 10px rgba(249,115,22,.3);}
...
.rev-leave{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px;border-radius:12px;box-shadow:0 6px 16px rgba(249,115,22,.3);}
...
.rev-card-svc{display:inline-block;font-size:11px;font-weight:700;color:var(--brand-deep);background:#fff7ed;padding:3px 9px;border-radius:7px;margin:11px 0 8px;}
...
.rev-form-note{display:flex;align-items:flex-start;gap:7px;font-size:12.5px;color:#6b5344;line-height:1.5;background:#fff7ed;border:1px solid #fed7aa;border-radius:11px;padding:11px 12px;margin-bottom:6px;}
...
.rev-input:focus,.rev-textarea:focus{outline:none;border-color:var(--brand);box-shadow:0 0 0 3px rgba(249,115,22,.12);}
