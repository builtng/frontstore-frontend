...
const MOCK_PRODUCT_FAQS = [
  ...
  ["Can I get a refund on a course?", "Courses come with a 14 day refund if you are not happy and have completed under 30 percent. Templates and downloads cannot be refunded once accessed."],
  ["How does the membership billing work?", "SEO Insiders is billed monthly and you can cancel anytime. You keep access until the end of the period you have paid for."],
  ["How do I pay?", "Pay securely through Frontstore, or by bank transfer where offered. Your receipt and access details arrive on WhatsApp or by email."],
];
const MOCK_SERVICE_FAQS = [
  ["How do I book a service?", "Pick a service, choose a date, and a deposit confirms it. We then have a short kickoff to align before the work begins."],
  ["How do deposits work?", "A deposit confirms your booking, and the balance is due before the work begins. The deposit amount is always shown before you confirm."],
...
`;
  ...
  const aboutBody = () => (<>
    ...
    {username === 'victorijomah' && <p className="ab-para">{store.about_intro_text || "What began as fixing broken sites and sharing what worked has grown into courses, templates and a newsletter read by thousands, with the same goal throughout: make technical SEO clear and usable."}</p>}
    {store.founder_name && (
  ...
  const contactForm = () => (
    <div className="ct-form">
      ...
      <p className="ct-form-sub">Tell us what you are after and we will reply by email. A real person, usually Victor, will answer.</p>
  ...
  const announcement = !annOff && (
    <div className="ps-ann">
      <Megaphone size={16} />
      <p><b>Announcement</b> Get the free 120-point Technical SEO checklist when you join the newsletter.</p>
              ...
              <div className="pd-listing">
                ...
                {page === "products" && (
                  <div className="svc-page">
                    ...
                    <div className="pd-sec-head"><h2>Access and refunds</h2></div>
              ...
              <div className="pd-listing">
                ...
                <div className="ct-wrap">
                  <div className="ct-main">
                    <p className="svc-intro">Reach us on WhatsApp for a quick reply, or send an enquiry and we will get back to you by email. A real person, usually Victor, will answer.</p>
                    {contactChannels()}
                    {contactForm()}
                  </div>
      ...
      {search && (
        <div className="ps-overlay" onClick={() => setSearch(false)}>
          <div className="ps-search-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ps-chips">{["Technical SEO Audit", "Masterclass", "Audit Checklist", "Consultation", "Notion Template"].map((c: any) => (<button key={c} onClick={() => { setSearch(false); ping("Searching " + c); }}>{c}</button>))}</div>
          </div>
        </div>
      )}
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#ebeafc;padding:7px 12px;border-radius:9px;margin-top:11px;}
...
.ps-notify{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:700;color:#fff;background:var(--brand);padding:8px 13px;border-radius:9px;box-shadow:0 4px 12px rgba(67,56,202,.28);cursor:pointer;}
...
.ct-map-pin{position:absolute;left:50%;top:44%;transform:translate(-50%,-50%) rotate(-45deg);display:grid;place-items:center;width:34px;height:34px;border-radius:50% 50% 50% 0;background:var(--brand);color:#fff;box-shadow:0 6px 13px rgba(49,46,129,.4);}
