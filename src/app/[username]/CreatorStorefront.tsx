...
const MOCK_SERVICE_INFO = {
  ...
  s3: { includes: ["A 60 minute one to one call", "Talk through your specific challenges", "A clear direction to move in", "Notes and next steps after", "No prep needed, just bring questions", "Recording shared if you want it"] },
  s4: { includes: ["A review of your migration plan", "Redirect and URL mapping checks", "Risk flags before you launch", "A go live and post launch checklist", "A walkthrough of the findings", "Support through the riskiest window"] },
  s5: { includes: ["Two coaching calls each month", "Async support between calls", "A plan tailored to your goals", "Reviews of your own work", "Resources and templates shared", "Cancel or pause anytime"] },
  s6: { includes: ["Ongoing technical SEO support", "Priority access and faster replies", "Monthly review and direction", "Hands-on help where needed", "A steady, senior pair of eyes", "Flexible to your team's needs"] },
...
const MOCK_HOURS = [
  ...
  ["Thu", "9:00am - 5:00pm"], ["Fri", "9:00am - 5:00pm"], ["Sat", "By appointment"], ["Sun", "Closed"],
];
const MOCK_NAV = [
  ["home", "Home"], ["services", "Services"], ["products", "Shop"], ["portfolio", "Case studies"], ["reviews", "Reviews"], ["blog", "Articles"],
...
export default function CreatorStorefront({
  ...
  const customBrandColor = store.primary_color || '#4338ca';
  const customBrandDeep = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 80%, black)` : '#312e81';
  ...
  const primaryCta = ({
    ...
    book: { label: "Book a call", Icon: Calendar, run: () => openBooking() },
  ...
  } as Record<string, any>)[DUMMY_STORE.primaryCta as any] || { label: "Book a call", Icon: Calendar, run: () => openBooking() };
...
.sv-cat.c0{background:var(--brand);}.sv-cat.c1{background:#6f6ca0;}.sv-cat.c2{background:#59566e;}.sv-cat.c3{background:var(--brand-deep);}
...
.sv-card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:18px;box-shadow:0 8px 30px rgba(67,56,202,.08);}
...
`;
  ...
  const serviceView = () => {
    ...
    const steps = [
      ["Book and pay a deposit", "Pick a service and a date. A deposit confirms it, and you see the amount before you confirm."],
      ["A quick kickoff", "We have a short call or exchange to align on goals, access and what success looks like."],
      ["The work", "I run the audit, review or session and document everything clearly, no jargon for its own sake."],
      ["Findings and next steps", "You get a clear, prioritised deliverable and a walkthrough, so you know exactly what to do next."],
    ];
    const brings = ["A clear, prioritised deliverable", "A walkthrough so nothing is a black box", "Direct access to ask follow up questions"];
    return (
        ...
        <div className="sv-grid">
          ...
          <aside className="sv-side">
            <div className="sv-card">
              ...
              <button className="sv-book" onClick={() => openBooking(s)}><Calendar size={17} /> Book a call</button>
              <a className="sv-enquire" href={waHref} target="_blank" rel="noreferrer"><MessageCircle size={16} /> Ask a question</a>
              <p className="sv-note">A deposit of about {money(deposit)} confirms your booking. The balance is due before the work begins.</p>
    ...
    return (
      <Sheet onClose={() => setBookOpen(false)} onBack={onBack} title={(titles as any)[bookStep]}>
        ...
        {bookStep === "review" && (<>
          ...
          <textarea className="bk-input bk-textarea" value={bookNote} onChange={(e) => setBookNote(e.target.value)} placeholder="Your site, your goal, anything useful" />
          <div className="bk-deposit-row"><span><Lock size={13} /> Deposit to secure</span><b>{money(deposit)}</b></div>
          <p className="ps-deposit">Frontstore holds your deposit until the work begins, then the {money(bookSvc.price - deposit)} balance is due before it starts. Covered by buyer protection.</p>
  ...
  const priceTest = (price: number) => prodPrice === "All" || (prodPrice === "lo" && price < 20000) || (prodPrice === "mid" && price >= 20000 && price <= 50000) || (prodPrice === "hi" && price > 50000);
  ...
  const homeCourses = PRODUCTS.filter((p) => p.cat === "Courses");
  const homeDigital = PRODUCTS.filter((p) => p.cat !== "Courses");
  ...
  const reviewsBody = () => (<>
    ...
    <div className="rev-trust rev-trust-m"><ShieldCheck size={14} /> Reviews are from verified orders. The team typically responds in {DUMMY_STORE.reply}.</div>
  ...
  const aboutBody = () => (<>
    ...
    <button className="ab-book-m" onClick={() => openBooking()}><Calendar size={16} /> Book a call</button></>);
  const faqSections = () => (
    <div className="faq-groups">
      {faqFiltered.length === 0 && (
        <div className="faq-empty">No questions match that search. Try another word, or message the team below.</div>
      )}
      ...
      <div className="ct-visit">
        ...
        <div className="ct-hours">
          ...
          {hasHours ? (
            ...
            <p className="ct-hours-empty">This {storeLabel} hasn't added opening hours yet. Message on WhatsApp to check availability.</p>
          )}
        </div>
      ...
      <div className="rf-section">
        ...
        <ul className="rf-list">
          {store.policy_bookings ? (
            ...
            <>
              <li><Check size={15} /> Reschedule or cancel a call up to 24 hours before for a full refund of your deposit.</li>
              <li><Check size={15} /> Inside 24 hours of a booking, the deposit is held against it and is not refunded.</li>
      ...
      <div className="rf-section">
        ...
        <ul className="rf-list">
          {store.policy_products ? (
            ...
            <>
              <li><Check size={15} /> Courses come with a 14 day refund if you have completed under 30 percent.</li>
              <li><Check size={15} /> Templates, checklists and downloads cannot be refunded once accessed or downloaded.</li>
              <li><Check size={15} /> If a file is broken or wrong, get in touch and we will fix or replace it.</li>
            </>
          )}
        </ul>
      </div>
  ...
  const refundsAction = () => (
    <div className="blog-convert">
      ...
      <p>Message the team and we will sort your cancellation, return or refund.</p>
  ...
  const caseGrid = (n: number) => (
    <div className="pf-grid">
      {displayPortfolio.slice(0, n).map((p: any) => (
        <button key={p.label} className={`pf-shot ${p.c}`} onClick={() => ping("Opening case study")}>
          <span className="pf-shot-cap"><b>{p.label}</b><span>{p.cat}</span></span>
        </button>
      ))}
    </div>
  ...
  const portfolioBody = () => (<>
    <p className="svc-intro">A few results from real audits and projects, what changed and what it meant for the business.</p>
    {portfolioChips()}
    {portfolioGrid()}
    <button className="ab-book-m" onClick={() => openBooking()}><Calendar size={16} /> Book a call</button>
    {portfolioFollow()}
        ...
        <div className="art-wrap">
          ...
          <aside className="art-rail">
            ...
            <div className="blog-convert">
              ...
              <p>Book a call, or browse the courses and templates.</p>
              <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book a call</button>
              {(store.storefront_sections || []).includes("products") && (
          <button className="blog-convert-ghost" onClick={() => go("products")}>View the shop</button>
        )}
            </div>
          </aside>
        </div>
        <div className="art-related">
          <h3 className="ab-subhead">More articles</h3>
          <div className="blog-grid">
    ...
    <div className="ps-root" style={storeStyleTheme}>
      ...
      {!isDesktop && (
        <div className="ps-col">
          ...
          <button className="ps-seeall" onClick={() => go("services")}>See all {SERVICES.length} services <ChevronRight size={16} /></button>
              ...
              <SectionHead title="Courses" action={`See all ${homeCourses.length}`} onAction={() => { setProdCat("Courses"); go("products"); }} />
              {productsGrid("ps-grid", homeCourses.slice(0, 4))}
              <button className="ps-seeall" onClick={() => { setProdCat("Courses"); go("products"); }}>See all courses <ChevronRight size={16} /></button>

              <SectionHead title="Digital products" action="See all" onAction={() => { setProdCat("All"); go("products"); }} />
              {productsGrid("ps-grid", homeDigital.slice(0, 4))}
              <button className="ps-seeall" onClick={() => { setProdCat("All"); go("products"); }}>See all in the shop <ChevronRight size={16} /></button>

              <SectionHead title="Case studies" action="See all" onAction={() => go("portfolio")} />
              {caseGrid(4)}
              <button className="ps-seeall" onClick={() => go("portfolio")}>See all case studies <ChevronRight size={16} /></button>
          ...
          <button className="ps-seeall" onClick={() => go("reviews")}>See all reviews <ChevronRight size={16} /></button>
            ...
            {page === "products" && <Sub title="Shop" slug={DUMMY_STORE.slug}>{productsGrid("ps-grid", PRODUCTS)}<StoreFoot onNav={go}  slug={DUMMY_STORE.slug} /></Sub>}
            {page === "portfolio" && <Sub title="Case studies" slug={DUMMY_STORE.slug}>{portfolioBody()}<StoreFoot onNav={go}  slug={DUMMY_STORE.slug} /></Sub>}
            ...
            {page === "blog" && <Sub title="Articles" slug={DUMMY_STORE.slug}>{blogBody()}<StoreFoot onNav={go}  slug={DUMMY_STORE.slug} /></Sub>}
      ...
      {isDesktop && (
        <div className="pd-wrap">
            ...
            <section className="pd-cover">
              ...
              <div className="pd-identity">
                ...
                <div className="pd-identity-actions">
                  ...
                  <button className="pd-ghost" onClick={() => setNotifyOpen(true)}><Bell size={16} /> Free checklist</button>
                </div>
              </div>
            </section>
          ...
          <button onClick={() => go("services")}>See all {SERVICES.length}</button>
                  ...
                  <div className="pd-sec-head"><h2>Courses</h2><button onClick={() => { setProdCat("Courses"); go("products"); }}>See all {homeCourses.length}</button></div>
                  {productsGrid("pd-grid", homeCourses.slice(0, 6))}
                  <div className="pd-sec-head"><h2>Digital products</h2><button onClick={() => { setProdCat("All"); go("products"); }}>See all</button></div>
                  {productsGrid("pd-grid", homeDigital.slice(0, 6))}
                  <div className="pd-sec-head"><h2>Case studies</h2><button onClick={() => go("portfolio")}>See all</button></div>
                  {caseGrid(6)}
              ...
              <div className="pd-listing">
                ...
                {page === "services" && (
                  <div className="svc-page">
                    <p className="svc-intro">Book a service. A deposit confirms your booking, with the balance due before the work begins.</p>
                    ...
                    <div className="svc-body">
                      <aside className="svc-rail">
                        ...
                        <div className="svc-book-card">
                          ...
                          <button className="svc-book-cta" onClick={() => openBooking()}><Calendar size={16} /> Book a call</button>
                ...
                {page === "products" && (
                  <div className="svc-page">
                    <p className="svc-intro">Courses, templates and checklists, delivered the moment you buy. Everything here is digital, so there is nothing to ship and nothing to wait for.</p>
                    ...
                    <div className="svc-body">
                      <aside className="svc-rail">
                        <div className="svc-search">
                          <Search size={16} />
                          <input value={prodQuery} onChange={(e) => setProdQuery(e.target.value)} placeholder="Search the shop" />
                        ...
                        <div className="svc-filters">
                          ...
                          <div className="svc-fgroup">
                            <h4>Price</h4>
                            <div className="svc-radios">
                              {[["All", "Any price"], ["lo", "Under ₦20,000"], ["mid", "₦20,000 to ₦50,000"], ["hi", "Over ₦50,000"]].map(([v, l]: any) => (
                        ...
                        <div className="svc-book-card">
                          ...
                          <ul className="svc-deliv">
                            <li><Truck size={14} /> Instant access right after checkout</li>
                            <li><Sparkles size={14} /> Lifetime access and free updates</li>
                          </ul>
                          <button className="svc-book-cta" onClick={() => setBag(true)}><ShoppingBag size={16} /> View bag{bagCount > 0 ? ` (${bagCount})` : ""}</button>
                          <button className="svc-msg" onClick={() => handleWa("Hello! I'm interested in your services.")}><WhatsApp size={15} /> Ask about a course</button>
                        </div>
                      </aside>
                  ...
                  <div className="svc-page">
                    ...
                    <div className="svc-body">
                      <aside className="svc-rail">
                        ...
                        <div className="rev-trust"><ShieldCheck size={14} /> Reviews are from verified orders. The team typically responds in {DUMMY_STORE.reply}.</div>
                      </aside>
                ...
                {page === "blog" && (
                  <div className="blogp">
                    <p className="svc-intro">Practical technical SEO, written plainly by {DUMMY_AUTHOR.name}.</p>
                    <div className="blog-topics">
                    ...
                    <div className="blogp-body">
                      ...
                      <aside className="blogp-rail">
                        <div className="author-card">
                          ...
                          <p>{DUMMY_AUTHOR.bio}</p>
                          <div className="author-cred"><Sparkles size={13} /> 10+ years in technical SEO, and 8,000+ students taught</div>
                          <button className="author-link" onClick={() => go("about")}>More about {DUMMY_AUTHOR.name.split(" ")[0]} <ChevronRight size={14} /></button>
                        </div>
                        <div className="blog-convert">
                          <b>Enjoyed the read?</b>
          ...
          <button className="blog-convert-ghost" onClick={() => go("products")}>View the shop</button>
            ...
            {page === "portfolio" && (
              <div className="pd-listing">
                ...
                <p className="svc-intro">A few results from real audits and projects, what changed and what it meant for the business. Tap any tile to see more.</p>
                {portfolioChips()}
                <div className="pf-wrap">
                  ...
                  <aside className="pf-rail">
                    <div className="blog-convert">
                      ...
                      <p>Book a call when you are ready.</p>
                      <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book a call</button>
                      {(store.storefront_sections || []).includes("services") && (
          <button className="blog-convert-ghost" onClick={() => go("services")}>Browse services</button>
                    ...
                    <div className="pd-railcard trust">
                      ...
                      <p>Bookings and orders placed through Frontstore are covered by buyer protection the team cannot remove.</p>
                    </div>
                  </aside>
                </div>
                <StoreFoot onNav={go}  slug={DUMMY_STORE.slug} />
                ...
                <div className="ab-wrap">
                  ...
                  <aside className="ab-rail">
                    ...
                    <div className="blog-convert">
                      ...
                      <p>Book a call, or see all the services.</p>
                      <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book a call</button>
                      {(store.storefront_sections || []).includes("services") && (
          <button className="blog-convert-ghost" onClick={() => go("services")}>Browse services</button>
        )}
                    ...
                    <div className="pd-railcard trust">
                      <div className="pd-trust-h"><ShieldCheck size={15} /> Secured by Frontstore</div>
                      <p>Bookings and orders placed through Frontstore are covered by buyer protection that the team cannot remove.</p>
                    </div>
                  </aside>
                </div>
                <StoreFoot onNav={go}  slug={DUMMY_STORE.slug} />
            ...
            {page === "faq" && (
              <div className="pd-listing">
                ...
                <p className="svc-intro">Answers to the questions we are asked most. If you cannot find yours, the team is a message away.</p>
                <div className="faq-wrap">
                  <aside className="faq-rail">
                    ...
                    <div className="pd-railcard trust">
                      <div className="pd-trust-h"><ShieldCheck size={15} /> Secured by Frontstore</div>
                      <p>Payments and orders through Frontstore are covered by buyer protection the team cannot remove.</p>
                    </div>
                  </aside>
              ...
              <div className="pd-listing">
                ...
                <div className="ct-wrap">
                  ...
                  <aside className="ct-rail">
                    ...
                    <div className="blog-convert">
                      <b>Prefer to book?</b>
                      <p>Skip the message and book a call in a couple of taps.</p>
                      <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book a call</button>
                      {(store.storefront_sections || []).includes("services") && (
          <button className="blog-convert-ghost" onClick={() => go("services")}>Browse services</button>
        )}
      ...
      {!isDesktop && (
        <nav className="ps-bottom">
          ...
          {(store.storefront_sections || []).includes("products") && (
          <button className={page === "products" ? "on" : ""} onClick={() => go("products")}><Package size={21} /><span>Shop</span></button>
        )}
          {(store.storefront_sections || []).includes("reviews") && (
      ...
      {notifyOpen && (
        <Sheet onClose={() => setNotifyOpen(false)} title={`Get the free SEO checklist`}>
          <p className="ps-sheet-sub"><Bell size={13} /> Drop your email and I will send the free 120-point Technical SEO checklist, plus the occasional tip worth reading.</p>
          <p className="ps-field-lbl">Your email</p>
          <input className="bk-input" value={notifyPhone} onChange={(e) => setNotifyPhone(e.target.value)} placeholder="you@example.com" inputMode="email" />
          ...
          <p className="ps-deposit">Your checklist arrives by email. No spam, unsubscribe any time, and your email is never shared with other stores.</p>
          <button className="ps-sheet-cta" disabled={!notifyPhone.trim() || notifyTopics.length === 0} onClick={() => { setNotifyOpen(false); ping(`Check your inbox for the checklist`); }}><Bell size={16} /> Send my checklist</button>
        </Sheet>
      )}
    ...
    <div className="rev-card">
      ...
      {rv.response && (
        <div className="rev-response">
          <div className="rev-response-head"><span className="rev-resp-av">{storeInitial || MOCK_STORE.initial}</span><b>Response from the team</b><span>{rv.response.when}</span></div>
...
const css = `
.ps-root{--brand:#4338ca;--brand-deep:#312e81;--gold:#c79a4b;--bg:#f5f5fb;--card:#fff;--ink:#181626;
  --muted:#79768c;--line:#e7e5f0;--ok:#1f9d57;--wa:#25d366;
  --shadow-sm:0 1px 2px rgba(24,22,38,.04),0 2px 8px rgba(24,22,38,.05);
  --shadow-md:0 4px 12px rgba(24,22,38,.06),0 10px 28px rgba(24,22,38,.08);
  --shadow-lg:0 8px 24px rgba(24,22,38,.09),0 20px 48px rgba(24,22,38,.11);
...
.ps-root :where(button){font-family:inherit;background:none;border:none;color:inherit;cursor:pointer;padding:0;transition:transform .15s ease,box-shadow .15s ease,opacity .15s ease,background-color .15s ease;}
.ps-root a{transition:opacity .15s ease,color .15s ease;}
.ps-root :where(button,a,[role="button"]):focus-visible{outline:2px solid var(--brand);outline-offset:2px;border-radius:6px;}
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#ebeafc;padding:7px 12px;border-radius:9px;margin-top:11px;max-width:100%;min-width:0}
...
.ps-notify{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:700;color:#fff;background:var(--brand);padding:8px 13px;border-radius:9px;box-shadow:0 4px 12px rgba(67,56,202,.28);cursor:pointer;flex-shrink:0}
.nt-topics{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:4px;}
.nt-topic{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:600;padding:9px 13px;border-radius:11px;border:1px solid var(--line);background:var(--card);color:#3c3a4e;cursor:pointer;}
.nt-topic.on{background:#ebeafc;border-color:var(--brand);color:var(--brand-deep);}
.ps-stats{display:flex;gap:10px;margin-top:16px;}
.ps-stats div{flex:1;background:var(--card);border:1px solid var(--line);border-radius:13px;padding:11px;text-align:center;box-shadow:var(--shadow-sm);}
...
.ps-bio{font-size:14px;line-height:1.55;color:#3c3a4e;margin-top:15px;}
...
.ps-signature{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:14px;margin-bottom:18px;box-shadow:0 8px 22px rgba(49,46,129,.07);}
.ps-sig-tag{display:inline-flex;align-items:center;gap:5px;font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:var(--brand-deep);background:#ebeafc;padding:4px 9px;border-radius:7px;}
...
.ps-book-btn,.ps-sheet-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:15px;padding:13px;border-radius:12px;box-shadow:0 6px 16px rgba(67,56,202,.3);}
.ps-book-btn:hover,.ps-sheet-cta:hover{box-shadow:0 8px 22px rgba(67,56,202,.4);transform:translateY(-1px);}
.ps-book-btn:active,.ps-sheet-cta:active{transform:translateY(2px);box-shadow:0 3px 10px rgba(67,56,202,.26);}
...
.ps-chips button{flex:0 0 auto;font-size:13px;font-weight:600;padding:8px 14px;border-radius:20px;background:var(--card);border:1px solid var(--line);color:#3c3a4e;}
...
.ps-card-body b{font-size:14px;font-weight:600;line-height:1.3;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;}
...
.ps-mini{font-size:13px;font-weight:700;padding:7px 15px;border-radius:9px;color:#fff;background:var(--brand);box-shadow:0 3px 8px rgba(67,56,202,.28);}
.ps-mini:hover{box-shadow:0 5px 14px rgba(67,56,202,.36);transform:translateY(-1px);}
.ps-mini:active{transform:translateY(1px);box-shadow:0 2px 6px rgba(67,56,202,.24);}
...
.ps-seeall:hover{background:#ebeafc;border-color:var(--brand);}

.ps-rating{display:flex;gap:18px;background:var(--card);border:1px solid var(--line);border-radius:16px;padding:16px;margin-bottom:14px;box-shadow:var(--shadow-sm);}
...
.ps-review{background:var(--card);border:1px solid var(--line);border-radius:14px;padding:14px;box-shadow:var(--shadow-sm);}
...
.ps-review p{font-size:13px;line-height:1.5;color:#3c3a4e;}

.ps-visit{background:var(--card);border:1px solid var(--line);border-radius:16px;overflow:hidden;margin-bottom:24px;box-shadow:var(--shadow-sm);}
...
.ps-addr{display:flex;align-items:flex-start;gap:7px;font-size:13.5px;color:#3c3a4e;line-height:1.5;}
...
.ps-dir:hover{background:#ebeafc;border-color:var(--brand);}
.ps-hours{list-style:none;margin-top:14px;border-top:1px solid var(--line);}
.ps-hours li{display:flex;justify-content:space-between;font-size:13px;padding:7px 0;border-bottom:1px solid var(--line);color:#4b4960;}
...
.ps-acc-item p{padding:0 14px 14px;font-size:13px;line-height:1.55;color:#3c3a4e;}

.ps-locked{background:#ebeafc;border:1px solid #d6d3f5;border-radius:14px;padding:14px;margin:16px 0 8px;}
...
.ps-locked p{font-size:12.5px;line-height:1.55;color:#59566e;margin-top:8px;}
...
.ps-prose{font-size:14px;line-height:1.65;color:#3c3a4e;margin-bottom:13px;}
...
.ps-foot-links button{font-size:12px;font-weight:600;color:#59566e;}
...
.ps-nav button{display:flex;align-items:center;justify-content:space-between;text-align:left;font-size:14.5px;font-weight:500;padding:11px 12px;border-radius:10px;color:#3c3a4e;}
.ps-nav button.on{background:#ebeafc;color:var(--brand-deep);font-weight:700;}
.ps-panel-actions{display:flex;flex-direction:column;gap:9px;margin-top:auto;}
.ps-act-book{display:flex;align-items:center;justify-content:center;gap:8px;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:13px;border-radius:12px;box-shadow:0 6px 16px rgba(67,56,202,.3);}
...
.ps-fab svg{position:relative;z-index:1;width:54px;height:54px;background:var(--brand);color:#fff;padding:16px;border-radius:50%;box-shadow:0 8px 20px rgba(67,56,202,.4);margin-top:-26px;}
...
.ps-field-lbl{font-size:12.5px;font-weight:700;color:#4b4960;margin:14px 0 8px;}
.ps-pick{display:flex;gap:9px;flex-wrap:wrap;}
.ps-pick button{font-size:13px;font-weight:600;padding:9px 15px;border-radius:11px;border:1px solid var(--line);background:var(--card);color:#3c3a4e;}
...
.bk-thumb.c2{background:linear-gradient(150deg,#1bb5d8,var(--brand));}
.bk-thumb.c3{background:linear-gradient(150deg,var(--brand),#8a86c4);}
...
.bk-svcbar{display:flex;align-items:center;justify-content:space-between;gap:10px;background:#ebeafc;border:1px solid #d6d3f5;border-radius:12px;padding:10px 13px;margin-bottom:16px;}
...
.bk-deposit-row{display:flex;align-items:center;justify-content:space-between;margin-top:16px;padding:12px 14px;background:#ebeafc;border:1px solid #d6d3f5;border-radius:12px;}
...
.ps-search-lbl{font-size:12px;font-weight:700;color:#4b4960;margin-bottom:9px;}
...
.ps-toast{position:fixed;left:50%;bottom:96px;transform:translateX(-50%);background:#181626;color:#fff;font-size:13px;font-weight:600;padding:11px 18px;border-radius:11px;box-shadow:0 8px 24px rgba(0,0,0,.25);z-index:90;animation:pstoast .2s;}
...
.pd-head-book{display:flex;align-items:center;gap:7px;background:var(--brand);color:#fff;font-weight:700;font-size:14px;padding:11px 18px;border-radius:12px;box-shadow:0 5px 14px rgba(67,56,202,.3);}
...
.pd-book{display:flex;align-items:center;gap:7px;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px 22px;border-radius:12px;box-shadow:0 6px 16px rgba(67,56,202,.3);}
...
.pd-tabs button{font-size:14px;font-weight:600;padding:10px 16px;border-radius:10px;color:#4b4960;white-space:nowrap;}
.pd-tabs button:hover{background:#f3e7e1;}
.pd-tabs button.on{background:#ebeafc;color:var(--brand-deep);font-weight:700;}
...
.pd-railcard{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:17px;box-shadow:var(--shadow-sm);transition:box-shadow .2s ease;}
.pd-railcard:hover{box-shadow:var(--shadow-md);}
.pd-railcard h3{font-family:'Fraunces';font-weight:600;font-size:16px;margin-bottom:10px;}
.pd-railcard>p{font-size:13px;line-height:1.55;color:#3c3a4e;}
...
.pd-raillink:hover{color:var(--brand-deep);gap:7px;}
...
.pd-railbtns button:hover{background:#ebeafc;border-color:var(--brand);}
.pd-railcard.trust{background:#ebeafc;border-color:#d6d3f5;}
.pd-trust-h{display:flex;align-items:center;gap:7px;font-size:13.5px;font-weight:700;color:var(--brand-deep);}
.pd-railcard.trust p{font-size:12px;line-height:1.55;color:#59566e;margin-top:8px;}
...
.pd-filter button{font-size:13px;font-weight:600;padding:8px 16px;border-radius:20px;background:var(--card);border:1px solid var(--line);color:#3c3a4e;}
...
.feat-tag{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--brand-deep);background:#ebeafc;padding:5px 11px;border-radius:8px;}
...
.feat-card{flex:0 0 76%;max-width:280px;background:var(--card);border:1px solid var(--line);border-radius:16px;overflow:hidden;box-shadow:0 8px 22px rgba(49,46,129,.08);display:flex;flex-direction:column;}
...
.feat-thumb.product{background:linear-gradient(150deg,#1bb5d8,var(--gold));}
...
.feat-body b{font-size:15px;font-weight:700;line-height:1.3;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;}
...
.feat-cta{font-size:13px;font-weight:700;padding:8px 18px;border-radius:10px;color:#fff;background:var(--brand);box-shadow:0 4px 10px rgba(67,56,202,.3);}
...
.blog-card:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(49,46,129,.1);}
...
.blog-img.c2{background:linear-gradient(135deg,#1bb5d8,var(--brand-deep));}
...
.blog-body p{font-size:13px;color:#59566e;line-height:1.5;}
...
.blog-topics button{font-size:13px;font-weight:600;padding:8px 16px;border-radius:20px;background:var(--card);border:1px solid var(--line);color:#3c3a4e;}
...
.blog-hero:hover{transform:translateY(-2px);box-shadow:0 14px 32px rgba(49,46,129,.1);}
...
.blog-hero-img.c2{background:linear-gradient(135deg,#1bb5d8,var(--brand-deep));}
...
.blog-hero-body>p{font-size:14px;color:#4b4960;line-height:1.55;}
...
.author-card>p{font-size:12.5px;line-height:1.55;color:#4b4960;}
...
.blog-convert{background:#ebeafc;border:1px solid #d6d3f5;border-radius:16px;padding:17px;}
.blog-convert>b{font-family:'Fraunces';font-weight:600;font-size:16px;}
.blog-convert>p{font-size:12.5px;color:#59566e;line-height:1.5;margin:6px 0 13px;}
.blog-convert-cta{display:flex;align-items:center;justify-content:center;gap:7px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:13.5px;padding:11px;border-radius:11px;box-shadow:0 6px 14px rgba(67,56,202,.28);}
...
.ab-lede{font-size:15.5px;line-height:1.6;color:#4b4960;max-width:none;}
...
.ab-bio{font-size:14px;line-height:1.6;color:#4b4960;margin:13px 0;}
.ab-chips{display:flex;flex-wrap:wrap;gap:7px;margin-bottom:14px;}
.ab-chips span{font-size:11.5px;font-weight:600;color:var(--brand-deep);background:#ebeafc;border:1px solid #ecd2dd;padding:5px 11px;border-radius:16px;}
...
.ab-approach-grid p{font-size:13px;line-height:1.55;color:#4b4960;}
...
.ab-addr{font-size:13px;line-height:1.5;color:#3c3a4e;margin:12px 0 8px;}
...
.ab-follow-icons button{width:40px;height:40px;border-radius:11px;display:grid;place-items:center;background:#ebeafc;border:1px solid #ecd2dd;color:var(--brand-deep);transition:background .12s,color .12s;}
...
.ab-book-m{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;margin-top:18px;background:var(--brand);color:#fff;font-weight:700;font-size:15px;padding:14px;border-radius:13px;box-shadow:0 8px 18px rgba(67,56,202,.3);}
.ab-para{font-size:14.5px;line-height:1.65;color:#4b4960;max-width:none;margin-top:13px;}
...
.ab-offer-card:hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(49,46,129,.09);border-color:#d6d3f5;}
.ab-offer-card b{display:block;font-size:15.5px;font-weight:700;margin-bottom:6px;}
.ab-offer-card p{font-size:13px;line-height:1.55;color:#4b4960;margin-bottom:11px;}
...
.ab-fact-v{font-size:13.5px;color:#3c3a4e;line-height:1.45;}
.ab-grid-m{grid-template-columns:1fr;}
.ab-facts-m{grid-template-columns:1fr;}
.ab-verified{display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:700;color:var(--brand-deep);background:#ebeafc;border:1px solid #ecd2dd;padding:4px 10px;border-radius:16px;margin-top:8px;}
...
.ab-creds li{display:flex;align-items:flex-start;gap:8px;font-size:13px;line-height:1.45;color:#3c3a4e;}
...
.ab-shot:hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(49,46,129,.14);}
...
.ab-shot.c2{background:linear-gradient(150deg,#1bb5d8,var(--brand-deep));}
...
.ab-review{position:relative;background:#ebeafc;border:1px solid #d6d3f5;border-radius:18px;padding:24px;margin:28px 0;}
...
.ab-journal-item:hover{transform:translateY(-2px);box-shadow:0 10px 22px rgba(49,46,129,.09);border-color:#d6d3f5;}
...
.faq-cats button{display:flex;align-items:center;gap:10px;width:100%;text-align:left;font-size:13px;font-weight:600;color:#3c3a4e;padding:10px 11px;border-radius:9px;transition:background .12s,color .12s;}
.faq-cats button:hover{background:#ebeafc;color:var(--brand-deep);}
.faq-cats button svg{color:var(--brand);flex:0 0 auto;}
.faq-help{background:#ebeafc;border:1px solid #d6d3f5;border-radius:16px;padding:17px;}
.faq-help>b{font-family:'Fraunces';font-weight:600;font-size:16px;}
.faq-help>p{font-size:12.5px;color:#59566e;line-height:1.5;margin:6px 0 13px;}
.faq-help-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:13.5px;padding:11px;border-radius:11px;box-shadow:0 6px 14px rgba(67,56,202,.28);}
...
.faq-empty{background:var(--card);border:1px solid var(--line);border-radius:13px;padding:18px;font-size:13.5px;color:#4b4960;}
...
.ct-wa{display:flex;align-items:center;justify-content:center;gap:9px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:15.5px;padding:15px;border-radius:13px;box-shadow:0 8px 18px rgba(67,56,202,.3);}
...
.ct-form-sub{font-size:13px;line-height:1.5;color:#4b4960;margin-bottom:15px;}
...
.ct-send{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14px;padding:13px;border-radius:11px;box-shadow:0 6px 14px rgba(67,56,202,.28);}
...
.ct-map-pin{position:absolute;left:50%;top:44%;transform:translate(-50%,-50%) rotate(-45deg);display:grid;place-items:center;width:34px;height:34px;border-radius:50% 50% 50% 0;background:var(--brand);color:#fff;box-shadow:0 6px 13px rgba(49,46,129,.4);z-index:2}
...
.ct-hours-list li{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:9px 13px;font-size:12.5px;color:#4b4960;}
.ct-hours-list li+li{border-top:1px solid var(--line);}
.ct-hours-list li b{font-weight:600;color:#3c3a4e;}
...
.rf-list li{display:flex;align-items:flex-start;gap:10px;font-size:14px;line-height:1.55;color:#3c3a4e;}
...
.rf-keys li span{font-size:12.5px;color:#4b4960;}
...
.rf-related button{display:flex;align-items:center;justify-content:space-between;width:100%;font-size:13.5px;font-weight:600;color:#3c3a4e;padding:11px;border-radius:10px;transition:background .12s,color .12s;}
.rf-related button:hover{background:#ebeafc;color:var(--brand-deep);}
...
.tm-intro{font-size:15px;line-height:1.6;color:#4b4960;max-width:70ch;}
...
.tm-section p{font-size:14px;line-height:1.65;color:#3c3a4e;margin-bottom:10px;}
...
.tm-contents button{text-align:left;font-size:12.5px;font-weight:600;color:#3c3a4e;padding:8px 10px;border-radius:8px;transition:background .12s,color .12s;}
.tm-contents button:hover{background:#ebeafc;color:var(--brand-deep);}
...
.pf-shot:hover{transform:translateY(-2px);box-shadow:0 14px 30px rgba(49,46,129,.16);}
...
.pf-shot.c2{background:linear-gradient(150deg,#1bb5d8,var(--brand-deep));}
...
.art-hero.c2{background:linear-gradient(150deg,#1bb5d8,var(--brand-deep));}
...
.svc-intro{font-size:14px;color:#59566e;line-height:1.5;margin:-4px 0 24px;max-width:620px;}
...
.svc-fgroup h4{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:#79768c;margin-bottom:8px;}
.svc-radios{display:flex;flex-direction:column;gap:1px;}
.svc-radios button{text-align:left;font-size:13.5px;font-weight:500;padding:8px 10px;border-radius:9px;color:#3c3a4e;}
.svc-radios button:hover{background:#f3e7e1;}
.svc-radios button.on{background:#ebeafc;color:var(--brand-deep);font-weight:700;}
.svc-book-card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:16px;box-shadow:0 8px 22px rgba(49,46,129,.06);}
...
.svc-next{font-size:13px;color:#4b4960;margin:10px 0 14px;}
.svc-next b{color:var(--ink);}
.svc-book-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px;border-radius:12px;box-shadow:0 6px 16px rgba(67,56,202,.3);}
...
.svc-card:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(49,46,129,.1);}
...
.svc-card-body>b{font-size:15px;font-weight:700;line-height:1.3;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;}
.svc-card-dur{display:flex;align-items:center;gap:5px;font-size:12px;color:var(--muted);margin-top:3px;}
.svc-card-desc{font-size:12.5px;color:#59566e;line-height:1.5;margin-top:8px;flex:1;}
...
.svc-card-book{display:inline-flex;align-items:center;gap:3px;font-size:13px;font-weight:700;color:#fff;background:var(--brand);padding:8px 14px;border-radius:10px;box-shadow:0 4px 10px rgba(67,56,202,.3);}
...
.svc-deliv li{display:flex;align-items:flex-start;gap:8px;font-size:12.5px;color:#4b4960;line-height:1.4;}
...
.rev-leave{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px;border-radius:12px;box-shadow:0 6px 16px rgba(67,56,202,.3);}
...
.rev-filter-m button{display:inline-flex;align-items:center;gap:4px;font-size:12.5px;font-weight:600;padding:7px 13px;border-radius:10px;border:1px solid var(--line);background:var(--card);color:#3c3a4e;}
...
.rev-photostrip h4{font-size:12.5px;font-weight:700;color:#4b4960;margin-bottom:10px;}
...
.rev-ph.c2{background:linear-gradient(150deg,#1bb5d8,var(--brand));}
...
.rev-card-svc{display:inline-block;font-size:11px;font-weight:700;color:var(--brand-deep);background:#ebeafc;padding:3px 9px;border-radius:7px;margin:11px 0 8px;}
.rev-card-text{font-size:13.5px;line-height:1.55;color:#3c3a4e;}
...
.rev-response p{font-size:12.5px;line-height:1.5;color:#4b4960;}
.rev-form-note{display:flex;align-items:flex-start;gap:7px;font-size:12.5px;color:#59566e;line-height:1.5;background:#ebeafc;border:1px solid #d6d3f5;border-radius:11px;padding:11px 12px;margin-bottom:6px;}
...
.rev-input:focus,.rev-textarea:focus{outline:none;border-color:var(--brand);box-shadow:0 0 0 3px rgba(67,56,202,.12);}
