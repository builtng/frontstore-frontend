...
const MOCK_PRODUCT_FAQS = [
  ["How long does delivery take?", "Textbooks and workbooks are delivered within 2 to 4 days within Lagos. Nationwide delivery takes 5 to 8 days."],
  ["Can I return a product?", "Unopened books and stationery can be returned within 7 days. Opened workbooks cannot be returned unless they are faulty or incorrectly supplied."],
  ["Do you ship outside Lagos?", "Yes, we ship nationwide. Delivery timing and cost are shown at checkout based on your location."],
  ["Can I order a full school kit?", "Yes. The student stationery pack and workbook bundles are available for purchase together. Message us for a custom kit quote."],
...
const MOCK_SERVICE_FAQS = [
  ["How do I enrol my child?", "Pick the programme that matches your child's level, choose a start date and pay securely through Frontstore. Enrolment is confirmed on WhatsApp."],
  ["How do deposits work?", "A deposit secures your child's place when you enrol. The balance is due before the first session. The deposit amount is shown at checkout."],
  ["Can I change or cancel an enrolment?", "You can transfer to another intake up to 7 days before the start date. After that, the deposit is held against a future session."],
  ["What is the class size?", "Group classes are capped at 8 to 10 students. Private 1-on-1 tutoring is available for students who need individual attention."],
...
const MOCK_CATS = ["Exams", "Classes", "Private", "Programmes", "Online"];
      ...
      {search && (
        <div className="ps-overlay" onClick={() => setSearch(false)}>
          <div className="ps-search-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ps-chips">{["WAEC Prep", "JAMB Package", "Private Tutoring", "Past Questions", "Summer Intensive"].map((c: any) => (<button key={c} onClick={() => { setSearch(false); ping("Searching " + c); }}>{c}</button>))}</div>
          </div>
        </div>
      )}
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#e0e7ff;padding:7px 12px;border-radius:9px;margin-top:11px;}
