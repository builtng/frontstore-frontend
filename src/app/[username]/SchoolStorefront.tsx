...
interface SchoolStorefrontProps {
  username: string;
  store: StoreType;
  categories: Category[];
  products: Product[];
...
const MOCK_SERVICE_INFO = {
  s1: { includes: ["All core WAEC subjects covered: Maths, English, Physics, Chemistry, Biology", "Small group classes of maximum 10 students", "Weekly mock tests and exam drills", "Past question practice and marking", "Progress report sent to parents monthly", "WhatsApp group for students and parents"] },
  s2: { includes: ["Use of English: comprehension, summary, lexis and structure", "Up to 3 subject areas across science, arts or commercial", "JAMB past question practice with timed drills", "Mock CBT exam in the final two weeks", "CBT training included", "Weekly performance summary shared with parents"] },
  s3: { includes: ["Dedicated tutor matched to your child's pace and school", "Four sessions per month, each 60 minutes", "Sessions timetabled around school schedule", "Direct feedback to parent after each session", "Homework support and continuous assessment practice", "Change tutor at any time if the match is not right"] },
  s4: { includes: ["Classes for JSS1, JSS2 and JSS3", "Core subjects: Maths, English, Basic Science, Social Studies", "Maximum 8 students per group", "Following the Nigerian school curriculum exactly", "End of term test and report card issued", "After-school and Saturday session options"] },
  s5: { includes: ["Classes for SS1, SS2 and SS3", "Core and elective subjects tailored to student's school combination", "Intensive exam practice in the final month of term", "Past papers and marking guide included", "Saturday and after-school session options available", "Parent performance report issued mid-term and end of term"] },
  s6: { includes: ["Monday to Friday intensive tuition across core subjects", "Catch-up programme for students who fell behind in the term", "A gap-fill diagnostic test in the first week", "Daily homework and review sessions", "End of programme assessment and report", "Builds a strong foundation before the new term begins"] },
  s7: { includes: ["Federal Government College Common Entrance preparation", "LASU and private institution admission test practice", "Subject combination advice based on the specific exam", "Practice tests under timed, exam conditions", "Full past paper archive for each target institution", "Application guidance and deadline tracking"] },
  s8: { includes: ["Live Zoom sessions with a qualified tutor", "Recordings available for 14 days after each session", "WhatsApp support group for the subject", "PDF notes and past question packs shared each week", "Monthly performance report sent to parents", "Session times flexible to suit your schedule"] },
};
...
const MOCK_NOTIFY_TOPICS = [["results", "Student results"], ["programmes", "New programmes"], ["tips", "Study tips"], ["news", "Academy news"]];
const MOCK_HOURS = [
  ["Mon", "9:00am - 7:00pm"], ["Tue", "9:00am - 7:00pm"], ["Wed", "9:00am - 7:00pm"],
  ["Thu", "9:00am - 7:00pm"], ["Fri", "9:00am - 7:00pm"], ["Sat", "8:00am - 5:00pm"], ["Sun", "Closed"],
];
const MOCK_NAV = [
  ["home", "Home"], ["services", "Programmes"], ["products", "Shop"], ["portfolio", "Gallery"], ["reviews", "Reviews"], ["blog", "Blog"],
...
export default function SchoolStorefront({
  ...
  const customBrandColor = store.primary_color || '#4f46e5';
  const customBrandDeep = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 80%, black)` : '#4338ca';
  const customTint = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 14%, white)` : '#eef2ff';
...
`;
  ...
  const serviceView = () => {
    ...
    const steps = [
      ["Enrol and pay a deposit", "Choose the programme, pick a start date and pay a deposit to secure your child's place. We confirm on WhatsApp."],
      ["We run a short diagnostic", "In the first session, we assess your child's current level to ensure they are placed in the right group."],
      ["Classes begin on schedule", "Your child attends as agreed. After-school and Saturday options are available to fit your schedule."],
      ["Monthly progress reports", "We share a detailed progress report with you every month so you always know where your child stands."],
    ];
    const brings = ["A dedicated, experienced tutor for the subject", "Structured past question packs and class materials", "Monthly parent progress reports on WhatsApp"];
    return (
                    ...
                    <div className="blogp-body">
                      ...
                      <aside className="blogp-rail">
                        <div className="author-card">
                          ...
                          <p>{DUMMY_AUTHOR.bio}</p>
                          <div className="author-cred"><Sparkles size={13} /> Over 8 years helping students across Lagos succeed</div>
                          <button className="author-link" onClick={() => go("about")}>More about {DUMMY_AUTHOR.name.split(" ")[0]} <ChevronRight size={14} /></button>
                        </div>
                        <div className="blog-convert">
                          <b>Enjoyed the read?</b>
                  ...
                  <div className="ab-main">
                    ...
                    <div className="ab-approach">
                      ...
                      <div className="ab-approach-grid">
                        <div><span className="ab-num">01</span><b>We learn your goals</b><p>Every registration starts by understanding what you need, so the session suits your goals.</p></div>
                        <div><span className="ab-num">02</span><b>The right approach</b><p>We bring the right tutors and materials for you, not a one size fits all routine.</p></div>
                        <div><span className="ab-num">03</span><b>Made to last</b><p>From a quick top-up class to a full term programme, it is done properly, not just quickly.</p></div>
                      </div>
                    </div>
              ...
              <div className="pd-listing">
                ...
                <div className="ct-wrap">
                  <div className="ct-main">
                    <p className="svc-intro">Reach us on WhatsApp for a quick reply about enrolment, or send an enquiry by email. A real person from Brightfield will answer, usually within one working day.</p>
                    {contactChannels()}
                    {contactForm()}
                  </div>
...
const css = `
.ps-root{--brand:#4f46e5;--brand-deep:#4338ca;--gold:#c79a4b;--bg:#f4f8f8;--card:#fff;--ink:#18272a;
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#e0e7ff;padding:7px 12px;border-radius:9px;margin-top:11px;max-width:100%;min-width:0}
...
.nt-topic.on{background:#e0e7ff;border-color:var(--brand);color:var(--brand-deep);}
...
.ps-sig-tag{display:inline-flex;align-items:center;gap:5px;font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:var(--brand-deep);background:#e0e7ff;padding:4px 9px;border-radius:7px;}
...
.ps-seeall:hover{background:#e0e7ff;border-color:var(--brand);}
...
.ps-dir:hover{background:#e0e7ff;border-color:var(--brand);}
...
.ps-locked{background:#e0e7ff;border:1px solid #c8e6e2;border-radius:14px;padding:14px;margin:16px 0 8px;}
...
.ps-nav button.on{background:#e0e7ff;color:var(--brand-deep);font-weight:700;}
...
.bk-svcbar{display:flex;align-items:center;justify-content:space-between;gap:10px;background:#e0e7ff;border:1px solid #c8e6e2;border-radius:12px;padding:10px 13px;margin-bottom:16px;}
...
.bk-deposit-row{display:flex;align-items:center;justify-content:space-between;margin-top:16px;padding:12px 14px;background:#e0e7ff;border:1px solid #c8e6e2;border-radius:12px;}
...
.pd-tabs button.on{background:#e0e7ff;color:var(--brand-deep);font-weight:700;}
...
.pd-railbtns button:hover{background:#e0e7ff;border-color:var(--brand);}
.pd-railcard.trust{background:#e0e7ff;border-color:#c8e6e2;}
...
.feat-tag{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--brand-deep);background:#e0e7ff;padding:5px 11px;border-radius:8px;}
...
.blog-convert{background:#e0e7ff;border:1px solid #c8e6e2;border-radius:16px;padding:17px;}
...
.ab-chips span{font-size:11.5px;font-weight:600;color:var(--brand-deep);background:#e0e7ff;border:1px solid #ecd2dd;padding:5px 11px;border-radius:16px;}
...
.ab-follow-icons button{width:40px;height:40px;border-radius:11px;display:grid;place-items:center;background:#e0e7ff;border:1px solid #ecd2dd;color:var(--brand-deep);transition:background .12s,color .12s;}
...
.ab-verified{display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:700;color:var(--brand-deep);background:#e0e7ff;border:1px solid #ecd2dd;padding:4px 10px;border-radius:16px;margin-top:8px;}
...
.ab-review{position:relative;background:#e0e7ff;border:1px solid #c8e6e2;border-radius:18px;padding:24px;margin:28px 0;}
...
.faq-cats button:hover{background:#e0e7ff;color:var(--brand-deep);}
.faq-cats button svg{color:var(--brand);flex:0 0 auto;}
.faq-help{background:#e0e7ff;border:1px solid #c8e6e2;border-radius:16px;padding:17px;}
...
.rf-related button:hover{background:#e0e7ff;color:var(--brand-deep);}
...
.tm-contents button:hover{background:#e0e7ff;color:var(--brand-deep);}
...
.svc-radios button.on{background:#e0e7ff;color:var(--brand-deep);font-weight:700;}
...
.rev-card-svc{display:inline-block;font-size:11px;font-weight:700;color:var(--brand-deep);background:#e0e7ff;padding:3px 9px;border-radius:7px;margin:11px 0 8px;}
...
.rev-form-note{display:flex;align-items:flex-start;gap:7px;font-size:12.5px;color:#54636a;line-height:1.5;background:#e0e7ff;border:1px solid #c8e6e2;border-radius:11px;padding:11px 12px;margin-bottom:6px;}
