...
interface WhatsAppTVStorefrontProps {
  username: string;
  store: StoreType;
  categories: Category[];
  products: Product[];
...
const MOCK_SERVICE_INFO = {
  ...
  s3: { includes: ["A full seven days on our Status", "Posted daily through the week", "Our best value for sales and launches", "Caption and contact details included", "Screenshot proof through the run", "Ideal when you need steady pressure"] },
  s4: { includes: ["A video advert on our Status", "Up to 30 seconds, live for 24 hours", "Motion that stops the scroll", "Caption and contact details included", "Screenshot proof of the live post", "Send your clip or we can edit one"] },
  s5: { includes: ["Posted first, ahead of the queue", "Repeated through the day for max reach", "Our highest visibility single placement", "Caption and contact details included", "Screenshot proof of each repost", "Best for time sensitive offers"] },
  s6: { includes: ["Countdown posts up to your event", "Multiple reminders as the day nears", "Drives turnout, not just awareness", "Event details and contact included", "Screenshot proof of each post", "Perfect for parties, shows and launches"] },
...
const MOCK_HOURS = [
  ...
  ["Thu", "9:00am - 9:00pm"], ["Fri", "9:00am - 9:00pm"], ["Sat", "9:00am - 9:00pm"], ["Sun", "11:00am - 8:00pm"],
];
const MOCK_NAV = [
  ["home", "Home"], ["services", "Rates"], ["products", "Add-ons"], ["portfolio", "Campaigns"], ["reviews", "Reviews"], ["blog", "Blog"],
...
export default function WhatsAppTVStorefront({
  username,
  store,
  categories,
  products,
  ...
  const customBrandColor = store.primary_color || '#6d28d9';
  const customBrandDeep = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 80%, black)` : '#4c1d95';
...
.sv-cat.c0{background:var(--brand);}.sv-cat.c1{background:#9a5fd6;}.sv-cat.c2{background:#7e63c0;}.sv-cat.c3{background:var(--brand-deep);}
...
.sv-card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:18px;box-shadow:0 8px 30px rgba(109,40,217,.08);}
...
`;
  ...
  const serviceView = () => {
    ...
    const steps = [
      ["Pick your slot and pay", "Choose the day your advert should run. A small deposit holds the slot, and you see the amount before you confirm."],
      ["Send us your advert", "Send your flyer, photos or details on WhatsApp. Need one made? Add flyer design and we create it for you."],
      ["It goes live on our Status", "Your advert posts to our full Status audience on the agreed day, in the placement you booked."],
      ["You get proof and results", "We send a screenshot of your live advert, and the views and replies come straight to you."],
    ];
    const brings = ["A screenshot proof of your live advert", "Posted to our full daily Status audience", "Your caption and contact details included"];
    return (
      <div className="sv-wrap">
        ...
        <div className="sv-grid">
          <div className="sv-main">
            ...
            <section className="sv-sec">
              <h2>What every advert includes</h2>
          ...
          <aside className="sv-side">
            <div className="sv-card">
              ...
              <button className="sv-book" onClick={() => openBooking(s)}><Calendar size={17} /> Book a slot</button>
              <a className="sv-enquire" href={waHref} target="_blank" rel="noreferrer"><MessageCircle size={16} /> Ask a question</a>
              <p className="sv-note">A deposit of about {money(deposit)} secures your slot. The balance is due before your advert goes live.</p>
    ...
    return (
      <Sheet onClose={() => setBookOpen(false)} onBack={onBack} title={(titles as any)[bookStep]}>
        ...
        {bookStep === "review" && (<>
          ...
          <textarea className="bk-input bk-textarea" value={bookNote} onChange={(e) => setBookNote(e.target.value)} placeholder="Your advert, link, budget, anything useful" />
          <div className="bk-deposit-row"><span><Lock size={13} /> Deposit to secure</span><b>{money(deposit)}</b></div>
          <p className="ps-deposit">Frontstore holds your deposit until your advert goes live, then the {money(bookSvc.price - deposit)} balance is due before it posts. Covered by buyer protection.</p>
  ...
  const aboutBody = () => (<>
      ...
      <div className="ab-offer-grid ab-grid-m">
        {MOCK_OFFERINGS.map(([t, d, pg]: any) => (
          <button key={t} className="ab-offer-card" onClick={() => go(pg)}>
            <b>{t}</b>
            <p>{d}</p>
            <span className="ab-offer-link">{pg === "products" ? "View add-ons" : "View rates"} <ChevronRight size={14} /></span>
          </button>
        ))}
      </div>
    </div>
    ...
    <div className="ps-about-grid">
      {DUMMY_STORE.orders ? <div><b>{DUMMY_STORE.orders}</b><span>status views</span></div> : null}
    ...
    <button className="ab-book-m" onClick={() => openBooking()}><Calendar size={16} /> Book a slot</button></>);
  const faqSections = () => (
      ...
      <div className="ct-visit">
        ...
        <div className="ct-hours">
          ...
          {hasHours ? (
            ...
            <p className="ct-hours-empty">This shop hasn't added opening hours yet. Message on WhatsApp to check availability.</p>
          )}
        </div>
  ...
  const portfolioBody = () => (<>
    <p className="svc-intro">A look at adverts and brands we have recently featured on our Status.</p>
    {portfolioChips()}
    {portfolioGrid()}
    <button className="ab-book-m" onClick={() => openBooking()}><Calendar size={16} /> Book a slot</button>
    {portfolioFollow()}
      ...
      {!isDesktop && (
        <div className="ps-col">
          ...
          <button className="ps-seeall" onClick={() => go("reviews")}>See all reviews <ChevronRight size={16} /></button>
            ...
            {page === "services" && <Sub title="Rates" slug={DUMMY_STORE.slug}>{servicesGrid("ps-grid", SERVICES)}<StoreFoot onNav={go}  slug={DUMMY_STORE.slug} /></Sub>}
            {page === "service" && serviceView()}
            {page === "products" && <Sub title="Add-ons" slug={DUMMY_STORE.slug}>{productsGrid("ps-grid", PRODUCTS)}<StoreFoot onNav={go}  slug={DUMMY_STORE.slug} /></Sub>}
            {page === "portfolio" && <Sub title="Campaigns" slug={DUMMY_STORE.slug}>{portfolioBody()}<StoreFoot onNav={go}  slug={DUMMY_STORE.slug} /></Sub>}
              ...
              <div className="pd-listing">
                ...
                {page === "services" && (
                  <div className="svc-page">
                    <p className="svc-intro">Book a slot with our team. A deposit secures your placement, with the balance due before it goes live.</p>
                ...
                {page === "products" && (
                  <div className="svc-page">
                    <p className="svc-intro">Optional extras that attach to your advert, nothing is shipped. We get the design or boost done and it goes out with your advert, usually within a day.</p>
                    ...
                    <div className="svc-body">
                      <aside className="svc-rail">
                        <div className="svc-search">
                          <Search size={16} />
                          <input value={prodQuery} onChange={(e) => setProdQuery(e.target.value)} placeholder="Search add-ons" />
                        ...
                        <div className="svc-book-card">
                          ...
                          <ul className="svc-deliv">
                            <li><Truck size={14} /> Most designs ready within a day</li>
                            <li><MapPin size={14} /> Boosts apply to your live advert</li>
                          </ul>
                          <button className="svc-book-cta" onClick={() => setBag(true)}><ShoppingBag size={16} /> View bag{bagCount > 0 ? ` (${bagCount})` : ""}</button>
                          <button className="svc-msg" onClick={() => handleWa("Hello! I'm interested in your services.")}><WhatsApp size={15} /> Ask about an add-on</button>
                        </div>
                      </aside>
                ...
                {page === "blog" && (
                  <div className="blogp">
                    <p className="svc-intro">Tips, ideas and advertising notes from {DUMMY_AUTHOR.name} and the team at {DUMMY_STORE.name}.</p>
                    <div className="blog-topics">
                    ...
                    <div className="blogp-body">
                      ...
                      <aside className="blogp-rail">
                        <div className="author-card">
                          ...
                          <p>{DUMMY_AUTHOR.bio}</p>
                          <div className="author-cred"><Sparkles size={13} /> Reaching tens of thousands across Delta every single day</div>
                          <button className="author-link" onClick={() => go("about")}>More about {DUMMY_AUTHOR.name.split(" ")[0]} <ChevronRight size={14} /></button>
                        </div>
                        <div className="blog-convert">
                          <b>Enjoyed the read?</b>
          ...
          <button className="blog-convert-ghost" onClick={() => go("products")}>View add-ons</button>
            ...
            {page === "portfolio" && (
              <div className="pd-listing">
                ...
                <p className="svc-intro">A look at adverts and brands we have recently featured on our Status. Tap any image to see it larger.</p>
                {portfolioChips()}
                  ...
                  <div className="ab-main">
                    ...
                    <div className="ab-approach">
                      ...
                      <div className="ab-approach-grid">
                        <div><span className="ab-num">01</span><b>You pick a placement</b><p>Choose the advert that fits your goal and budget, then send your flyer or details, or let us design one for you.</p></div>
                        <div><span className="ab-num">02</span><b>It goes live on our Status</b><p>Your advert posts to our full audience on the day you booked, in the slot you chose.</p></div>
                        <div><span className="ab-num">03</span><b>You see the results</b><p>We send a screenshot of your live advert, and the views, calls and replies start coming in.</p></div>
                      </div>
                    </div>
...
const css = `
.ps-root{--brand:#6d28d9;--brand-deep:#4c1d95;--gold:#c79a4b;--bg:#f7f5fc;--card:#fff;--ink:#1c1726;
  --muted:#7c7390;--line:#ece8f3;--ok:#1f9d57;--wa:#25d366;
  --shadow-sm:0 1px 2px rgba(28,23,38,.04),0 2px 8px rgba(28,23,38,.05);
  --shadow-md:0 4px 12px rgba(28,23,38,.06),0 10px 28px rgba(28,23,38,.08);
  --shadow-lg:0 8px 24px rgba(28,23,38,.09),0 20px 48px rgba(28,23,38,.11);
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#efe9fd;padding:7px 12px;border-radius:9px;margin-top:11px;max-width:100%;min-width:0}
...
.ps-notify{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:700;color:#fff;background:var(--brand);padding:8px 13px;border-radius:9px;box-shadow:0 4px 12px rgba(109,40,217,.28);cursor:pointer;flex-shrink:0}
.nt-topics{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:4px;}
.nt-topic{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:600;padding:9px 13px;border-radius:11px;border:1px solid var(--line);background:var(--card);color:#423a52;cursor:pointer;}
.nt-topic.on{background:#efe9fd;border-color:var(--brand);color:var(--brand-deep);}
...
.ps-bio{font-size:14px;line-height:1.55;color:#423a52;margin-top:15px;}
...
.ps-signature{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:14px;margin-bottom:18px;box-shadow:0 8px 22px rgba(76,29,149,.07);}
.ps-sig-tag{display:inline-flex;align-items:center;gap:5px;font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:var(--brand-deep);background:#efe9fd;padding:4px 9px;border-radius:7px;}
...
.ps-book-btn,.ps-sheet-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:15px;padding:13px;border-radius:12px;box-shadow:0 6px 16px rgba(109,40,217,.3);}
.ps-book-btn:hover,.ps-sheet-cta:hover{box-shadow:0 8px 22px rgba(109,40,217,.4);transform:translateY(-1px);}
.ps-book-btn:active,.ps-sheet-cta:active{transform:translateY(2px);box-shadow:0 3px 10px rgba(109,40,217,.26);}
...
.ps-chips button{flex:0 0 auto;font-size:13px;font-weight:600;padding:8px 14px;border-radius:20px;background:var(--card);border:1px solid var(--line);color:#423a52;}
...
.ps-card-thumb.prod{background:linear-gradient(150deg,#d24aa0,var(--gold));}
...
.ps-mini{font-size:13px;font-weight:700;padding:7px 15px;border-radius:9px;color:#fff;background:var(--brand);box-shadow:0 3px 8px rgba(109,40,217,.28);}
.ps-mini:hover{box-shadow:0 5px 14px rgba(109,40,217,.36);transform:translateY(-1px);}
...
.ps-seeall:hover{background:#efe9fd;border-color:var(--brand);}
...
.ps-review p{font-size:13px;line-height:1.5;color:#423a52;}
...
.ps-addr{display:flex;align-items:flex-start;gap:7px;font-size:13.5px;color:#423a52;line-height:1.5;}
...
.ps-dir:hover{background:#efe9fd;border-color:var(--brand);}
.ps-hours{list-style:none;margin-top:14px;border-top:1px solid var(--line);}
.ps-hours li{display:flex;justify-content:space-between;font-size:13px;padding:7px 0;border-bottom:1px solid var(--line);color:#524a62;}
...
.ps-acc-item p{padding:0 14px 14px;font-size:13px;line-height:1.55;color:#423a52;}

.ps-locked{background:#efe9fd;border:1px solid #ddd0f7;border-radius:14px;padding:14px;margin:16px 0 8px;}
...
.ps-locked p{font-size:12.5px;line-height:1.55;color:#5f5670;margin-top:8px;}
...
.ps-prose{font-size:14px;line-height:1.65;color:#423a52;margin-bottom:13px;}
...
.ps-foot-links button{font-size:12px;font-weight:600;color:#5f5670;}
...
.ps-nav button{display:flex;align-items:center;justify-content:space-between;text-align:left;font-size:14.5px;font-weight:500;padding:11px 12px;border-radius:10px;color:#423a52;}
.ps-nav button.on{background:#efe9fd;color:var(--brand-deep);font-weight:700;}
.ps-panel-actions{display:flex;flex-direction:column;gap:9px;margin-top:auto;}
.ps-act-book{display:flex;align-items:center;justify-content:center;gap:8px;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:13px;border-radius:12px;box-shadow:0 6px 16px rgba(109,40,217,.3);}
...
.ps-fab svg{position:relative;z-index:1;width:54px;height:54px;background:var(--brand);color:#fff;padding:16px;border-radius:50%;box-shadow:0 8px 20px rgba(109,40,217,.4);margin-top:-26px;}
...
.ps-field-lbl{font-size:12.5px;font-weight:700;color:#524a62;margin:14px 0 8px;}
.ps-pick{display:flex;gap:9px;flex-wrap:wrap;}
.ps-pick button{font-size:13px;font-weight:600;padding:9px 15px;border-radius:11px;border:1px solid var(--line);background:var(--card);color:#423a52;}
...
.bk-thumb.c2{background:linear-gradient(150deg,#d24aa0,var(--brand));}
.bk-thumb.c3{background:linear-gradient(150deg,var(--brand),#b06fe6);}
...
.bk-svcbar{display:flex;align-items:center;justify-content:space-between;gap:10px;background:#efe9fd;border:1px solid #ddd0f7;border-radius:12px;padding:10px 13px;margin-bottom:16px;}
...
.bk-deposit-row{display:flex;align-items:center;justify-content:space-between;margin-top:16px;padding:12px 14px;background:#efe9fd;border:1px solid #ddd0f7;border-radius:12px;}
...
.ps-search-lbl{font-size:12px;font-weight:700;color:#524a62;margin-bottom:9px;}
...
.ps-toast{position:fixed;left:50%;bottom:96px;transform:translateX(-50%);background:#1c1726;color:#fff;font-size:13px;font-weight:600;padding:11px 18px;border-radius:11px;box-shadow:0 8px 24px rgba(0,0,0,.25);z-index:90;animation:pstoast .2s;}
...
.pd-head-book{display:flex;align-items:center;gap:7px;background:var(--brand);color:#fff;font-weight:700;font-size:14px;padding:11px 18px;border-radius:12px;box-shadow:0 5px 14px rgba(109,40,217,.3);}
...
.pd-tabs button{font-size:14px;font-weight:600;padding:10px 16px;border-radius:10px;color:#524a62;white-space:nowrap;}
.pd-tabs button:hover{background:#f3e7e1;}
.pd-tabs button.on{background:#efe9fd;color:var(--brand-deep);font-weight:700;}
...
.pd-railcard>p{font-size:13px;line-height:1.55;color:#423a52;}
...
.pd-railbtns button:hover{background:#efe9fd;border-color:var(--brand);}
.pd-railcard.trust{background:#efe9fd;border-color:#ddd0f7;}
.pd-trust-h{display:flex;align-items:center;gap:7px;font-size:13.5px;font-weight:700;color:var(--brand-deep);}
.pd-railcard.trust p{font-size:12px;line-height:1.55;color:#5f5670;margin-top:8px;}
...
.pd-filter button{font-size:13px;font-weight:600;padding:8px 16px;border-radius:20px;background:var(--card);border:1px solid var(--line);color:#423a52;}
...
.feat-tag{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--brand-deep);background:#efe9fd;padding:5px 11px;border-radius:8px;}
...
.feat-card{flex:0 0 76%;max-width:280px;background:var(--card);border:1px solid var(--line);border-radius:16px;overflow:hidden;box-shadow:0 8px 22px rgba(76,29,149,.08);display:flex;flex-direction:column;}
...
.feat-thumb.product{background:linear-gradient(150deg,#d24aa0,var(--gold));}
...
.feat-cta{font-size:13px;font-weight:700;padding:8px 18px;border-radius:10px;color:#fff;background:var(--brand);box-shadow:0 4px 10px rgba(109,40,217,.3);}
...
.blog-card:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(76,29,149,.1);}
...
.blog-img.c2{background:linear-gradient(135deg,#d24aa0,var(--brand-deep));}
...
.blog-body p{font-size:13px;color:#5f5670;line-height:1.5;}
...
.blog-topics button{font-size:13px;font-weight:600;padding:8px 16px;border-radius:20px;background:var(--card);border:1px solid var(--line);color:#423a52;}
...
.blog-hero:hover{transform:translateY(-2px);box-shadow:0 14px 32px rgba(76,29,149,.1);}
...
.blog-hero-img.c2{background:linear-gradient(135deg,#d24aa0,var(--brand-deep));}
...
.blog-hero-body>p{font-size:14px;color:#524a62;line-height:1.55;}
...
.author-card>p{font-size:12.5px;line-height:1.55;color:#524a62;}
...
.blog-convert{background:#efe9fd;border:1px solid #ddd0f7;border-radius:16px;padding:17px;}
.blog-convert>b{font-family:'Fraunces';font-weight:600;font-size:16px;}
.blog-convert>p{font-size:12.5px;color:#5f5670;line-height:1.5;margin:6px 0 13px;}
.blog-convert-cta{display:flex;align-items:center;justify-content:center;gap:7px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:13.5px;padding:11px;border-radius:11px;box-shadow:0 6px 14px rgba(109,40,217,.28);}
...
.ab-lede{font-size:15.5px;line-height:1.6;color:#524a62;max-width:none;}
...
.ab-bio{font-size:14px;line-height:1.6;color:#524a62;margin:13px 0;}
.ab-chips{display:flex;flex-wrap:wrap;gap:7px;margin-bottom:14px;}
.ab-chips span{font-size:11.5px;font-weight:600;color:var(--brand-deep);background:#efe9fd;border:1px solid #ecd2dd;padding:5px 11px;border-radius:16px;}
...
.ab-approach-grid p{font-size:13px;line-height:1.55;color:#524a62;}
...
.ab-addr{font-size:13px;line-height:1.5;color:#423a52;margin:12px 0 8px;}
...
.ab-follow-icons button{width:40px;height:40px;border-radius:11px;display:grid;place-items:center;background:#efe9fd;border:1px solid #ecd2dd;color:var(--brand-deep);transition:background .12s,color .12s;}
...
.ab-book-m{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;margin-top:18px;background:var(--brand);color:#fff;font-weight:700;font-size:15px;padding:14px;border-radius:13px;box-shadow:0 8px 18px rgba(109,40,217,.3);}
.ab-para{font-size:14.5px;line-height:1.65;color:#524a62;max-width:none;margin-top:13px;}
...
.ab-offer-card:hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(76,29,149,.09);border-color:#ddd0f7;}
.ab-offer-card b{display:block;font-size:15.5px;font-weight:700;margin-bottom:6px;}
.ab-offer-card p{font-size:13px;line-height:1.55;color:#524a62;margin-bottom:11px;}
...
.ab-fact-v{font-size:13.5px;color:#423a52;line-height:1.45;}
.ab-grid-m{grid-template-columns:1fr;}
.ab-facts-m{grid-template-columns:1fr;}
.ab-verified{display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:700;color:var(--brand-deep);background:#efe9fd;border:1px solid #ecd2dd;padding:4px 10px;border-radius:16px;margin-top:8px;}
...
.ab-creds li{display:flex;align-items:flex-start;gap:8px;font-size:13px;line-height:1.45;color:#423a52;}
...
.ab-shot:hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(76,29,149,.14);}
...
.ab-shot.c2{background:linear-gradient(150deg,#d24aa0,var(--brand-deep));}
...
.ab-review{position:relative;background:#efe9fd;border:1px solid #ddd0f7;border-radius:18px;padding:24px;margin:28px 0;}
...
.ab-journal-item:hover{transform:translateY(-2px);box-shadow:0 10px 22px rgba(76,29,149,.09);border-color:#ddd0f7;}
...
.faq-cats button{display:flex;align-items:center;gap:10px;width:100%;text-align:left;font-size:13px;font-weight:600;color:#423a52;padding:10px 11px;border-radius:9px;transition:background .12s,color .12s;}
.faq-cats button:hover{background:#efe9fd;color:var(--brand-deep);}
.faq-cats button svg{color:var(--brand);flex:0 0 auto;}
.faq-help{background:#efe9fd;border:1px solid #ddd0f7;border-radius:16px;padding:17px;}
.faq-help>b{font-family:'Fraunces';font-weight:600;font-size:16px;}
.faq-help>p{font-size:12.5px;color:#5f5670;line-height:1.5;margin:6px 0 13px;}
.faq-help-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:13.5px;padding:11px;border-radius:11px;box-shadow:0 6px 14px rgba(109,40,217,.28);}
...
.faq-empty{background:var(--card);border:1px solid var(--line);border-radius:13px;padding:18px;font-size:13.5px;color:#524a62;}
...
.ct-wa{display:flex;align-items:center;justify-content:center;gap:9px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:15.5px;padding:15px;border-radius:13px;box-shadow:0 8px 18px rgba(109,40,217,.3);}
...
.ct-form-sub{font-size:13px;line-height:1.5;color:#524a62;margin-bottom:15px;}
...
.ct-send{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14px;padding:13px;border-radius:11px;box-shadow:0 6px 14px rgba(109,40,217,.28);}
...
.ct-map-pin{position:absolute;left:50%;top:44%;transform:translate(-50%,-50%) rotate(-45deg);display:grid;place-items:center;width:34px;height:34px;border-radius:50% 50% 50% 0;background:var(--brand);color:#fff;box-shadow:0 6px 13px rgba(76,29,149,.4);z-index:2}
...
.ct-hours-list li{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:9px 13px;font-size:12.5px;color:#524a62;}
.ct-hours-list li+li{border-top:1px solid var(--line);}
.ct-hours-list li b{font-weight:600;color:#423a52;}
.ct-hours-list li.today{background:#efe9fd;color:var(--brand-deep);}
...
.rf-list li{display:flex;align-items:flex-start;gap:10px;font-size:14px;line-height:1.55;color:#423a52;}
...
.rf-keys li span{font-size:12.5px;color:#524a62;}
...
.rf-related button{display:flex;align-items:center;justify-content:space-between;width:100%;font-size:13.5px;font-weight:600;color:#423a52;padding:11px;border-radius:10px;transition:background .12s,color .12s;}
.rf-related button:hover{background:#efe9fd;color:var(--brand-deep);}
...
.tm-intro{font-size:15px;line-height:1.6;color:#524a62;max-width:70ch;}
...
.tm-section p{font-size:14px;line-height:1.65;color:#423a52;margin-bottom:10px;}
...
.tm-contents button{text-align:left;font-size:12.5px;font-weight:600;color:#423a52;padding:8px 10px;border-radius:8px;transition:background .12s,color .12s;}
.tm-contents button:hover{background:#efe9fd;color:var(--brand-deep);}
...
.pf-shot:hover{transform:translateY(-2px);box-shadow:0 14px 30px rgba(76,29,149,.16);}
...
.pf-shot.c2{background:linear-gradient(150deg,#d24aa0,var(--brand-deep));}
...
.art-hero.c2{background:linear-gradient(150deg,#d24aa0,var(--brand-deep));}
...
.svc-intro{font-size:14px;color:#5f5670;line-height:1.5;margin:-4px 0 24px;max-width:620px;}
...
.svc-fgroup h4{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:#7c7390;margin-bottom:8px;}
.svc-radios{display:flex;flex-direction:column;gap:1px;}
.svc-radios button{text-align:left;font-size:13.5px;font-weight:500;padding:8px 10px;border-radius:9px;color:#423a52;}
.svc-radios button:hover{background:#f3e7e1;}
.svc-radios button.on{background:#efe9fd;color:var(--brand-deep);font-weight:700;}
.svc-book-card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:16px;box-shadow:0 8px 22px rgba(76,29,149,.06);}
...
.svc-next{font-size:13px;color:#524a62;margin:10px 0 14px;}
.svc-next b{color:var(--ink);}
.svc-book-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px;border-radius:12px;box-shadow:0 6px 16px rgba(109,40,217,.3);}
...
.svc-card:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(76,29,149,.1);}
...
.svc-card-desc{font-size:12.5px;color:#5f5670;line-height:1.5;margin-top:8px;flex:1;}
...
.svc-card-book{display:inline-flex;align-items:center;gap:3px;font-size:13px;font-weight:700;color:#fff;background:var(--brand);padding:8px 14px;border-radius:10px;box-shadow:0 4px 10px rgba(109,40,217,.3);}
...
.svc-deliv li{display:flex;align-items:flex-start;gap:8px;font-size:12.5px;color:#524a62;line-height:1.4;}
...
.rev-leave{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px;border-radius:12px;box-shadow:0 6px 16px rgba(109,40,217,.3);}
...
.rev-filter-m button{display:inline-flex;align-items:center;gap:4px;font-size:12.5px;font-weight:600;padding:7px 13px;border-radius:10px;border:1px solid var(--line);background:var(--card);color:#423a52;}
...
.rev-photostrip h4{font-size:12.5px;font-weight:700;color:#524a62;margin-bottom:10px;}
...
.rev-ph.c2{background:linear-gradient(150deg,#d24aa0,var(--brand));}
...
.rev-card-svc{display:inline-block;font-size:11px;font-weight:700;color:var(--brand-deep);background:#efe9fd;padding:3px 9px;border-radius:7px;margin:11px 0 8px;}
.rev-card-text{font-size:13.5px;line-height:1.55;color:#423a52;}
...
.rev-response p{font-size:12.5px;line-height:1.5;color:#524a62;}
.rev-form-note{display:flex;align-items:flex-start;gap:7px;font-size:12.5px;color:#5f5670;line-height:1.5;background:#efe9fd;border:1px solid #ddd0f7;border-radius:11px;padding:11px 12px;margin-bottom:6px;}
...
.rev-input:focus,.rev-textarea:focus{outline:none;border-color:var(--brand);box-shadow:0 0 0 3px rgba(109,40,217,.12);}
