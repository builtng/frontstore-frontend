...
const MOCK_PRODUCT_FAQS = [
  ["When will my order be delivered?", "Same-day orders placed before 1pm are delivered by evening. Standard orders arrive the next day within your chosen window."],
  ["Do you deliver outside Surulere?", "Yes, we deliver across Lagos. The delivery fee is calculated at checkout based on your location."],
  ["Can I substitute items if something is out of stock?", "We will contact you on WhatsApp before substituting. You can approve, decline or choose an alternative before dispatch."],
  ["Can I return a product?", "Perishable items cannot be returned, but if something arrives damaged or spoilt, send us a photo within 2 hours and we will replace it."],
  ["How do I pay?", "Pay securely through Frontstore at checkout, or by bank transfer. Your receipt arrives on WhatsApp."],
];
const MOCK_SERVICE_FAQS = [
  ["Can I customise my weekly basket?", "Yes. When you subscribe to the weekly basket, you give us a list of your staples and preferences and we shop to that list each week."],
  ["How does the delivery subscription work?", "The Priority Member plan gives you free delivery and front-of-queue processing for 30 days. It renews monthly and you can cancel any time."],
  ["What if I want to skip a week?", "Message us on WhatsApp before your delivery date and we will pause your basket for that week, no charge."],
  ["Do you do same-day delivery for bulk orders?", "Bulk and catering orders need at least 24 hours notice. We confirm a delivery time when your order is received."],
  ["How do I pay?", "Pay securely through Frontstore at checkout, or by bank transfer. Your receipt always arrives on WhatsApp."],
];
...
const MOCK_CATS = ["Grains", "Oils & Seasoning", "Canned Goods", "Dry Goods", "Household", "Dairy & Drinks"];
      ...
      {search && (
        <div className="ps-overlay" onClick={() => setSearch(false)}>
          <div className="ps-search-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ps-chips">{["Weekly Basket", "Same-Day Delivery", "Ofada Rice", "Bulk Order", "Fresh Produce Box"].map((c: any) => (<button key={c} onClick={() => { setSearch(false); ping("Searching " + c); }}>{c}</button>))}</div>
          </div>
        </div>
      )}
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#dcfce7;padding:7px 12px;border-radius:9px;margin-top:11px;}
