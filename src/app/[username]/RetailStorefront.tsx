...
const MOCK_SERVICE_INFO = {
  s1: { includes: ["Order by 1pm for same-day delivery", "Fresh produce and dry goods picked same morning", "Household items sourced from store stock", "Contactless drop at your gate or building", "WhatsApp confirmation with delivery window", "Reorder from your saved list at checkout"] },
  s2: { includes: ["Order anytime before 6pm for next morning", "Choose your delivery window at checkout", "Substitution alerts sent by WhatsApp if items change", "Packed fresh and sealed before dispatch", "Full receipt sent on delivery", "No subscription needed, order as you go"] },
  s3: { includes: ["Curated weekly basket based on your preferences", "Dry goods, grains and staples included", "Fresh produce sourced on delivery day", "Household items and cleaning supplies", "Drinks and beverages as specified", "Swap or pause your basket anytime"] },
  s4: { includes: ["Full month of provisions in one delivery", "Bulk pricing applied on eligible items", "Grains, oils, canned goods and pantry staples", "Household supplies included in bundle", "Order review call to confirm your list", "Delivery scheduled to suit your date"] },
  s5: { includes: ["Tomatoes, peppers, garden eggs and onions", "Leafy greens: ugu, water leaf, bitter leaf", "Fruits in season each week", "Plantain, yam and cassava as available", "Sourced from Oyingbo and Mile 12 markets", "Packed in food-safe bags and sealed"] },
  s6: { includes: ["Custom quantity list accepted by WhatsApp or form", "Dry goods, oils, canned tomatoes and spices", "Drinks, water and beverages in bulk", "Paper goods and disposable packaging available", "Invoice issued on confirmation", "Delivery to your restaurant, hall or location"] },
  s7: { includes: ["Drinks, water, juice and tea supplies", "Coffee, milk, sugar and hot drink accessories", "Snacks: biscuits, nuts, crisps and bites", "Condiments and cooking basics restocked", "Regular schedule set to avoid running out", "WhatsApp to adjust the list before dispatch"] },
  s8: { includes: ["Free delivery on every order for 30 days", "Front-of-queue order processing", "Member-only discounts on selected items", "Early access to new products and seasonal drops", "Pause or cancel any time through WhatsApp", "Priority customer support for members"] },
};
...
const MOCK_NOTIFY_TOPICS = [["deals", "Weekly deals"], ["drops", "New stock drops"], ["tips", "Kitchen & cooking tips"], ["news", "Announcements"]];
const MOCK_HOURS = [
  ["Mon", "8:00am - 7:00pm"], ["Tue", "8:00am - 7:00pm"], ["Wed", "8:00am - 7:00pm"],
  ["Thu", "8:00am - 7:00pm"], ["Fri", "8:00am - 7:00pm"], ["Sat", "7:00am - 6:00pm"], ["Sun", "9:00am - 3:00pm"],
];
const MOCK_NAV = [
  ["home", "Home"], ["services", "Delivery"], ["products", "Shop"], ["portfolio", "Gallery"], ["reviews", "Reviews"], ["blog", "Blog"],
...
export default function RetailStorefront({
  ...
  const customBrandColor = store.primary_color || '#16a34a';
  const customBrandDeep = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 80%, black)` : '#15803d';
  const customTint = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 14%, white)` : '#f0fdf4';
  ...
  const primaryCta = ({
    ...
    book: { label: "Order now", Icon: Calendar, run: () => openBooking() },
  ...
  } as Record<string, any>)[DUMMY_STORE.primaryCta as any] || { label: "Order now", Icon: Calendar, run: () => openBooking() };
...
`;
  ...
  const serviceView = () => {
    ...
    const steps = [
      ["Add to cart and checkout", "Browse the shop or delivery services, add what you need, and pay securely at checkout."],
      ["We confirm on WhatsApp", "Once your order is placed, we send a confirmation with your delivery window and a contact number."],
      ["We pack fresh for you", "Your order is picked and packed on the morning of delivery, especially fresh produce and perishables."],
      ["Delivered to your door", "Your groceries arrive within the agreed window. We call ahead if there is any change."],
    ];
    const brings = ["Fresh produce sourced on delivery morning", "Dry goods in sealed, undamaged packaging", "Accurate order with your receipt on WhatsApp"];
    return (
        ...
        <div className="sv-grid">
          ...
          <aside className="sv-side">
            <div className="sv-card">
              ...
              <button className="sv-book" onClick={() => openBooking(s)}><Calendar size={17} /> Order now</button>
              <a className="sv-enquire" href={waHref} target="_blank" rel="noreferrer"><MessageCircle size={16} /> Ask a question</a>
              <p className="sv-note">A deposit of about {money(deposit)} secures your slot. The balance is paid after the order, once you are happy.</p>
  ...
  const aboutBody = () => (<>
    ...
    <button className="ab-book-m" onClick={() => openBooking()}><Calendar size={16} /> Order now</button></>);
  const faqSections = () => (
      ...
      <div className="rf-section">
        ...
        <ul className="rf-list">
          {store.policy_bookings ? (
            ...
            <>
              <li><Check size={15} /> Reschedule or cancel up to 24 hours before your order for a full refund of your deposit.</li>
        ...
        <div className="art-wrap">
          ...
          <aside className="art-rail">
            ...
            <div className="blog-convert">
              ...
              <p>Order now, or browse the full list of what we offer.</p>
              <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Order now</button>
              ...
              <div className="pd-listing">
                ...
                {page === "services" && (
                  <div className="svc-page">
                    <p className="svc-intro">Order now with our team. A deposit secures your slot, with the balance paid after the order.</p>
                    ...
                    <div className="blogp-body">
                      ...
                      <aside className="blogp-rail">
                        <div className="author-card">
                          ...
                          <p>{DUMMY_AUTHOR.bio}</p>
                          <div className="author-cred"><Sparkles size={13} /> Over 8 years serving Lagos shoppers</div>
                          <button className="author-link" onClick={() => go("about")}>More about {DUMMY_AUTHOR.name.split(" ")[0]} <ChevronRight size={14} /></button>
                        </div>
                        <div className="blog-convert">
                          <b>Enjoyed the read?</b>
              ...
              <div className="pd-listing">
                ...
                <div className="ab-wrap">
                  <div className="ab-main">
                    ...
                    <h2 className="ab-headline">Good products, picked with care, delivered straight to you.</h2>
                    ...
                    <div className="ab-approach">
                      ...
                      <div className="ab-approach-grid">
                        <div><span className="ab-num">01</span><b>We pick with care</b><p>Every order starts by choosing what is fresh and in stock, so it suits what you actually need.</p></div>
                        <div><span className="ab-num">02</span><b>The right products</b><p>We bring the right items and quantities for your order, not a one size fits all routine.</p></div>
                        <div><span className="ab-num">03</span><b>Made to last</b><p>From a quick top-up to a full weekly shop, the order is packed properly, not just quickly.</p></div>
                      </div>
                    </div>
                  ...
                  <aside className="ab-rail">
                    ...
                    <div className="blog-convert">
                      ...
                      <p>Order now, or browse the full list of services.</p>
                      <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Order now</button>
                      {(store.storefront_sections || []).includes("services") && (
          <button className="blog-convert-ghost" onClick={() => go("services")}>Browse services</button>
        )}
...
const css = `
.ps-root{--brand:#16a34a;--brand-deep:#15803d;--gold:#c79a4b;--bg:#f4f8f8;--card:#fff;--ink:#18272a;
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#dcfce7;padding:7px 12px;border-radius:9px;margin-top:11px;max-width:100%;min-width:0}
...
.nt-topic.on{background:#dcfce7;border-color:var(--brand);color:var(--brand-deep);}
...
.ps-sig-tag{display:inline-flex;align-items:center;gap:5px;font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:var(--brand-deep);background:#dcfce7;padding:4px 9px;border-radius:7px;}
...
.ps-seeall:hover{background:#dcfce7;border-color:var(--brand);}
...
.ps-dir:hover{background:#dcfce7;border-color:var(--brand);}
...
.ps-locked{background:#dcfce7;border:1px solid #c8e6e2;border-radius:14px;padding:14px;margin:16px 0 8px;}
...
.ps-nav button.on{background:#dcfce7;color:var(--brand-deep);font-weight:700;}
...
.bk-svcbar{display:flex;align-items:center;justify-content:space-between;gap:10px;background:#dcfce7;border:1px solid #c8e6e2;border-radius:12px;padding:10px 13px;margin-bottom:16px;}
...
.bk-deposit-row{display:flex;align-items:center;justify-content:space-between;margin-top:16px;padding:12px 14px;background:#dcfce7;border:1px solid #c8e6e2;border-radius:12px;}
...
.pd-tabs button.on{background:#dcfce7;color:var(--brand-deep);font-weight:700;}
...
.pd-railbtns button:hover{background:#dcfce7;border-color:var(--brand);}
.pd-railcard.trust{background:#dcfce7;border-color:#c8e6e2;}
...
.feat-tag{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--brand-deep);background:#dcfce7;padding:5px 11px;border-radius:8px;}
...
.blog-convert{background:#dcfce7;border:1px solid #c8e6e2;border-radius:16px;padding:17px;}
...
.ab-chips span{font-size:11.5px;font-weight:600;color:var(--brand-deep);background:#dcfce7;border:1px solid #ecd2dd;padding:5px 11px;border-radius:16px;}
...
.ab-follow-icons button{width:40px;height:40px;border-radius:11px;display:grid;place-items:center;background:#dcfce7;border:1px solid #ecd2dd;color:var(--brand-deep);transition:background .12s,color .12s;}
...
.ab-verified{display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:700;color:var(--brand-deep);background:#dcfce7;border:1px solid #ecd2dd;padding:4px 10px;border-radius:16px;margin-top:8px;}
...
.ab-review{position:relative;background:#dcfce7;border:1px solid #c8e6e2;border-radius:18px;padding:24px;margin:28px 0;}
...
.faq-cats button:hover{background:#dcfce7;color:var(--brand-deep);}
.faq-cats button svg{color:var(--brand);flex:0 0 auto;}
.faq-help{background:#dcfce7;border:1px solid #c8e6e2;border-radius:16px;padding:17px;}
...
.ct-hours-list li.today{background:#dcfce7;color:var(--brand-deep);}
...
.rf-related button:hover{background:#dcfce7;color:var(--brand-deep);}
...
.tm-contents button:hover{background:#dcfce7;color:var(--brand-deep);}
...
.svc-radios button.on{background:#dcfce7;color:var(--brand-deep);font-weight:700;}
...
.rev-card-svc{display:inline-block;font-size:11px;font-weight:700;color:var(--brand-deep);background:#dcfce7;padding:3px 9px;border-radius:7px;margin:11px 0 8px;}
...
.rev-form-note{display:flex;align-items:flex-start;gap:7px;font-size:12.5px;color:#54636a;line-height:1.5;background:#dcfce7;border:1px solid #c8e6e2;border-radius:11px;padding:11px 12px;margin-bottom:6px;}
