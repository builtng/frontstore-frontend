...
const MOCK_SERVICE_INFO = {
  s1: { includes: ["Send prescription by WhatsApp or upload at checkout", "Pharmacist reviews and prepares your medications", "Pick up ready same day, or choose delivery", "Substitution alert sent if a brand is unavailable", "Generic equivalents offered with savings noted", "All medications dispensed from NAFDAC registered stock"] },
  s2: { includes: ["30-minute private session with a registered pharmacist", "Drug interaction review for existing medications", "Advice on over-the-counter options for common conditions", "Dosage review and medication schedule planning", "Referral letter issued if a GP visit is advised", "Notes summarised and sent to you on WhatsApp"] },
  s3: { includes: ["Order online and pay at checkout", "Lagos-wide delivery within 2 to 4 hours", "Pharmacist confirms your order before dispatch", "Cold chain maintained for temperature sensitive items", "Delivery rider tracked in real time", "Receipt and medication guide sent on WhatsApp"] },
  s4: { includes: ["Automated digital blood pressure cuff", "Both readings taken and averaged", "Result explained in plain language", "Written guidance on healthy ranges provided", "Lifestyle and dietary advice included", "Referral recommended if result is outside safe range"] },
  s5: { includes: ["Capillary fingertip blood sample, no fasting required", "Result ready within 5 minutes", "Pharmacist explains the result and what it means", "Normal range guide and dietary advice provided", "Fasting test also available by appointment", "Result slip issued for your records"] },
  s6: { includes: ["Rapid antigen test, result in 15 to 20 minutes", "No prescription required", "Pharmacist explains the result and next steps", "Treatment options discussed if test is positive", "Antimalarials stocked in-pharmacy for same-visit dispensing", "Child-appropriate testing available"] },
  s7: { includes: ["Tell us your medications and refill schedule", "We prepare your refill and contact you before it is ready", "Delivery or pick-up on your preferred date each month", "Pharmacist flag if a prescription renewal is needed", "Auto-reminder sent 5 days before your refill date", "Change or pause your schedule any time on WhatsApp"] },
  s8: { includes: ["Bulk medication supply for company medical rooms and clinics", "School and institutional health kit supply", "Monthly or quarterly supply schedule as needed", "Competitive pricing with invoice and delivery note", "Account manager assigned for corporate accounts", "NAFDAC compliant supply chain throughout"] },
};
...
const MOCK_NOTIFY_TOPICS = [["refills", "Refill reminders"], ["health", "Health tips"], ["offers", "Product deals"], ["news", "Pharmacy news"]];
const MOCK_HOURS = [
  ["Mon", "8:00am - 9:00pm"], ["Tue", "8:00am - 9:00pm"], ["Wed", "8:00am - 9:00pm"],
  ["Thu", "8:00am - 9:00pm"], ["Fri", "8:00am - 9:00pm"], ["Sat", "8:00am - 8:00pm"], ["Sun", "10:00am - 6:00pm"],
];
const MOCK_NAV = [
  ["home", "Home"], ["services", "Services"], ["products", "Shop"], ["portfolio", "Gallery"], ["reviews", "Reviews"], ["blog", "Blog"],
...
export default function PharmacyStorefront({
  ...
  const customBrandColor = store.primary_color || '#0d9488';
  const customBrandDeep = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 80%, black)` : '#0f766e';
  const customTint = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 14%, white)` : '#f0fdfa';
      ...
      <div className="ct-visit">
        ...
        <div className="ct-hours">
          ...
          {hasHours ? (
            ...
            <p className="ct-hours-empty">This pharmacy hasn't added opening hours yet. Message on WhatsApp to check availability.</p>
          )}
        </div>
              ...
              <div className="pd-listing">
                ...
                <div className="ab-wrap">
                  <div className="ab-main">
                    ...
                    <h2 className="ab-headline">Trusted care and the right medication, close to home.</h2>
              ...
              <div className="pd-listing">
                ...
                <div className="ct-wrap">
                  <div className="ct-main">
                    <p className="svc-intro">Reach us on WhatsApp for a quick reply about prescriptions or orders, or send an enquiry by email. A pharmacist or team member will respond the same day.</p>
                    {contactChannels()}
                    {contactForm()}
                  </div>
...
const css = `
.ps-root{--brand:#0d9488;--brand-deep:#0f766e;--gold:#c79a4b;--bg:#f4f8f8;--card:#fff;--ink:#18272a;
...
.ps-sig-tag{display:inline-flex;align-items:center;gap:5px;font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:var(--brand-deep);background:#ccfbf1;padding:4px 9px;border-radius:7px;}
...
.ps-seeall:hover{background:#ccfbf1;border-color:var(--brand);}
...
.ps-dir:hover{background:#ccfbf1;border-color:var(--brand);}
...
.ps-locked{background:#ccfbf1;border:1px solid #c8e6e2;border-radius:14px;padding:14px;margin:16px 0 8px;}
...
.ps-nav button.on{background:#ccfbf1;color:var(--brand-deep);font-weight:700;}
...
.bk-svcbar{display:flex;align-items:center;justify-content:space-between;gap:10px;background:#ccfbf1;border:1px solid #c8e6e2;border-radius:12px;padding:10px 13px;margin-bottom:16px;}
...
.bk-deposit-row{display:flex;align-items:center;justify-content:space-between;margin-top:16px;padding:12px 14px;background:#ccfbf1;border:1px solid #c8e6e2;border-radius:12px;}
...
.pd-tabs button.on{background:#ccfbf1;color:var(--brand-deep);font-weight:700;}
...
.pd-railbtns button:hover{background:#ccfbf1;border-color:var(--brand);}
.pd-railcard.trust{background:#ccfbf1;border-color:#c8e6e2;}
...
.feat-tag{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--brand-deep);background:#ccfbf1;padding:5px 11px;border-radius:8px;}
...
.blog-convert{background:#ccfbf1;border:1px solid #c8e6e2;border-radius:16px;padding:17px;}
...
.ab-chips span{font-size:11.5px;font-weight:600;color:var(--brand-deep);background:#ccfbf1;border:1px solid #ecd2dd;padding:5px 11px;border-radius:16px;}
...
.ab-follow-icons button{width:40px;height:40px;border-radius:11px;display:grid;place-items:center;background:#ccfbf1;border:1px solid #ecd2dd;color:var(--brand-deep);transition:background .12s,color .12s;}
...
.ab-verified{display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:700;color:var(--brand-deep);background:#ccfbf1;border:1px solid #ecd2dd;padding:4px 10px;border-radius:16px;margin-top:8px;}
...
.ab-review{position:relative;background:#ccfbf1;border:1px solid #c8e6e2;border-radius:18px;padding:24px;margin:28px 0;}
...
.faq-cats button:hover{background:#ccfbf1;color:var(--brand-deep);}
.faq-cats button svg{color:var(--brand);flex:0 0 auto;}
.faq-help{background:#ccfbf1;border:1px solid #c8e6e2;border-radius:16px;padding:17px;}
...
.ct-hours-list li.today{background:#ccfbf1;color:var(--brand-deep);}
...
.rf-related button:hover{background:#ccfbf1;color:var(--brand-deep);}
...
.tm-contents button:hover{background:#ccfbf1;color:var(--brand-deep);}
...
.rev-card-svc{display:inline-block;font-size:11px;font-weight:700;color:var(--brand-deep);background:#ccfbf1;padding:3px 9px;border-radius:7px;margin:11px 0 8px;}
...
.rev-form-note{display:flex;align-items:flex-start;gap:7px;font-size:12.5px;color:#54636a;line-height:1.5;background:#ccfbf1;border:1px solid #c8e6e2;border-radius:11px;padding:11px 12px;margin-bottom:6px;}
