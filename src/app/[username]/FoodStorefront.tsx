...
const MOCK_HOURS = [
  ...
  ["Thu", "10:00am - 8:00pm"], ["Fri", "10:00am - 9:00pm"], ["Sat", "10:00am - 9:00pm"], ["Sun", "12:00pm - 8:00pm"],
];
const MOCK_NAV = [
...
export default function FoodStorefront({
  ...
  const customBrandColor = store.primary_color || '#c4452f';
  const customBrandDeep = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 80%, black)` : '#8a2c1d';
  ...
  const openProduct = (p?: any) => { setSelProduct(p); setSelOpts(Object.fromEntries((p.options || []).map((o: any) => [o.label, o.choices[0]]))); setPage("product"); setDrawer(false); setSearch(false); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  const bookingFlow = () => {
    ...
    return (
      <Sheet onClose={() => setBookOpen(false)} onBack={onBack} title={(titles as any)[bookStep]}>
        {bookStep === "service" && (<>
          <p className="ps-sheet-sub">Pick the package you would like to book.</p>
  ...
  const productsGrid = (g: any, list: any) => (
    <div className={g}>{(list || PRODUCTS).map((p: any) => <ProductCard key={p.id} p={p} onOpen={() => openProduct(p)} />)}</div>
  );
  const productView = () => {
    const p = selProduct;
    if (!p) return null;
    ...
    return (
      <div className="pv">
          ...
          <div className="pv-info">
            ...
            {(p.options || []).map((o: any) => (
              <div key={o.label}>
                ...
                <div className="pv-sizes">{o.choices.map((ch: any) => <button key={ch} className={`pv-size ${selOpts[o.label] === ch ? "on" : ""}`} onClick={() => setSelOpts((sx: any) => ({ ...sx, [o.label]: ch }))}>{ch}</button>)}</div>
              </div>
            ))}

            <button className="ps-sheet-cta pv-add" onClick={() => addToBag(p)}><ShoppingBag size={17} /> Add to order</button>
        ...
        <div className="pv-look">
          ...
          <div className="ps-grid">{more.map((x: any) => <ProductCard key={x.id} p={x} onOpen={() => openProduct(x)} />)}</div>
        </div>
  ...
  const reviewsBody = () => (<>
    <p className="svc-intro">Every review here comes from a verified order on Frontstore. The kitchen can respond, but cannot remove genuine reviews.</p>
    ...
    <div className="rev-trust rev-trust-m"><ShieldCheck size={14} /> Reviews are from verified orders. The kitchen typically responds in {DUMMY_STORE.reply}.</div>
  ...
  const faqSections = () => (
    <div className="faq-groups">
      {faqFiltered.length === 0 && (
        <div className="faq-empty">No questions match that search. Try another word, or message the kitchen below.</div>
      )}
  ...
  const contactVisit = () => {
    ...
    return (
      <div className="ct-visit">
        <div className="ab-rail-h"><MapPin size={15} /> Visit the kitchen</div>
        ...
        <div className="ct-hours">
          ...
          {hasHours ? (
            ...
            <p className="ct-hours-empty">This kitchen hasn't added opening hours yet. Message on WhatsApp to check availability.</p>
          )}
        </div>
  ...
  const refundsKeyPoints = () => (
    <div className="rf-keys">
      ...
      <ul>
        ...
        <li><ShoppingBag size={16} /><div><b>Issues</b><span>Tell us within 1 hour</span></div></li>
  ...
  const refundsAction = () => (
    <div className="blog-convert">
      ...
      <p>Message the kitchen and we will sort your cancellation, return or refund.</p>
    ...
    <section className="feat">
      ...
      <div className="feat-grid">
        {MOCK_FEATURED.map((f: any) => (
          <FeaturedCard key={f.id} f={f} onAction={() => (f.type === "service" ? openBooking(f) : openProduct(PRODUCTS.find((x) => x.id === f.id) || PRODUCTS[0]))} />
        ))}
      </div>
    </section>
  ) : null;
      ...
      {!isDesktop && (
        <div className="ps-col">
          ...
          <button className="ps-seeall" onClick={() => go("reviews")}>See all reviews <ChevronRight size={16} /></button>
            ...
            {page === "products" && <Sub title="Menu" slug={DUMMY_STORE.slug}>{productsGrid("ps-grid", PRODUCTS)}<StoreFoot onNav={go}  slug={DUMMY_STORE.slug} /></Sub>}
            {page === "portfolio" && <Sub title="Gallery" slug={DUMMY_STORE.slug}>{portfolioBody()}<StoreFoot onNav={go}  slug={DUMMY_STORE.slug} /></Sub>}
        ...
        <div className="pd-wrap">
              ...
              <div className="pd-listing">
                ...
                {page === "services" && (
                  <div className="svc-page">
                    <p className="svc-intro">Book catering or meal prep with our team. A deposit secures your date, with the balance settled before the event.</p>
                ...
                {page === "products" && (
                  <div className="svc-page">
                    <p className="svc-intro">Order our most loved dishes. Delivery across Lagos in 1 to 2 hours, with nationwide shipping{DUMMY_STORE.location ? ` and ${DUMMY_STORE.location} pickup at checkout` : ""}.</p>
                    ...
                    <div className="svc-body">
                      <aside className="svc-rail">
                        ...
                        <div className="svc-book-card">
                          ...
                          <ul className="svc-deliv">
                            <li><Truck size={14} /> Delivery across Lagos in 1 to 2 hours</li>
                      ...
                      <div className="svc-main">
                        ...
                        {prodFiltered.length > 0 ? (
                          <div className="svc-grid">
                            {prodFiltered.map((p: any) => <ProductCardRich key={p.id} p={p} colour={prodColor(p.cat)} onView={() => openProduct(p)} />)}
                          </div>
                        ) : (
                  ...
                  <div className="svc-page">
                    ...
                    <div className="svc-body">
                      <aside className="svc-rail">
                        ...
                        <div className="rev-trust"><ShieldCheck size={14} /> Reviews are from verified orders. The kitchen typically responds in {DUMMY_STORE.reply}.</div>
                      </aside>
                ...
                {page === "blog" && (
                  <div className="blogp">
                    <p className="svc-intro">Recipes, tips and kitchen notes from {DUMMY_AUTHOR.name} and the team at {DUMMY_STORE.name}.</p>
                    <div className="blog-topics">
          ...
          <button className="blog-convert-cta" onClick={() => go("products")}><ShoppingBag size={15} /> See the menu</button>
            ...
            {page === "portfolio" && (
              <div className="pd-listing">
                ...
                <p className="svc-intro">A look at recent work from the kitchen, from steaming jollof to fresh small chops. Tap any image to see it larger.</p>
                {portfolioChips()}
          ...
          <button className="blog-convert-cta" onClick={() => go("products")}><ShoppingBag size={15} /> See the menu</button>
                    ...
                    <div className="pd-railcard trust">
                      ...
                      <p>Bookings and orders placed through Frontstore are covered by buyer protection the kitchen cannot remove.</p>
                    </div>
                  </aside>
                </div>
                <StoreFoot onNav={go}  slug={DUMMY_STORE.slug} />
                  ...
                  <div className="ab-main">
                    ...
                    <div className="ab-approach">
                      ...
                      <div className="ab-approach-grid">
                        <div><span className="ab-num">01</span><b>Cooked to order</b><p>Nothing sits under a warmer. We start cooking when you order, so your food is fresh from the pot.</p></div>
                        <div><span className="ab-num">02</span><b>Real ingredients</b><p>Fresh produce, proper stock and real spice. We shop daily and cut no corners on flavour.</p></div>
                        <div><span className="ab-num">03</span><b>Delivered hot</b><p>Sealed, insulated and sent straight to you, so it reaches your door tasting the way it left the kitchen.</p></div>
                      </div>
                    </div>
          ...
          <button className="blog-convert-cta" onClick={() => go("products")}><ShoppingBag size={15} /> See the menu</button>
                    ...
                    <div className="pd-railcard trust">
                      <div className="pd-trust-h"><ShieldCheck size={15} /> Secured by Frontstore</div>
                      <p>Bookings and orders placed through Frontstore are covered by buyer protection that the kitchen cannot remove.</p>
                    </div>
                  </aside>
                </div>
                <StoreFoot onNav={go}  slug={DUMMY_STORE.slug} />
            ...
            {page === "faq" && (
              <div className="pd-listing">
                ...
                <p className="svc-intro">Answers to the questions we are asked most. If you cannot find yours, the kitchen is a message away.</p>
                <div className="faq-wrap">
                  <aside className="faq-rail">
                    ...
                    <div className="pd-railcard trust">
                      <div className="pd-trust-h"><ShieldCheck size={15} /> Secured by Frontstore</div>
                      <p>Payments and orders through Frontstore are covered by buyer protection the kitchen cannot remove.</p>
                    </div>
                  </aside>
...
function ServiceCard({ s, onBook }: { s: any, onBook: () => void }) {
  return (<div className="ps-card"><div className="ps-card-thumb svc">
      {s.image_url ? <img src={s.image_url} alt={s.name} style={{ width: '100%', height: '100%', objectFit: 'contain' }} /> : <Sparkles size={22} />}
    </div>
...
function ProductCard({ p, onOpen }: { p: any, onOpen: () => void }) {
  ...
  return (<div className="ps-card prod-card" onClick={onOpen}>
    <div className="relative w-full overflow-hidden">
      <ProductImage src={img} alt={p.name} aspectRatio="4/5" fallbackIcon={<UtensilsCrossed size={22} />} />
    </div>
  ...
  return (
    <div className="svc-card" onClick={onView}>
      <div className="relative w-full overflow-hidden rounded-t-xl">
        <ProductImage src={img} alt={p.name} aspectRatio="4/5" fallbackIcon={<UtensilsCrossed size={24} />} />
      ...
      <div className="svc-card-body">
        ...
        <div className="svc-card-foot"><em>{money(p.price)}</em>
          <button className="svc-card-book" onClick={(e) => { e.stopPropagation(); onView(); }}>View</button></div>
      </div>
    </div>
  );
}
    ...
    <div className="feat-card">
      ...
      <div className="feat-body">
        ...
        <div className="feat-foot"><em>{money(f.price)}</em>
          <button className="feat-cta" onClick={onAction}>{f.type === "service" ? "Book" : "View"}</button></div>
      </div>
    </div>
  );
}
    ...
    <div className="rev-card">
      ...
      {rv.response && (
        <div className="rev-response">
          <div className="rev-response-head"><span className="rev-resp-av">{storeInitial || MOCK_STORE.initial}</span><b>Response from the kitchen</b><span>{rv.response.when}</span></div>
...
const css = `
.ps-root{--brand:#c4452f;--brand-deep:#8a2c1d;--gold:#c79a4b;--bg:#faf4ef;--card:#fff;--ink:#2a1a16;
  --muted:#8a7680;--line:#efe1da;--ok:#1f9d57;--wa:#25d366;
  --shadow-sm:0 1px 2px rgba(42,26,22,.04),0 2px 8px rgba(42,26,22,.05);
  --shadow-md:0 4px 12px rgba(42,26,22,.06),0 10px 28px rgba(42,26,22,.08);
  --shadow-lg:0 8px 24px rgba(42,26,22,.09),0 20px 48px rgba(42,26,22,.11);
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#fbe9e4;padding:7px 12px;border-radius:9px;margin-top:11px;max-width:100%;min-width:0}
...
.ps-notify{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:700;color:#fff;background:var(--brand);padding:8px 13px;border-radius:9px;box-shadow:0 4px 12px rgba(196,69,47,.28);cursor:pointer;flex-shrink:0}
...
.nt-topic.on{background:#fbe9e4;border-color:var(--brand);color:var(--brand-deep);}
...
.ps-signature{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:14px;margin-bottom:18px;box-shadow:0 8px 22px rgba(138,44,29,.07);}
.ps-sig-tag{display:inline-flex;align-items:center;gap:5px;font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:var(--brand-deep);background:#fbe9e4;padding:4px 9px;border-radius:7px;}
...
.ps-book-btn,.ps-sheet-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:15px;padding:13px;border-radius:12px;box-shadow:0 6px 16px rgba(196,69,47,.3);}
.ps-book-btn:hover,.ps-sheet-cta:hover{box-shadow:0 8px 22px rgba(196,69,47,.4);transform:translateY(-1px);}
.ps-book-btn:active,.ps-sheet-cta:active{transform:translateY(2px);box-shadow:0 3px 10px rgba(196,69,47,.26);}
...
.ps-mini{font-size:13px;font-weight:700;padding:7px 15px;border-radius:9px;color:#fff;background:var(--brand);box-shadow:0 3px 8px rgba(196,69,47,.28);}
.ps-mini:hover{box-shadow:0 5px 14px rgba(196,69,47,.36);transform:translateY(-1px);}
.ps-mini:active{transform:translateY(1px);box-shadow:0 2px 6px rgba(196,69,47,.24);}
...
.ps-seeall:hover{background:#fbe9e4;border-color:var(--brand);}
...
.ps-dir:hover{background:#fbe9e4;border-color:var(--brand);}
...
.ps-locked{background:#fbe9e4;border:1px solid #f2cdc3;border-radius:14px;padding:14px;margin:16px 0 8px;}
...
.ps-nav button.on{background:#fbe9e4;color:var(--brand-deep);font-weight:700;}
.ps-panel-actions{display:flex;flex-direction:column;gap:9px;margin-top:auto;}
.ps-act-book{display:flex;align-items:center;justify-content:center;gap:8px;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:13px;border-radius:12px;box-shadow:0 6px 16px rgba(196,69,47,.3);}
...
.ps-fab svg{position:relative;z-index:1;width:54px;height:54px;background:var(--brand);color:#fff;padding:16px;border-radius:50%;box-shadow:0 8px 20px rgba(196,69,47,.4);margin-top:-26px;}
...
.bk-svcbar{display:flex;align-items:center;justify-content:space-between;gap:10px;background:#fbe9e4;border:1px solid #f2cdc3;border-radius:12px;padding:10px 13px;margin-bottom:16px;}
...
.bk-deposit-row{display:flex;align-items:center;justify-content:space-between;margin-top:16px;padding:12px 14px;background:#fbe9e4;border:1px solid #f2cdc3;border-radius:12px;}
...
.pd-head-book{display:flex;align-items:center;gap:7px;background:var(--brand);color:#fff;font-weight:700;font-size:14px;padding:11px 18px;border-radius:12px;box-shadow:0 5px 14px rgba(196,69,47,.3);}
...
.pd-tabs button.on{background:#fbe9e4;color:var(--brand-deep);font-weight:700;}
...
.pd-railbtns button:hover{background:#fbe9e4;border-color:var(--brand);}
.pd-railcard.trust{background:#fbe9e4;border-color:#f2cdc3;}
...
.feat-tag{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--brand-deep);background:#fbe9e4;padding:5px 11px;border-radius:8px;}
...
.feat-card{flex:0 0 76%;max-width:280px;background:var(--card);border:1px solid var(--line);border-radius:16px;overflow:hidden;box-shadow:0 8px 22px rgba(138,44,29,.08);display:flex;flex-direction:column;}
...
.feat-cta{font-size:13px;font-weight:700;padding:8px 18px;border-radius:10px;color:#fff;background:var(--brand);box-shadow:0 4px 10px rgba(196,69,47,.3);}
...
.blog-card:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(138,44,29,.1);}
...
.blog-hero:hover{transform:translateY(-2px);box-shadow:0 14px 32px rgba(138,44,29,.1);}
...
.blog-convert{background:#fbe9e4;border:1px solid #f2cdc3;border-radius:16px;padding:17px;}
...
.blog-convert-cta{display:flex;align-items:center;justify-content:center;gap:7px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:13.5px;padding:11px;border-radius:11px;box-shadow:0 6px 14px rgba(196,69,47,.28);}
...
.ab-chips span{font-size:11.5px;font-weight:600;color:var(--brand-deep);background:#fbe9e4;border:1px solid #ecd2dd;padding:5px 11px;border-radius:16px;}
...
.ab-follow-icons button{width:40px;height:40px;border-radius:11px;display:grid;place-items:center;background:#fbe9e4;border:1px solid #ecd2dd;color:var(--brand-deep);transition:background .12s,color .12s;}
...
.ab-book-m{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;margin-top:18px;background:var(--brand);color:#fff;font-weight:700;font-size:15px;padding:14px;border-radius:13px;box-shadow:0 8px 18px rgba(196,69,47,.3);}
...
.ab-offer-card:hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(138,44,29,.09);border-color:#f2cdc3;}
...
.ab-verified{display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:700;color:var(--brand-deep);background:#fbe9e4;border:1px solid #ecd2dd;padding:4px 10px;border-radius:16px;margin-top:8px;}
...
.ab-shot:hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(138,44,29,.14);}
...
.ab-review{position:relative;background:#fbe9e4;border:1px solid #f2cdc3;border-radius:18px;padding:24px;margin:28px 0;}
...
.ab-journal-item:hover{transform:translateY(-2px);box-shadow:0 10px 22px rgba(138,44,29,.09);border-color:#f2cdc3;}
...
.faq-cats button:hover{background:#fbe9e4;color:var(--brand-deep);}
.faq-cats button svg{color:var(--brand);flex:0 0 auto;}
.faq-help{background:#fbe9e4;border:1px solid #f2cdc3;border-radius:16px;padding:17px;}
...
.faq-help-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:13.5px;padding:11px;border-radius:11px;box-shadow:0 6px 14px rgba(196,69,47,.28);}
...
.ct-wa{display:flex;align-items:center;justify-content:center;gap:9px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:15.5px;padding:15px;border-radius:13px;box-shadow:0 8px 18px rgba(196,69,47,.3);}
...
.ct-send{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14px;padding:13px;border-radius:11px;box-shadow:0 6px 14px rgba(196,69,47,.28);}
...
.ct-map-pin{position:absolute;left:50%;top:44%;transform:translate(-50%,-50%) rotate(-45deg);display:grid;place-items:center;width:34px;height:34px;border-radius:50% 50% 50% 0;background:var(--brand);color:#fff;box-shadow:0 6px 13px rgba(138,44,29,.4);z-index:2}
...
.ct-hours-list li.today{background:#fbe9e4;color:var(--brand-deep);}
...
.rf-related button:hover{background:#fbe9e4;color:var(--brand-deep);}
...
.tm-contents button:hover{background:#fbe9e4;color:var(--brand-deep);}
...
.pf-shot:hover{transform:translateY(-2px);box-shadow:0 14px 30px rgba(138,44,29,.16);}
...
.svc-radios button.on{background:#fbe9e4;color:var(--brand-deep);font-weight:700;}
.svc-book-card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:16px;box-shadow:0 8px 22px rgba(138,44,29,.06);}
...
.svc-book-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px;border-radius:12px;box-shadow:0 6px 16px rgba(196,69,47,.3);}
...
.svc-card:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(138,44,29,.1);}
.svc-card-thumb{height:140px;position:relative;overflow:hidden;display:grid;place-items:center;color:#fff;}
.svc-card-thumb img{width:100%;height:100%;object-fit:contain;display:block;}
...
.svc-card-cat{position:absolute;bottom:10px;left:10px;width:max-content;font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.04em;color:var(--brand-deep);background:rgba(255,255,255,.94);box-shadow:0 2px 6px rgba(0,0,0,.15);padding:4px 9px;border-radius:7px;z-index:2;}
.svc-badge{position:absolute;top:10px;left:10px;display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.04em;color:#fff;background:rgba(0,0,0,.65);backdrop-filter:blur(4px);padding:4px 8px;border-radius:7px;z-index:2;}
.svc-badge svg{color:var(--gold);fill:var(--gold);}
.svc-card-body{padding:13px 14px 14px;display:flex;flex-direction:column;flex:1;background:var(--card);position:relative;z-index:2;}
.svc-card-body>b{font-size:15px;font-weight:700;line-height:1.3;color:var(--ink);display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;}
.svc-card-dur{display:flex;align-items:center;gap:5px;font-size:12px;font-weight:600;color:var(--ink);opacity:0.82;margin-top:4px;}
.svc-card-desc{font-size:12.5px;color:var(--ink);opacity:0.75;line-height:1.5;margin-top:8px;flex:1;}
...
.svc-card-book{display:inline-flex;align-items:center;gap:3px;font-size:13px;font-weight:700;color:#fff;background:var(--brand);padding:8px 14px;border-radius:10px;box-shadow:0 4px 10px rgba(196,69,47,.3);}
...
.rev-leave{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px;border-radius:12px;box-shadow:0 6px 16px rgba(196,69,47,.3);}
...
.rev-card-svc{display:inline-block;font-size:11px;font-weight:700;color:var(--brand-deep);background:#fbe9e4;padding:3px 9px;border-radius:7px;margin:11px 0 8px;}
...
.rev-form-note{display:flex;align-items:flex-start;gap:7px;font-size:12.5px;color:#6e545d;line-height:1.5;background:#fbe9e4;border:1px solid #f2cdc3;border-radius:11px;padding:11px 12px;margin-bottom:6px;}
...
.rev-input:focus,.rev-textarea:focus{outline:none;border-color:var(--brand);box-shadow:0 0 0 3px rgba(196,69,47,.12);}
...
/* product view */
.ps-card.prod-card{cursor:pointer;}
.pv{padding-bottom:8px;}
.pv-back{display:inline-flex;align-items:center;gap:5px;font-size:13.5px;font-weight:700;color:var(--brand-deep);margin-bottom:14px;}
.pv-grid{display:grid;grid-template-columns:1fr;gap:22px;}
.pv-main{position:relative;aspect-ratio:4/5;border-radius:18px;display:grid;place-items:center;overflow:hidden;}
.pv-main-icn{color:rgba(255,255,255,.55);}
.pv-main.c0,.pv-thumb.c0{background:linear-gradient(150deg,var(--brand),var(--brand-deep));}
.pv-main.c1,.pv-thumb.c1{background:linear-gradient(150deg,var(--brand-deep),var(--gold));}
.pv-main.c2,.pv-thumb.c2{background:linear-gradient(150deg,#d98a4a,var(--brand));}
.pv-main.c3,.pv-thumb.c3{background:linear-gradient(150deg,var(--gold),var(--brand-deep));}
...
.pv-tag svg{fill:var(--gold);color:var(--gold);}
...
.pv-thumb{width:64px;height:80px;border-radius:11px;}
.pv-infocat{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:var(--muted);}
.pv-name{font-family:'Fraunces';font-weight:600;font-size:27px;line-height:1.12;margin:5px 0 8px;}
.pv-price{font-size:19px;font-weight:800;display:flex;align-items:baseline;gap:9px;}
.pv-approx{font-size:12px;font-weight:600;color:var(--muted);}
.pv-desc{font-size:14.5px;line-height:1.6;color:#4c403a;margin:13px 0 4px;}
.pv-row{display:flex;align-items:center;justify-content:space-between;margin:18px 0 9px;}
.pv-label{font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.04em;}
.pv-sizes{display:flex;flex-wrap:wrap;gap:8px;}
.pv-size{min-width:46px;padding:11px 15px;border-radius:11px;border:1px solid var(--line);background:var(--card);font-size:13.5px;font-weight:700;color:var(--ink);cursor:pointer;}
.pv-size.on{background:var(--ink);color:#fff;border-color:var(--ink);}
.pv-add{margin-top:22px;}
.pv-meta{margin-top:20px;display:flex;flex-direction:column;gap:11px;border-top:1px solid var(--line);padding-top:16px;}
.pv-meta div{display:flex;gap:10px;align-items:flex-start;font-size:12.5px;color:#564a44;line-height:1.45;}
.pv-meta svg{flex:none;color:var(--brand-deep);margin-top:1px;}
.pv-look{margin-top:30px;}
.pv-look-h{font-family:'Fraunces';font-weight:600;font-size:20px;margin-bottom:13px;}
/* functional order bag */
.ps-bag-empty{text-align:center;color:var(--muted);padding:22px;font-size:14px;}
.ps-bag-line>div:nth-child(2) span{display:block;font-size:11.5px;color:var(--muted);margin-top:1px;}
.ps-bag-rm{font-size:11px;font-weight:600;color:var(--muted);margin-top:3px;}
@media(min-width:980px){
  .pv-grid{grid-template-columns:1.05fr 1fr;gap:40px;align-items:start;}
  .pv-gallery{position:sticky;top:120px;}
  .pv-name{font-size:32px;}
}
