...
interface AutoRepairStorefrontProps {
  username: string;
  store: StoreType;
  categories: Category[];
  products: Product[];
...
const MOCK_SERVICE_INFO = {
  s1: { includes: ["Engine oil drained and replaced with correct grade", "Oil filter replaced as standard", "Coolant, brake and power steering fluid checked and topped", "Air filter inspected, replaced if due", "Brake pads and disc thickness measured", "Drive belts and hoses visually inspected"] },
  s2: { includes: ["Full OBD-II scan on all control modules", "Live data read for engine, gearbox and sensors", "Fault codes interpreted and explained in plain language", "Probable root cause identified before repairs start", "Written job card issued with findings", "No repair work started without your approval"] },
  s3: { includes: ["Old brake pads removed and caliper slides cleaned", "Disc surface inspected and measured", "Genuine or quality matched pads fitted front and rear", "Caliper pistons wound back and greased", "Brake fluid level checked and topped", "Test drive completed to confirm stopping performance"] },
  s4: { includes: ["Tyre condition and tread depth checked before fitment", "New tyre mounted and seated correctly", "Dynamic balancing on the wheel balancer", "Tyre pressure set to manufacturer specification", "Wheel nuts torqued to spec", "Old tyre disposed of responsibly"] },
  s5: { includes: ["AC system pressure tested with gauges", "Refrigerant recovered and recharged to correct weight", "Compressor operation and clutch checked", "Condenser and evaporator visually inspected", "Cabin temperature drop confirmed on test", "Re-gas warranty covers the refrigerant filled"] },
  s6: { includes: ["Battery load test to confirm failure before removal", "Correct battery specification identified for your vehicle", "Old battery removed and terminals cleaned", "New battery fitted and terminals greased", "Electrical system voltage checked after fitment", "Old battery disposed of safely"] },
  s7: { includes: ["Pre-wash, snow foam and contact wash", "Clay bar decontamination of all painted panels", "Wheel and tyre deep clean and dressing", "Interior vacuumed including carpets and seats", "Dashboard, console and plastics treated", "Glass cleaned inside and out, windows polished"] },
  s8: { includes: ["Front and rear shock absorber condition assessed", "Ball joints and tie rod ends checked for play", "Wheel alignment measured and adjusted", "Steering rack inspected for leaks and response", "Rubber bushings inspected for wear", "Test drive to confirm steering and handling improvement"] },
};
...
const MOCK_HOURS = [
  ...
  ["Thu", "7:00am - 6:00pm"], ["Fri", "7:00am - 6:00pm"], ["Sat", "8:00am - 4:00pm"], ["Sun", "Closed"],
];
const MOCK_NAV = [
  ["home", "Home"], ["services", "Services"], ["products", "Shop"], ["portfolio", "Before & After"], ["reviews", "Reviews"], ["blog", "Blog"],
...
export default function AutoRepairStorefront({
  username,
  store,
  categories,
  products,
  ...
  const customBrandColor = store.primary_color || '#1d4ed8';
  const customBrandDeep = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 80%, black)` : '#1e3a8a';
  const customTint = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 14%, white)` : '#eff6ff';
...
`;
  ...
  const serviceView = () => {
    ...
    const steps = [
      ...
      ["Drop off or we collect you", "Bring the car to the workshop or, for some services, we can come to you. WhatsApp us to arrange."],
      ["We diagnose and get your approval", "We scan, test and confirm the fault before any work starts. Nothing is done without your go-ahead on the job card."],
      ["Collect when ready, pay the balance", "We call you when the car is ready. The balance is paid on collection. Receipt arrives on WhatsApp immediately."],
    ];
    const brings = ["Workshop diagnostic equipment covering all major vehicle brands", "Genuine or quality-matched parts, no counterfeit components", "Trained mechanic with manufacturer experience on your vehicle type"];
    return (
        ...
        <div className="sv-grid">
          ...
          <aside className="sv-side">
            <div className="sv-card">
              ...
              <button className="sv-book" onClick={() => openBooking(s)}><Calendar size={17} /> Book a service</button>
              <a className="sv-enquire" href={waHref} target="_blank" rel="noreferrer"><MessageCircle size={16} /> Ask a question</a>
              <p className="sv-note">A deposit of about {money(deposit)} secures your slot. The balance is paid after the service, once you are happy.</p>
        ...
        <div className="art-wrap">
          ...
          <aside className="art-rail">
            ...
            <div className="blog-convert">
              ...
              <p>Book a service, or browse the parts and products we stock.</p>
                    ...
                    <div className="blogp-body">
                      ...
                      <aside className="blogp-rail">
                        <div className="author-card">
                          ...
                          <p>{DUMMY_AUTHOR.bio}</p>
                          <div className="author-cred"><Sparkles size={13} /> Over 8 years fixing cars across Lagos</div>
                          <button className="author-link" onClick={() => go("about")}>More about {DUMMY_AUTHOR.name.split(" ")[0]} <ChevronRight size={14} /></button>
                        </div>
                        <div className="blog-convert">
                          <b>Enjoyed the read?</b>
                ...
                <div className="ab-wrap">
                  <div className="ab-main">
                    ...
                    <div className="ab-approach">
                      ...
                      <div className="ab-approach-grid">
                        <div><span className="ab-num">01</span><b>We run a diagnostic</b><p>Every booking starts by understanding the issue, so the fix suits your car and how you drive it.</p></div>
                        <div><span className="ab-num">02</span><b>The right approach</b><p>We bring the right parts and process for your car, not a one size fits all routine.</p></div>
                        <div><span className="ab-num">03</span><b>Made to last</b><p>From a quick fix to a full overhaul, the job is done properly, not just quickly.</p></div>
                      </div>
                    </div>
                  ...
                  <aside className="ab-rail">
                    ...
                    <div className="blog-convert">
                      ...
                      <p>Book a service, or browse the full list of services.</p>
                      <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book a service</button>
                      {(store.storefront_sections || []).includes("services") && (
          <button className="blog-convert-ghost" onClick={() => go("services")}>Browse services</button>
        )}
...
const css = `
.ps-root{--brand:#1d4ed8;--brand-deep:#1e3a8a;--gold:#c79a4b;--bg:#f4f8f8;--card:#fff;--ink:#18272a;
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#dbeafe;padding:7px 12px;border-radius:9px;margin-top:11px;max-width:100%;min-width:0}
...
.nt-topic.on{background:#dbeafe;border-color:var(--brand);color:var(--brand-deep);}
...
.ps-sig-tag{display:inline-flex;align-items:center;gap:5px;font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:var(--brand-deep);background:#dbeafe;padding:4px 9px;border-radius:7px;}
...
.ps-seeall:hover{background:#dbeafe;border-color:var(--brand);}
...
.ps-dir:hover{background:#dbeafe;border-color:var(--brand);}
...
.ps-locked{background:#dbeafe;border:1px solid #c8e6e2;border-radius:14px;padding:14px;margin:16px 0 8px;}
...
.ps-nav button.on{background:#dbeafe;color:var(--brand-deep);font-weight:700;}
...
.bk-svcbar{display:flex;align-items:center;justify-content:space-between;gap:10px;background:#dbeafe;border:1px solid #c8e6e2;border-radius:12px;padding:10px 13px;margin-bottom:16px;}
...
.bk-deposit-row{display:flex;align-items:center;justify-content:space-between;margin-top:16px;padding:12px 14px;background:#dbeafe;border:1px solid #c8e6e2;border-radius:12px;}
...
.pd-tabs button.on{background:#dbeafe;color:var(--brand-deep);font-weight:700;}
...
.pd-railbtns button:hover{background:#dbeafe;border-color:var(--brand);}
.pd-railcard.trust{background:#dbeafe;border-color:#c8e6e2;}
...
.feat-tag{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--brand-deep);background:#dbeafe;padding:5px 11px;border-radius:8px;}
...
.blog-convert{background:#dbeafe;border:1px solid #c8e6e2;border-radius:16px;padding:17px;}
...
.ab-chips span{font-size:11.5px;font-weight:600;color:var(--brand-deep);background:#dbeafe;border:1px solid #ecd2dd;padding:5px 11px;border-radius:16px;}
...
.ab-follow-icons button{width:40px;height:40px;border-radius:11px;display:grid;place-items:center;background:#dbeafe;border:1px solid #ecd2dd;color:var(--brand-deep);transition:background .12s,color .12s;}
...
.ab-verified{display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:700;color:var(--brand-deep);background:#dbeafe;border:1px solid #ecd2dd;padding:4px 10px;border-radius:16px;margin-top:8px;}
...
.ab-review{position:relative;background:#dbeafe;border:1px solid #c8e6e2;border-radius:18px;padding:24px;margin:28px 0;}
...
.faq-cats button:hover{background:#dbeafe;color:var(--brand-deep);}
.faq-cats button svg{color:var(--brand);flex:0 0 auto;}
.faq-help{background:#dbeafe;border:1px solid #c8e6e2;border-radius:16px;padding:17px;}
...
.ct-hours-list li.today{background:#dbeafe;color:var(--brand-deep);}
...
.rf-related button:hover{background:#dbeafe;color:var(--brand-deep);}
...
.tm-contents button:hover{background:#dbeafe;color:var(--brand-deep);}
...
.svc-radios button.on{background:#dbeafe;color:var(--brand-deep);font-weight:700;}
...
.rev-card-svc{display:inline-block;font-size:11px;font-weight:700;color:var(--brand-deep);background:#dbeafe;padding:3px 9px;border-radius:7px;margin:11px 0 8px;}
...
.rev-form-note{display:flex;align-items:flex-start;gap:7px;font-size:12.5px;color:#54636a;line-height:1.5;background:#dbeafe;border:1px solid #c8e6e2;border-radius:11px;padding:11px 12px;margin-bottom:6px;}
