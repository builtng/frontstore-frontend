...
const MOCK_PRODUCT_FAQS = [
  ["How long does delivery take?", "Most Lagos orders arrive in 1 to 2 days. Delivery cost is calculated at checkout based on your area."],
  ["Do you ship outside Lagos?", "Yes, nationwide delivery is available. Timing and cost are confirmed at checkout when you enter your address."],
  ["Are the oils and fluids right for my car?", "We stock common grades suited to most Nigerian market vehicles. If you are unsure, send us your make, model and year on WhatsApp and we will confirm."],
  ["Can I return a product?", "Unopened products can be returned within 7 days in their original packaging. Opened or used products cannot be returned unless there is a fault."],
  ["How do I pay?", "Pay securely through Frontstore, or by bank transfer where offered. Your receipt arrives on WhatsApp immediately after."],
];
const MOCK_SERVICE_FAQS = [
  ["Do I need to book or can I just drive in?", "Booking is always recommended, especially for diagnostics and servicing. Walk-ins are welcome but booked jobs are prioritised and get faster turnaround."],
  ["How do deposits work?", "A deposit secures your booking slot. The balance is settled when you collect the car. The exact deposit amount is shown before you confirm."],
  ["Can I reschedule or cancel?", "Yes, up to 24 hours before your appointment for a full deposit refund. Inside 24 hours the deposit is held against the booking."],
  ["Do I need to stay while the work is done?", "Not at all. Many customers drop off and we call or WhatsApp you once the car is ready. We can also update you mid-job if something changes."],
...
const MOCK_CATS = ["Servicing", "Diagnostics", "Brakes & Tyres", "Electrical", "Detailing"];
      ...
      {search && (
        <div className="ps-overlay" onClick={() => setSearch(false)}>
          <div className="ps-search-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ps-chips">{["Full Service", "Diagnostics", "Brake Pads", "AC Regas", "Car Detailing"].map((c: any) => (<button key={c} onClick={() => { setSearch(false); ping("Searching " + c); }}>{c}</button>))}</div>
          </div>
        </div>
      )}
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#dbeafe;padding:7px 12px;border-radius:9px;margin-top:11px;}
