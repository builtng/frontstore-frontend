...
interface PhotographerStorefrontProps {
  username: string;
  store: StoreType;
  categories: Category[];
  products: Product[];
...
const MOCK_SERVICE_INFO = {
  ...
  s3: { includes: ["Up to half a day of shooting", "Products, team or workspace", "A planned shot list agreed with you", "Around 50 edited images", "Web and print ready formats", "Usage rights for your own channels"] },
  s4: { includes: ["Up to four hours of coverage", "Candid and posed moments", "One photographer on the day", "Around 150 edited images", "A private gallery within days", "Print ready high resolution files"] },
  s5: { includes: ["Full day coverage, prep to party", "Two photographers for key moments", "An agreed timeline and shot list", "300 plus hand-edited images", "A private online gallery to share", "Sneak peeks within 48 hours"] },
  s6: { includes: ["Up to two hours of shooting", "One location you love", "Outfit and concept guidance", "Around 40 edited images", "Perfect for save the dates", "Print ready high resolution files"] },
...
const MOCK_HOURS = [
  ...
  ["Thu", "9:00am - 7:00pm"], ["Fri", "9:00am - 7:00pm"], ["Sat", "By appointment"], ["Sun", "By appointment"],
];
const MOCK_NAV = [
  ["home", "Home"], ["services", "Sessions"], ["products", "Prints"], ["portfolio", "Portfolio"], ["reviews", "Reviews"], ["blog", "Journal"],
...
export default function PhotographerStorefront({
  username,
  store,
  categories,
  products,
  ...
  const customBrandColor = store.primary_color || '#2a2724';
  const customBrandDeep = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 80%, black)` : '#161310';
  ...
  const primaryCta = ({
    ...
    book: { label: "Book a session", Icon: Calendar, run: () => openBooking() },
  ...
  } as Record<string, any>)[DUMMY_STORE.primaryCta as any] || { label: "Book a session", Icon: Calendar, run: () => openBooking() };
...
.sv-cat.c0{background:var(--brand);}.sv-cat.c1{background:#8a8174;}.sv-cat.c2{background:#6a6458;}.sv-cat.c3{background:var(--brand-deep);}
...
.sv-card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:18px;box-shadow:0 8px 30px rgba(42,39,36,.08);}
...
`;
  ...
  const serviceView = () => {
    ...
    const steps = [
      ["Pick your date and pay", "Choose a date for your shoot. A small deposit holds it, and you see the amount before you confirm."],
      ["We plan the shoot", "We talk through your vision, location, outfits and the shots that matter most before the day."],
      ["Shoot day", "We shoot in a relaxed, directed way so you look natural, with plenty of frames to choose from."],
      ["Your edited gallery", "You receive a private online gallery of hand-edited high resolution images, ready to download and print."],
    ];
    const brings = ["A private online gallery of edited photos", "High resolution images ready to print", "A relaxed, directed shoot experience"];
    return (
      <div className="sv-wrap">
        <style>{svCss}</style>
        {(store.storefront_sections || []).includes("services") && (
          <button className="sv-back" onClick={() => go("services")}><ChevronLeft size={16} /> All sessions</button>
        )}
        ...
        <div className="sv-grid">
          ...
          <aside className="sv-side">
            <div className="sv-card">
              ...
              <button className="sv-book" onClick={() => openBooking(s)}><Calendar size={17} /> Book a session</button>
              <a className="sv-enquire" href={waHref} target="_blank" rel="noreferrer"><MessageCircle size={16} /> Ask a question</a>
              <p className="sv-note">A deposit of about {money(deposit)} secures your date. The balance is due on the shoot day.</p>
    ...
    return (
      <Sheet onClose={() => setBookOpen(false)} onBack={onBack} title={(titles as any)[bookStep]}>
        ...
        {bookStep === "review" && (<>
          ...
          <textarea className="bk-input bk-textarea" value={bookNote} onChange={(e) => setBookNote(e.target.value)} placeholder="Date, location, what you have in mind" />
          <div className="bk-deposit-row"><span><Lock size={13} /> Deposit to secure</span><b>{money(deposit)}</b></div>
          <p className="ps-deposit">Frontstore holds your deposit until your shoot, then the {money(bookSvc.price - deposit)} balance is due on the day. Covered by buyer protection.</p>
  ...
  const aboutBody = () => (<>
      ...
      <div className="ab-offer-grid ab-grid-m">
        {MOCK_OFFERINGS.map(([t, d, pg]: any) => (
          <button key={t} className="ab-offer-card" onClick={() => go(pg)}>
            <b>{t}</b>
            <p>{d}</p>
            <span className="ab-offer-link">{pg === "products" ? "View prints" : "View sessions"} <ChevronRight size={14} /></span>
          </button>
        ))}
      </div>
    </div>
    ...
    <div className="ps-about-grid">
      {DUMMY_STORE.orders ? <div><b>{DUMMY_STORE.orders}</b><span>shoots done</span></div> : null}
    ...
    <button className="ab-book-m" onClick={() => openBooking()}><Calendar size={16} /> Book a session</button></>);
  const faqSections = () => (
  ...
  const contactForm = () => (
    <div className="ct-form">
      ...
      <p className="ct-form-sub">Tell us what you are after and we will reply by email. A real person, usually from our team, will answer.</p>
      ...
      <div className="rf-section">
        ...
        <ul className="rf-list">
          {store.policy_bookings ? (
            ...
            <>
              <li><Check size={15} /> Reschedule or cancel up to 7 days before your shoot for a full refund of your deposit.</li>
              <li><Check size={15} /> Within 7 days of your shoot, the deposit is held against the booking and is not refunded.</li>
      ...
      <div className="rf-section">
        ...
        <ul className="rf-list">
          {store.policy_products ? (
            ...
            <>
              <li><Check size={15} /> Prints and albums not yet sent to production can be cancelled for a full refund.</li>
              <li><Check size={15} /> Prints and albums are made to order and cannot be refunded once in production.</li>
              <li><Check size={15} /> Prints or albums that arrive damaged or faulty are replaced at no cost to you.</li>
            </>
          )}
        </ul>
      </div>
  ...
  const portfolioBody = () => (<>
    <p className="svc-intro">A selection of recent work across weddings, portraits, brands and events.</p>
    {portfolioChips()}
    {portfolioGrid()}
    <button className="ab-book-m" onClick={() => openBooking()}><Calendar size={16} /> Book a session</button>
    {portfolioFollow()}
        ...
        <div className="art-wrap">
          ...
          <aside className="art-rail">
            ...
            <div className="blog-convert">
              ...
              <p>Book a session, or browse prints from past shoots.</p>
              <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book a session</button>
              {(store.storefront_sections || []).includes("products") && (
          <button className="blog-convert-ghost" onClick={() => go("products")}>View prints</button>
        )}
            </div>
          </aside>
        </div>
    ...
    <div className="ps-root" style={storeStyleTheme}>
        ...
        <div className="ps-col">
          ...
          <main className="ps-main">
            {page === "home" && (<>
              ...
              <SectionHead title="Sessions" action={`See all ${SERVICES.length}`} onAction={() => go("services")} />
          ...
          <button className="ps-seeall" onClick={() => go("services")}>See all {SERVICES.length} services <ChevronRight size={16} /></button>
              ...
              <SectionHead title="Prints" action={`See all ${PRODUCTS.length}`} onAction={() => go("products")} />
          ...
          <button className="ps-seeall" onClick={() => go("reviews")}>See all reviews <ChevronRight size={16} /></button>
            ...
            {page === "services" && <Sub title="Sessions" slug={DUMMY_STORE.slug}>{servicesGrid("ps-grid", SERVICES)}<StoreFoot onNav={go}  slug={DUMMY_STORE.slug} /></Sub>}
            {page === "service" && serviceView()}
            {page === "products" && <Sub title="Prints" slug={DUMMY_STORE.slug}>{productsGrid("ps-grid", PRODUCTS)}<StoreFoot onNav={go}  slug={DUMMY_STORE.slug} /></Sub>}
            ...
            {page === "blog" && <Sub title="Journal" slug={DUMMY_STORE.slug}>{blogBody()}<StoreFoot onNav={go}  slug={DUMMY_STORE.slug} /></Sub>}
            ...
            {page === "home" && (<>
              ...
              <div className="pd-home">
                ...
                <div className="pd-feed">
                  {featured}
                  <div className="pd-sec-head"><h2>Sessions</h2>{(store.storefront_sections || []).includes("services") && (
          <button onClick={() => go("services")}>See all {SERVICES.length}</button>
        )}</div>
                  {servicesGrid("pd-grid", homeServices.slice(0, 6))}
                  <div className="pd-sec-head"><h2>Prints</h2>{(store.storefront_sections || []).includes("products") && (
              ...
              <div className="pd-listing">
                ...
                {page === "services" && (
                  <div className="svc-page">
                    <p className="svc-intro">Book a session with us. A deposit secures your date, with the balance due on the shoot day.</p>
                    ...
                    <div className="svc-body">
                      <aside className="svc-rail">
                        ...
                        <div className="svc-book-card">
                          ...
                          <button className="svc-book-cta" onClick={() => openBooking()}><Calendar size={16} /> Book a session</button>
                ...
                {page === "products" && (
                  <div className="svc-page">
                    <p className="svc-intro">Fine art prints, albums and digital editions of your favourite images. Delivery across Lagos in a few days, with nationwide shipping at checkout.</p>
                    ...
                    <div className="svc-body">
                      <aside className="svc-rail">
                        <div className="svc-search">
                          <Search size={16} />
                          <input value={prodQuery} onChange={(e) => setProdQuery(e.target.value)} placeholder="Search prints" />
                        ...
                        <div className="svc-book-card">
                          ...
                          <ul className="svc-deliv">
                            <li><Truck size={14} /> Delivery across Lagos in 5 to 10 days</li>
                            <li><MapPin size={14} /> Studio pickup available in Lagos</li>
                          </ul>
                          <button className="svc-book-cta" onClick={() => setBag(true)}><ShoppingBag size={16} /> View bag{bagCount > 0 ? ` (${bagCount})` : ""}</button>
                          <button className="svc-msg" onClick={() => handleWa("Hello! I'm interested in your services.")}><WhatsApp size={15} /> Ask about a print</button>
                        </div>
                      </aside>
                ...
                {page === "blog" && (
                  <div className="blogp">
                    <p className="svc-intro">Tips, ideas and photography notes from {DUMMY_AUTHOR.name} and the team at {DUMMY_STORE.name}.</p>
                    <div className="blog-topics">
                    ...
                    <div className="blogp-body">
                      ...
                      <aside className="blogp-rail">
                        <div className="author-card">
                          ...
                          <p>{DUMMY_AUTHOR.bio}</p>
                          <div className="author-cred"><Sparkles size={13} /> Over 8 years behind the lens across Lagos and beyond</div>
                          <button className="author-link" onClick={() => go("about")}>More about {DUMMY_AUTHOR.name.split(" ")[0]} <ChevronRight size={14} /></button>
                        </div>
                        <div className="blog-convert">
                          <b>Enjoyed the read?</b>
          ...
          <button className="blog-convert-ghost" onClick={() => go("products")}>View prints</button>
            ...
            {page === "portfolio" && (
              <div className="pd-listing">
                ...
                <p className="svc-intro">A selection of recent work across weddings, portraits, brands and events. Tap any image to see it larger.</p>
                {portfolioChips()}
                ...
                <div className="ab-wrap">
                  <div className="ab-main">
                    ...
                    <div className="ab-approach">
                      ...
                      <div className="ab-approach-grid">
                        <div><span className="ab-num">01</span><b>We plan it together</b><p>We start with a quick chat about your vision, location, outfits and the shots that matter most to you.</p></div>
                        <div><span className="ab-num">02</span><b>A relaxed shoot</b><p>On the day we keep it easy and directed, so you feel comfortable and simply look like yourself.</p></div>
                        <div><span className="ab-num">03</span><b>Your gallery, delivered</b><p>You receive a private online gallery of hand-edited high resolution images, ready to download, share and print.</p></div>
                      </div>
                    </div>
                  ...
                  <aside className="ab-rail">
                    ...
                    <div className="blog-convert">
                      ...
                      <p>Book a session, or see all the sessions on offer.</p>
                      <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book a session</button>
                      {(store.storefront_sections || []).includes("services") && (
          <button className="blog-convert-ghost" onClick={() => go("services")}>Browse services</button>
        )}
              ...
              <div className="pd-listing">
                ...
                <div className="ct-wrap">
                  <div className="ct-main">
                    <p className="svc-intro">Reach us on WhatsApp for a quick reply, or send an enquiry and we will get back to you by email. A real person, usually from our team, will answer.</p>
                    {contactChannels()}
                    {contactForm()}
                  </div>
                  <aside className="ct-rail">
                    ...
                    <div className="blog-convert">
                      <b>Prefer to book?</b>
                      <p>Skip the message and book a session in a couple of taps.</p>
                      <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book a session</button>
                      {(store.storefront_sections || []).includes("services") && (
          <button className="blog-convert-ghost" onClick={() => go("services")}>Browse services</button>
        )}
      ...
      {!isDesktop && (
        <nav className="ps-bottom">
          ...
          {(store.storefront_sections || []).includes("services") && (
          <button className={page === "services" ? "on" : ""} onClick={() => go("services")}><Sparkles size={21} /><span>Sessions</span></button>
        )}
          ...
          {(store.storefront_sections || []).includes("products") && (
          <button className={page === "products" ? "on" : ""} onClick={() => go("products")}><Package size={21} /><span>Prints</span></button>
        )}
          {(store.storefront_sections || []).includes("reviews") && (
...
const css = `
.ps-root{--brand:#2a2724;--brand-deep:#161310;--gold:#c79a4b;--bg:#f4f1ea;--card:#fff;--ink:#1e1b17;
  --muted:#8c8276;--line:#e8e1d5;--ok:#1f9d57;--wa:#25d366;
  --shadow-sm:0 1px 2px rgba(30,27,23,.04),0 2px 8px rgba(30,27,23,.05);
  --shadow-md:0 4px 12px rgba(30,27,23,.06),0 10px 28px rgba(30,27,23,.08);
  --shadow-lg:0 8px 24px rgba(30,27,23,.09),0 20px 48px rgba(30,27,23,.11);
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#ece6dc;padding:7px 12px;border-radius:9px;margin-top:11px;max-width:100%;min-width:0}
...
.ps-notify{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:700;color:#fff;background:var(--brand);padding:8px 13px;border-radius:9px;box-shadow:0 4px 12px rgba(42,39,36,.28);cursor:pointer;flex-shrink:0}
.nt-topics{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:4px;}
.nt-topic{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:600;padding:9px 13px;border-radius:11px;border:1px solid var(--line);background:var(--card);color:#4a463e;cursor:pointer;}
.nt-topic.on{background:#ece6dc;border-color:var(--brand);color:var(--brand-deep);}
...
.ps-bio{font-size:14px;line-height:1.55;color:#4a463e;margin-top:15px;}
...
.ps-signature{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:14px;margin-bottom:18px;box-shadow:0 8px 22px rgba(22,19,16,.07);}
.ps-sig-tag{display:inline-flex;align-items:center;gap:5px;font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:var(--brand-deep);background:#ece6dc;padding:4px 9px;border-radius:7px;}
...
.ps-book-btn,.ps-sheet-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:15px;padding:13px;border-radius:12px;box-shadow:0 6px 16px rgba(42,39,36,.3);}
.ps-book-btn:hover,.ps-sheet-cta:hover{box-shadow:0 8px 22px rgba(42,39,36,.4);transform:translateY(-1px);}
.ps-book-btn:active,.ps-sheet-cta:active{transform:translateY(2px);box-shadow:0 3px 10px rgba(42,39,36,.26);}
...
.ps-chips button{flex:0 0 auto;font-size:13px;font-weight:600;padding:8px 14px;border-radius:20px;background:var(--card);border:1px solid var(--line);color:#4a463e;}
...
.ps-card-thumb.prod{background:linear-gradient(150deg,#b08d57,var(--gold));}
...
.ps-mini{font-size:13px;font-weight:700;padding:7px 15px;border-radius:9px;color:#fff;background:var(--brand);box-shadow:0 3px 8px rgba(42,39,36,.28);}
.ps-mini:hover{box-shadow:0 5px 14px rgba(42,39,36,.36);transform:translateY(-1px);}
...
.ps-seeall:hover{background:#ece6dc;border-color:var(--brand);}
...
.ps-review p{font-size:13px;line-height:1.5;color:#4a463e;}
...
.ps-addr{display:flex;align-items:flex-start;gap:7px;font-size:13.5px;color:#4a463e;line-height:1.5;}
...
.ps-dir:hover{background:#ece6dc;border-color:var(--brand);}
.ps-hours{list-style:none;margin-top:14px;border-top:1px solid var(--line);}
.ps-hours li{display:flex;justify-content:space-between;font-size:13px;padding:7px 0;border-bottom:1px solid var(--line);color:#5a554b;}
...
.ps-acc-item p{padding:0 14px 14px;font-size:13px;line-height:1.55;color:#4a463e;}

.ps-locked{background:#ece6dc;border:1px solid #ddd4c6;border-radius:14px;padding:14px;margin:16px 0 8px;}
...
.ps-locked p{font-size:12.5px;line-height:1.55;color:#6a6458;margin-top:8px;}
...
.ps-prose{font-size:14px;line-height:1.65;color:#4a463e;margin-bottom:13px;}
...
.ps-foot-links button{font-size:12px;font-weight:600;color:#6a6458;}
...
.ps-nav button{display:flex;align-items:center;justify-content:space-between;text-align:left;font-size:14.5px;font-weight:500;padding:11px 12px;border-radius:10px;color:#4a463e;}
.ps-nav button.on{background:#ece6dc;color:var(--brand-deep);font-weight:700;}
.ps-panel-actions{display:flex;flex-direction:column;gap:9px;margin-top:auto;}
.ps-act-book{display:flex;align-items:center;justify-content:center;gap:8px;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:13px;border-radius:12px;box-shadow:0 6px 16px rgba(42,39,36,.3);}
...
.ps-fab svg{position:relative;z-index:1;width:54px;height:54px;background:var(--brand);color:#fff;padding:16px;border-radius:50%;box-shadow:0 8px 20px rgba(42,39,36,.4);margin-top:-26px;}
...
.ps-field-lbl{font-size:12.5px;font-weight:700;color:#5a554b;margin:14px 0 8px;}
.ps-pick{display:flex;gap:9px;flex-wrap:wrap;}
.ps-pick button{font-size:13px;font-weight:600;padding:9px 15px;border-radius:11px;border:1px solid var(--line);background:var(--card);color:#4a463e;}
...
.bk-thumb.c2{background:linear-gradient(150deg,#b08d57,var(--brand));}
.bk-thumb.c3{background:linear-gradient(150deg,var(--brand),#9c8f7a);}
...
.bk-svcbar{display:flex;align-items:center;justify-content:space-between;gap:10px;background:#ece6dc;border:1px solid #ddd4c6;border-radius:12px;padding:10px 13px;margin-bottom:16px;}
...
.bk-deposit-row{display:flex;align-items:center;justify-content:space-between;margin-top:16px;padding:12px 14px;background:#ece6dc;border:1px solid #ddd4c6;border-radius:12px;}
...
.ps-search-lbl{font-size:12px;font-weight:700;color:#5a554b;margin-bottom:9px;}
...
.ps-toast{position:fixed;left:50%;bottom:96px;transform:translateX(-50%);background:#1e1b17;color:#fff;font-size:13px;font-weight:600;padding:11px 18px;border-radius:11px;box-shadow:0 8px 24px rgba(0,0,0,.25);z-index:90;animation:pstoast .2s;}
...
.pd-head-book{display:flex;align-items:center;gap:7px;background:var(--brand);color:#fff;font-weight:700;font-size:14px;padding:11px 18px;border-radius:12px;box-shadow:0 5px 14px rgba(42,39,36,.3);}
...
.pd-avatar{width:128px;height:128px;border-radius:30px;flex:0 0 auto;background:linear-gradient(150deg,var(--brand),var(--brand-deep));color:#fff;font-family:'Fraunces';font-weight:700;font-size:56px;display:grid;place-items:center;border:6px solid var(--bg);box-shadow:0 10px 28px rgba(22,19,16,.22);}
...
.pd-book{display:flex;align-items:center;gap:7px;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px 22px;border-radius:12px;box-shadow:0 6px 16px rgba(42,39,36,.3);}
...
.pd-tabs button{font-size:14px;font-weight:600;padding:10px 16px;border-radius:10px;color:#5a554b;white-space:nowrap;}
.pd-tabs button:hover{background:#f3e7e1;}
.pd-tabs button.on{background:#ece6dc;color:var(--brand-deep);font-weight:700;}
...
.pd-railcard>p{font-size:13px;line-height:1.55;color:#4a463e;}
...
.pd-railbtns button:hover{background:#ece6dc;border-color:var(--brand);}
.pd-railcard.trust{background:#ece6dc;border-color:#ddd4c6;}
.pd-trust-h{display:flex;align-items:center;gap:7px;font-size:13.5px;font-weight:700;color:var(--brand-deep);}
.pd-railcard.trust p{font-size:12px;line-height:1.55;color:#6a6458;margin-top:8px;}
...
.pd-filter button{font-size:13px;font-weight:600;padding:8px 16px;border-radius:20px;background:var(--card);border:1px solid var(--line);color:#4a463e;}
...
.feat-tag{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--brand-deep);background:#ece6dc;padding:5px 11px;border-radius:8px;}
...
.feat-card{flex:0 0 76%;max-width:280px;background:var(--card);border:1px solid var(--line);border-radius:16px;overflow:hidden;box-shadow:0 8px 22px rgba(22,19,16,.08);display:flex;flex-direction:column;}
...
.feat-thumb.product{background:linear-gradient(150deg,#b08d57,var(--gold));}
...
.feat-cta{font-size:13px;font-weight:700;padding:8px 18px;border-radius:10px;color:#fff;background:var(--brand);box-shadow:0 4px 10px rgba(42,39,36,.3);}
...
.blog-card:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(22,19,16,.1);}
...
.blog-img.c2{background:linear-gradient(135deg,#b08d57,var(--brand-deep));}
...
.blog-body p{font-size:13px;color:#6a6458;line-height:1.5;}
...
.blog-topics button{font-size:13px;font-weight:600;padding:8px 16px;border-radius:20px;background:var(--card);border:1px solid var(--line);color:#4a463e;}
...
.blog-hero:hover{transform:translateY(-2px);box-shadow:0 14px 32px rgba(22,19,16,.1);}
...
.blog-hero-img.c2{background:linear-gradient(135deg,#b08d57,var(--brand-deep));}
...
.blog-hero-body>p{font-size:14px;color:#5a554b;line-height:1.55;}
...
.author-card>p{font-size:12.5px;line-height:1.55;color:#5a554b;}
...
.blog-convert{background:#ece6dc;border:1px solid #ddd4c6;border-radius:16px;padding:17px;}
.blog-convert>b{font-family:'Fraunces';font-weight:600;font-size:16px;}
.blog-convert>p{font-size:12.5px;color:#6a6458;line-height:1.5;margin:6px 0 13px;}
.blog-convert-cta{display:flex;align-items:center;justify-content:center;gap:7px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:13.5px;padding:11px;border-radius:11px;box-shadow:0 6px 14px rgba(42,39,36,.28);}
...
.ab-lede{font-size:15.5px;line-height:1.6;color:#5a554b;max-width:none;}
...
.ab-bio{font-size:14px;line-height:1.6;color:#5a554b;margin:13px 0;}
.ab-chips{display:flex;flex-wrap:wrap;gap:7px;margin-bottom:14px;}
.ab-chips span{font-size:11.5px;font-weight:600;color:var(--brand-deep);background:#ece6dc;border:1px solid #ecd2dd;padding:5px 11px;border-radius:16px;}
...
.ab-approach-grid p{font-size:13px;line-height:1.55;color:#5a554b;}
...
.ab-addr{font-size:13px;line-height:1.5;color:#4a463e;margin:12px 0 8px;}
...
.ab-follow-icons button{width:40px;height:40px;border-radius:11px;display:grid;place-items:center;background:#ece6dc;border:1px solid #ecd2dd;color:var(--brand-deep);transition:background .12s,color .12s;}
...
.ab-book-m{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;margin-top:18px;background:var(--brand);color:#fff;font-weight:700;font-size:15px;padding:14px;border-radius:13px;box-shadow:0 8px 18px rgba(42,39,36,.3);}
.ab-para{font-size:14.5px;line-height:1.65;color:#5a554b;max-width:none;margin-top:13px;}
...
.ab-offer-card:hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(22,19,16,.09);border-color:#ddd4c6;}
.ab-offer-card b{display:block;font-size:15.5px;font-weight:700;margin-bottom:6px;}
.ab-offer-card p{font-size:13px;line-height:1.55;color:#5a554b;margin-bottom:11px;}
...
.ab-fact-v{font-size:13.5px;color:#4a463e;line-height:1.45;}
.ab-grid-m{grid-template-columns:1fr;}
.ab-facts-m{grid-template-columns:1fr;}
.ab-verified{display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:700;color:var(--brand-deep);background:#ece6dc;border:1px solid #ecd2dd;padding:4px 10px;border-radius:16px;margin-top:8px;}
...
.ab-creds li{display:flex;align-items:flex-start;gap:8px;font-size:13px;line-height:1.45;color:#4a463e;}
...
.ab-shot:hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(22,19,16,.14);}
...
.ab-shot.c2{background:linear-gradient(150deg,#b08d57,var(--brand-deep));}
...
.ab-review{position:relative;background:#ece6dc;border:1px solid #ddd4c6;border-radius:18px;padding:24px;margin:28px 0;}
...
.ab-journal-item:hover{transform:translateY(-2px);box-shadow:0 10px 22px rgba(22,19,16,.09);border-color:#ddd4c6;}
...
.faq-cats button{display:flex;align-items:center;gap:10px;width:100%;text-align:left;font-size:13px;font-weight:600;color:#4a463e;padding:10px 11px;border-radius:9px;transition:background .12s,color .12s;}
.faq-cats button:hover{background:#ece6dc;color:var(--brand-deep);}
.faq-cats button svg{color:var(--brand);flex:0 0 auto;}
.faq-help{background:#ece6dc;border:1px solid #ddd4c6;border-radius:16px;padding:17px;}
.faq-help>b{font-family:'Fraunces';font-weight:600;font-size:16px;}
.faq-help>p{font-size:12.5px;color:#6a6458;line-height:1.5;margin:6px 0 13px;}
.faq-help-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:13.5px;padding:11px;border-radius:11px;box-shadow:0 6px 14px rgba(42,39,36,.28);}
...
.faq-empty{background:var(--card);border:1px solid var(--line);border-radius:13px;padding:18px;font-size:13.5px;color:#5a554b;}
...
.ct-wa{display:flex;align-items:center;justify-content:center;gap:9px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:15.5px;padding:15px;border-radius:13px;box-shadow:0 8px 18px rgba(42,39,36,.3);}
...
.ct-form-sub{font-size:13px;line-height:1.5;color:#5a554b;margin-bottom:15px;}
...
.ct-send{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14px;padding:13px;border-radius:11px;box-shadow:0 6px 14px rgba(42,39,36,.28);}
...
.ct-map-pin{position:absolute;left:50%;top:44%;transform:translate(-50%,-50%) rotate(-45deg);display:grid;place-items:center;width:34px;height:34px;border-radius:50% 50% 50% 0;background:var(--brand);color:#fff;box-shadow:0 6px 13px rgba(22,19,16,.4);z-index:2}
...
.ct-hours-list li{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:9px 13px;font-size:12.5px;color:#5a554b;}
.ct-hours-list li+li{border-top:1px solid var(--line);}
.ct-hours-list li b{font-weight:600;color:#4a463e;}
.ct-hours-list li.today{background:#ece6dc;color:var(--brand-deep);}
...
.rf-list li{display:flex;align-items:flex-start;gap:10px;font-size:14px;line-height:1.55;color:#4a463e;}
...
.rf-keys li span{font-size:12.5px;color:#5a554b;}
...
.rf-related button{display:flex;align-items:center;justify-content:space-between;width:100%;font-size:13.5px;font-weight:600;color:#4a463e;padding:11px;border-radius:10px;transition:background .12s,color .12s;}
.rf-related button:hover{background:#ece6dc;color:var(--brand-deep);}
...
.tm-intro{font-size:15px;line-height:1.6;color:#5a554b;max-width:70ch;}
...
.tm-section p{font-size:14px;line-height:1.65;color:#4a463e;margin-bottom:10px;}
...
.tm-contents button{text-align:left;font-size:12.5px;font-weight:600;color:#4a463e;padding:8px 10px;border-radius:8px;transition:background .12s,color .12s;}
.tm-contents button:hover{background:#ece6dc;color:var(--brand-deep);}
...
.pf-shot:hover{transform:translateY(-2px);box-shadow:0 14px 30px rgba(22,19,16,.16);}
...
.pf-shot.c2{background:linear-gradient(150deg,#b08d57,var(--brand-deep));}
...
.art-hero.c2{background:linear-gradient(150deg,#b08d57,var(--brand-deep));}
...
.svc-intro{font-size:14px;color:#6a6458;line-height:1.5;margin:-4px 0 24px;max-width:620px;}
...
.svc-fgroup h4{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:#8c8276;margin-bottom:8px;}
.svc-radios{display:flex;flex-direction:column;gap:1px;}
.svc-radios button{text-align:left;font-size:13.5px;font-weight:500;padding:8px 10px;border-radius:9px;color:#4a463e;}
.svc-radios button:hover{background:#f3e7e1;}
.svc-radios button.on{background:#ece6dc;color:var(--brand-deep);font-weight:700;}
.svc-book-card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:16px;box-shadow:0 8px 22px rgba(22,19,16,.06);}
...
.svc-next{font-size:13px;color:#5a554b;margin:10px 0 14px;}
.svc-next b{color:var(--ink);}
.svc-book-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px;border-radius:12px;box-shadow:0 6px 16px rgba(42,39,36,.3);}
...
.svc-card:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(22,19,16,.1);}
...
.svc-card-desc{font-size:12.5px;color:#6a6458;line-height:1.5;margin-top:8px;flex:1;}
...
.svc-card-book{display:inline-flex;align-items:center;gap:3px;font-size:13px;font-weight:700;color:#fff;background:var(--brand);padding:8px 14px;border-radius:10px;box-shadow:0 4px 10px rgba(42,39,36,.3);}
...
.svc-deliv li{display:flex;align-items:flex-start;gap:8px;font-size:12.5px;color:#5a554b;line-height:1.4;}
...
.rev-leave{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px;border-radius:12px;box-shadow:0 6px 16px rgba(42,39,36,.3);}
...
.rev-filter-m button{display:inline-flex;align-items:center;gap:4px;font-size:12.5px;font-weight:600;padding:7px 13px;border-radius:10px;border:1px solid var(--line);background:var(--card);color:#4a463e;}
...
.rev-photostrip h4{font-size:12.5px;font-weight:700;color:#5a554b;margin-bottom:10px;}
...
.rev-ph.c2{background:linear-gradient(150deg,#b08d57,var(--brand));}
...
.rev-card-svc{display:inline-block;font-size:11px;font-weight:700;color:var(--brand-deep);background:#ece6dc;padding:3px 9px;border-radius:7px;margin:11px 0 8px;}
.rev-card-text{font-size:13.5px;line-height:1.55;color:#4a463e;}
...
.rev-response p{font-size:12.5px;line-height:1.5;color:#5a554b;}
.rev-form-note{display:flex;align-items:flex-start;gap:7px;font-size:12.5px;color:#6a6458;line-height:1.5;background:#ece6dc;border:1px solid #ddd4c6;border-radius:11px;padding:11px 12px;margin-bottom:6px;}
...
.rev-input:focus,.rev-textarea:focus{outline:none;border-color:var(--brand);box-shadow:0 0 0 3px rgba(42,39,36,.12);}
