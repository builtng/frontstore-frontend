...
const MOCK_PRODUCT_FAQS = [
  ...
  ["What is the print quality like?", "Prints are produced on archival, museum quality paper and canvas, colour managed to match your edited images."],
  ["Can I order a print of any image?", "Yes, any image from your gallery can be printed. Digital products like presets are delivered as a download, nothing is shipped."],
...
const MOCK_SERVICE_FAQS = [
  ["How do I book a session?", "Pick a session, choose a date, and a deposit secures it. We then have a quick chat to plan the shoot before the day."],
  ["How do deposits work?", "A deposit secures your date when you book, and the balance is due on the shoot day. The deposit amount is always shown before you confirm."],
  ["When do I get my photos?", "Edited galleries are usually ready within 2 to 3 weeks, with wedding sneak peeks inside 48 hours. Turnaround is confirmed when you book."],
      ...
      {search && (
        <div className="ps-overlay" onClick={() => setSearch(false)}>
          <div className="ps-search-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ps-chips">{["Portrait", "Wedding", "Brand Shoot", "Photo Album", "Preset Pack"].map((c: any) => (<button key={c} onClick={() => { setSearch(false); ping("Searching " + c); }}>{c}</button>))}</div>
          </div>
        </div>
      )}
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#ece6dc;padding:7px 12px;border-radius:9px;margin-top:11px;}
...
.ps-notify{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:700;color:#fff;background:var(--brand);padding:8px 13px;border-radius:9px;box-shadow:0 4px 12px rgba(42,39,36,.28);cursor:pointer;}
...
.ct-map-pin{position:absolute;left:50%;top:44%;transform:translate(-50%,-50%) rotate(-45deg);display:grid;place-items:center;width:34px;height:34px;border-radius:50% 50% 50% 0;background:var(--brand);color:#fff;box-shadow:0 6px 13px rgba(22,19,16,.4);}
