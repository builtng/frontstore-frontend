...
const MOCK_PRODUCT_FAQS = [
  ...
  ["Are the products safe for sensitive skin?", "Our eco range is plant based and low irritation. Always follow the label and patch test if your skin is very sensitive."],
...
const MOCK_SERVICE_FAQS = [
  ["How does pickup and delivery work?", "Book a pickup time and we come to you, bag and tag your items, then deliver everything back once it is ready. Pickup and delivery are free within our core Lagos zones."],
  ["How do deposits work?", "A deposit secures your pickup when you book, and the balance is paid on delivery. The deposit amount is always shown before you confirm."],
  ["Can I reschedule or cancel?", "Yes, up to 24 hours before your pickup for a full deposit refund. Inside 24 hours the deposit is held against the booking."],
...
`;
  ...
  const aboutBody = () => (<>
    ...
    <p className="ab-para">{store.about_intro_text || "What began in 2019 with one pickup route is now a laundry service that Lagos genuinely trusts with its best clothes, known for fresh results and delivery that arrives on time."}</p>
      ...
      {search && (
        <div className="ps-overlay" onClick={() => setSearch(false)}>
          <div className="ps-search-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ps-chips">{["Wash & Fold", "Dry Cleaning", "Sneaker Care", "Weekly Plan", "Scent Booster"].map((c: any) => (<button key={c} onClick={() => { setSearch(false); ping("Searching " + c); }}>{c}</button>))}</div>
          </div>
        </div>
      )}
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#e7eefc;padding:7px 12px;border-radius:9px;margin-top:11px;}
...
.ps-notify{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:700;color:#fff;background:var(--brand);padding:8px 13px;border-radius:9px;box-shadow:0 4px 12px rgba(47,111,224,.28);cursor:pointer;}
...
.ct-map-pin{position:absolute;left:50%;top:44%;transform:translate(-50%,-50%) rotate(-45deg);display:grid;place-items:center;width:34px;height:34px;border-radius:50% 50% 50% 0;background:var(--brand);color:#fff;box-shadow:0 6px 13px rgba(20,61,143,.4);}
