...
interface LaundryStorefrontProps {
  username: string;
  store: StoreType;
  categories: Category[];
  products: Product[];
...
const MOCK_SERVICE_INFO = {
  ...
  s3: { includes: ["Professional steam pressing", "Shirts, trousers, dresses and linen", "Collars and cuffs sharpened", "Returned on hangers, ready to wear", "No wash, pressing only", "Free pickup and delivery in zone"] },
  s4: { includes: ["Duvets, blankets, throws and linen", "Deep washed and fully dried", "Freshened to lift odours", "Folded and packed ready to store", "Handled by piece, not by weight", "Free pickup and delivery in zone"] },
  s5: { includes: ["Curtains and drapes cleaned by fabric", "Pressed and returned ready to hang", "Odour and dust lifted", "Linings treated with care", "Priced clearly per panel", "Free pickup and delivery in zone"] },
  s6: { includes: ["Deep clean of uppers, soles and laces", "Trainers, canvas and leather shoes", "Deodorised inside and out", "Light scuffs and marks reduced", "Whitening where the material allows", "Free pickup and delivery in zone"] },
...
const MOCK_HOURS = [
  ...
  ["Thu", "8:00am - 7:00pm"], ["Fri", "8:00am - 7:00pm"], ["Sat", "9:00am - 5:00pm"], ["Sun", "Closed"],
];
const MOCK_NAV = [
...
export default function LaundryStorefront({
  username,
  store,
  categories,
  products,
  ...
  const customBrandColor = store.primary_color || '#2f6fe0';
  const customBrandDeep = store.primary_color ? `color-mix(in srgb, ${store.primary_color} 80%, black)` : '#163e90';
  ...
  const primaryCta = ({
    ...
    book: { label: "Book a pickup", Icon: Calendar, run: () => openBooking() },
  ...
  } as Record<string, any>)[DUMMY_STORE.primaryCta as any] || { label: "Book a pickup", Icon: Calendar, run: () => openBooking() };
...
.sv-cat.c0{background:var(--brand);}.sv-cat.c1{background:#4f86d8;}.sv-cat.c2{background:#6f96c8;}.sv-cat.c3{background:var(--brand-deep);}
...
.sv-card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:18px;box-shadow:0 8px 30px rgba(47,111,224,.08);}
...
`;
  ...
  const serviceView = () => {
    ...
    const steps = [
      ["Book a pickup", "Choose a pickup time. A small deposit holds your slot, and you see the amount before you confirm."],
      ["We collect and sort", "We come to you, bag and tag your items, then sort by fabric and colour and pre-treat any stains."],
      ["Washed, dried, pressed", "Your order is washed on its own, dried at the right setting, then neatly pressed or folded, delicates handled by hand."],
      ["Delivered back to you", "We deliver everything back fresh and ready to wear, and the balance is paid on delivery."],
    ];
    const brings = ["Every order washed separately, never mixed", "Sorted by fabric and colour before washing", "Eco friendly detergents, gentle on clothes and skin"];
    return (
        ...
        <div className="sv-grid">
          ...
          <aside className="sv-side">
            <div className="sv-card">
              ...
              <button className="sv-book" onClick={() => openBooking(s)}><Calendar size={17} /> Book a pickup</button>
              <a className="sv-enquire" href={waHref} target="_blank" rel="noreferrer"><MessageCircle size={16} /> Ask a question</a>
              <p className="sv-note">A deposit of about {money(deposit)} secures your pickup. The balance is paid on delivery.</p>
    ...
    return (
      <Sheet onClose={() => setBookOpen(false)} onBack={onBack} title={(titles as any)[bookStep]}>
        ...
        {bookStep === "review" && (<>
          ...
          <textarea className="bk-input bk-textarea" value={bookNote} onChange={(e) => setBookNote(e.target.value)} placeholder="Items, fabrics, stains, anything useful" />
          <div className="bk-deposit-row"><span><Lock size={13} /> Deposit to secure</span><b>{money(deposit)}</b></div>
          <p className="ps-deposit">Frontstore holds your deposit until pickup, then the {money(bookSvc.price - deposit)} balance is due on delivery. Covered by buyer protection.</p>
  ...
  const reviewsBody = () => (<>
    <p className="svc-intro">Every review here comes from a verified order on Frontstore. The {storeLabel} can respond, but cannot remove genuine reviews.</p>
    ...
    <div className="rev-trust rev-trust-m"><ShieldCheck size={14} /> Reviews are from verified orders. The {storeLabel} typically responds in {DUMMY_STORE.reply}.</div>
    ...
    {revFiltered.length > 0 ? (
      <div className="rev-list rev-list-m">{revFiltered.map((rv: any) => <ReviewCardRich storeInitial={DUMMY_STORE.initial} key={rv.id} rv={rv} storeLabel={storeLabel} />)}</div>
  ...
  const aboutBody = () => (<>
    ...
    <div className="ab-follow">
      <span className="ab-follow-h">Follow the {storeLabel}</span>
      <div className="ab-socials">
    ...
    <button className="ab-book-m" onClick={() => openBooking()}><Calendar size={16} /> Book a pickup</button></>);
  const faqSections = () => (
    <div className="faq-groups">
      {faqFiltered.length === 0 && (
        <div className="faq-empty">No questions match that search. Try another word, or message the {storeLabel} below.</div>
      )}
  ...
  const faqHelp = () => (
    <div className="faq-help">
      <b>Still need help?</b>
      <p>{store.faq_help_text || `Message the ${storeLabel} directly and we will get back to you` + (DUMMY_STORE.reply ? ", usually in " + DUMMY_STORE.reply : "") + "."}</p>
  ...
  const contactVisit = () => {
    ...
    return (
      <div className="ct-visit">
        <div className="ab-rail-h"><MapPin size={15} /> Visit the {storeLabel}</div>
        ...
        <div className="ct-hours">
          ...
          {hasHours ? (
            ...
            <p className="ct-hours-empty">This laundry hasn't added opening hours yet. Message on WhatsApp to check availability.</p>
          )}
        </div>
        <div className="ab-follow-rail">
          <span>Follow the {storeLabel}</span>
      ...
      <div className="rf-section">
        ...
        <ul className="rf-list">
          {store.policy_bookings ? (
            ...
            <>
              <li><Check size={15} /> Reschedule or cancel up to 24 hours before your pickup for a full refund of your deposit.</li>
              <li><Check size={15} /> Within 24 hours of your pickup, the deposit is held against the booking and is not refunded.</li>
        ...
        <ul className="rf-list">
          {store.policy_refunds ? (
            ...
            <>
              ...
              <li><Check size={15} /> For bank transfer orders, the {storeLabel} arranges your refund directly, since those funds are paid straight to them.</li>
            </>
          )}
        </ul>
      </div>
  ...
  const refundsAction = () => (
    <div className="blog-convert">
      ...
      <p>Message the {storeLabel} and we will sort your cancellation, return or refund.</p>
  ...
  const portfolioBody = () => (<>
    <p className="svc-intro">Before and after from real orders, from restored suits and trainers to lifted stains and freshened bedding.</p>
    {portfolioChips()}
    {portfolioGrid()}
    <button className="ab-book-m" onClick={() => openBooking()}><Calendar size={16} /> Book a pickup</button>
    {portfolioFollow()}
        ...
        <div className="art-wrap">
          ...
          <aside className="art-rail">
            ...
            <div className="blog-convert">
              ...
              <p>Book a pickup, or shop the laundry care we swear by.</p>
              <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book a pickup</button>
            ...
            {page === "home" && (<>
              <section className="ps-cover-wrap">
                ...
                <div className="ps-stats">
                  ...
                  {DUMMY_STORE.orders ? <div><b>{DUMMY_STORE.orders}</b><span>orders done</span></div> : null}
        ...
        <div className="pd-wrap">
              ...
              <div className="pd-listing">
                ...
                {page === "services" && (
                  <div className="svc-page">
                    <p className="svc-intro">Book a pickup with our team. A deposit secures your pickup, with the balance paid on delivery.</p>
                ...
                {page === "products" && (
                  <div className="svc-page">
                    <p className="svc-intro">Shop the {storeLabel}'s favourites. Delivery across Lagos in 1 to 3 days, with nationwide shipping{DUMMY_STORE.location ? ` and ${DUMMY_STORE.location} pickup at checkout` : ""}.</p>
                  ...
                  <div className="svc-page">
                    ...
                    <div className="svc-body">
                      <aside className="svc-rail">
                        ...
                        <div className="rev-trust"><ShieldCheck size={14} /> Reviews are from verified orders. The {storeLabel} typically responds in {DUMMY_STORE.reply}.</div>
                      </aside>
                      ...
                      <div className="svc-main">
                        ...
                        {revFiltered.length > 0 ? (
                          <div className="rev-list">{revFiltered.map((rv: any) => <ReviewCardRich storeInitial={DUMMY_STORE.initial} key={rv.id} rv={rv} storeLabel={storeLabel} />)}</div>
                        ) : (
                          <div className="svc-empty">No reviews match your filters.<button onClick={() => { setRevStar(0); setRevPhotos(false); }}>Clear filters</button></div>
                        )}
                      </div>
                ...
                {page === "blog" && (
                  <div className="blogp">
                    <p className="svc-intro">Tips, routines and {storeLabel} notes from {DUMMY_AUTHOR.name} and the team at {DUMMY_STORE.name}.</p>
                    <div className="blog-topics">
                    ...
                    <div className="blogp-body">
                      ...
                      <aside className="blogp-rail">
                        <div className="author-card">
                          ...
                          <p>{DUMMY_AUTHOR.bio}</p>
                          <div className="author-cred"><Sparkles size={13} /> Over 7 years keeping Lagos in fresh, pressed clothes</div>
                          <button className="author-link" onClick={() => go("about")}>More about {DUMMY_AUTHOR.name.split(" ")[0]} <ChevronRight size={14} /></button>
                        </div>
                        <div className="blog-convert">
                          <b>Enjoyed the read?</b>
          ...
          <button className="blog-convert-ghost" onClick={() => go("products")}>View supplies</button>
            ...
            {page === "portfolio" && (
              <div className="pd-listing">
                ...
                <p className="svc-intro">Before and after from real orders, from restored suits and trainers to lifted stains and freshened bedding. Tap any image to see it larger.</p>
                {portfolioChips()}
                <div className="pf-wrap">
                  ...
                  <aside className="pf-rail">
                    <div className="blog-convert">
                      ...
                      <p>Book your next pickup with the team.</p>
                      <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book a pickup</button>
                      {(store.storefront_sections || []).includes("services") && (
          <button className="blog-convert-ghost" onClick={() => go("services")}>Browse services</button>
                    ...
                    <div className="pd-railcard trust">
                      ...
                      <p>Bookings and orders placed through Frontstore are covered by buyer protection the {storeLabel} cannot remove.</p>
                    </div>
                  </aside>
                </div>
                <StoreFoot onNav={go}  slug={DUMMY_STORE.slug} />
                ...
                <div className="ab-wrap">
                  <div className="ab-main">
                    ...
                    <div className="ab-approach">
                      ...
                      <div className="ab-approach-grid">
                        <div><span className="ab-num">01</span><b>We collect from you</b><p>Every order starts with a pickup at a time that suits you, then it is bagged, tagged and logged so nothing gets mixed up.</p></div>
                        <div><span className="ab-num">02</span><b>Sorted and treated right</b><p>We sort by fabric and colour and choose the right wash for each piece, not a one size fits all cycle.</p></div>
                        <div><span className="ab-num">03</span><b>Fresh, pressed, returned</b><p>From everyday wash and fold to delicate dry cleaning, it comes back fresh, neatly pressed and on time.</p></div>
                      </div>
                    </div>
                  ...
                  <aside className="ab-rail">
                    ...
                    <div className="blog-convert">
                      ...
                      <p>Book a pickup, or browse the full list of services.</p>
                      <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book a pickup</button>
                      {(store.storefront_sections || []).includes("services") && (
          <button className="blog-convert-ghost" onClick={() => go("services")}>Browse services</button>
        )}
                    ...
                    <div className="pd-railcard trust">
                      <div className="pd-trust-h"><ShieldCheck size={15} /> Secured by Frontstore</div>
                      <p>Bookings and orders placed through Frontstore are covered by buyer protection that the {storeLabel} cannot remove.</p>
                    </div>
                  </aside>
                </div>
                <StoreFoot onNav={go}  slug={DUMMY_STORE.slug} />
            ...
            {page === "faq" && (
              <div className="pd-listing">
                ...
                <p className="svc-intro">Answers to the questions we are asked most. If you cannot find yours, the {storeLabel} is a message away.</p>
                <div className="faq-wrap">
                  <aside className="faq-rail">
                    ...
                    <div className="pd-railcard trust">
                      <div className="pd-trust-h"><ShieldCheck size={15} /> Secured by Frontstore</div>
                      <p>Payments and orders through Frontstore are covered by buyer protection the {storeLabel} cannot remove.</p>
                    </div>
                  </aside>
              ...
              <div className="pd-listing">
                ...
                <div className="ct-wrap">
                  ...
                  <aside className="ct-rail">
                    ...
                    <div className="blog-convert">
                      <b>Prefer to book?</b>
                      <p>Skip the message and book a pickup in a couple of taps.</p>
                      <button className="blog-convert-cta" onClick={() => openBooking()}><Calendar size={15} /> Book a pickup</button>
                      {(store.storefront_sections || []).includes("services") && (
          <button className="blog-convert-ghost" onClick={() => go("services")}>Browse services</button>
        )}
...
function ReviewCardRich({ rv, storeInitial, storeLabel = 'store' }: { rv: any, storeInitial?: string, storeLabel?: string }) {
  return (
    <div className="rev-card">
      ...
      {rv.response && (
        <div className="rev-response">
          <div className="rev-response-head"><span className="rev-resp-av">{storeInitial || MOCK_STORE.initial}</span><b>Response from the {storeLabel}</b><span>{rv.response.when}</span></div>
...
const css = `
.ps-root{--brand:#2f6fe0;--brand-deep:#163e90;--gold:#c79a4b;--bg:#f5f8fc;--card:#fff;--ink:#16202e;
  --muted:#6c7c92;--line:#e4eaf2;--ok:#1f9d57;--wa:#25d366;
  --shadow-sm:0 1px 2px rgba(22,32,46,.04),0 2px 8px rgba(22,32,46,.05);
  --shadow-md:0 4px 12px rgba(22,32,46,.06),0 10px 28px rgba(22,32,46,.08);
  --shadow-lg:0 8px 24px rgba(22,32,46,.09),0 20px 48px rgba(22,32,46,.11);
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#e7eefc;padding:7px 12px;border-radius:9px;margin-top:11px;max-width:100%;min-width:0}
...
.ps-notify{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:700;color:#fff;background:var(--brand);padding:8px 13px;border-radius:9px;box-shadow:0 4px 12px rgba(47,111,224,.28);cursor:pointer;flex-shrink:0}
.nt-topics{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:4px;}
.nt-topic{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:600;padding:9px 13px;border-radius:11px;border:1px solid var(--line);background:var(--card);color:#36465a;cursor:pointer;}
.nt-topic.on{background:#e7eefc;border-color:var(--brand);color:var(--brand-deep);}
...
.ps-bio{font-size:14px;line-height:1.55;color:#36465a;margin-top:15px;}
...
.ps-signature{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:14px;margin-bottom:18px;box-shadow:0 8px 22px rgba(20,61,143,.07);}
.ps-sig-tag{display:inline-flex;align-items:center;gap:5px;font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:var(--brand-deep);background:#e7eefc;padding:4px 9px;border-radius:7px;}
...
.ps-book-btn,.ps-sheet-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:15px;padding:13px;border-radius:12px;box-shadow:0 6px 16px rgba(47,111,224,.3);}
.ps-book-btn:hover,.ps-sheet-cta:hover{box-shadow:0 8px 22px rgba(47,111,224,.4);transform:translateY(-1px);}
.ps-book-btn:active,.ps-sheet-cta:active{transform:translateY(2px);box-shadow:0 3px 10px rgba(47,111,224,.26);}
...
.ps-chips button{flex:0 0 auto;font-size:13px;font-weight:600;padding:8px 14px;border-radius:20px;background:var(--card);border:1px solid var(--line);color:#36465a;}
...
.ps-card-thumb.prod{background:linear-gradient(150deg,#6fb0ec,var(--gold));}
...
.ps-mini{font-size:13px;font-weight:700;padding:7px 15px;border-radius:9px;color:#fff;background:var(--brand);box-shadow:0 3px 8px rgba(47,111,224,.28);}
.ps-mini:hover{box-shadow:0 5px 14px rgba(47,111,224,.36);transform:translateY(-1px);}
...
.ps-seeall:hover{background:#e7eefc;border-color:var(--brand);}
...
.ps-review p{font-size:13px;line-height:1.5;color:#36465a;}
...
.ps-addr{display:flex;align-items:flex-start;gap:7px;font-size:13.5px;color:#36465a;line-height:1.5;}
...
.ps-dir:hover{background:#e7eefc;border-color:var(--brand);}
.ps-hours{list-style:none;margin-top:14px;border-top:1px solid var(--line);}
.ps-hours li{display:flex;justify-content:space-between;font-size:13px;padding:7px 0;border-bottom:1px solid var(--line);color:#46566a;}
...
.ps-acc-item p{padding:0 14px 14px;font-size:13px;line-height:1.55;color:#36465a;}

.ps-locked{background:#e7eefc;border:1px solid #cfe0f7;border-radius:14px;padding:14px;margin:16px 0 8px;}
...
.ps-locked p{font-size:12.5px;line-height:1.55;color:#54647a;margin-top:8px;}
...
.ps-prose{font-size:14px;line-height:1.65;color:#36465a;margin-bottom:13px;}
...
.ps-foot-links button{font-size:12px;font-weight:600;color:#54647a;}
...
.ps-nav button{display:flex;align-items:center;justify-content:space-between;text-align:left;font-size:14.5px;font-weight:500;padding:11px 12px;border-radius:10px;color:#36465a;}
.ps-nav button.on{background:#e7eefc;color:var(--brand-deep);font-weight:700;}
.ps-panel-actions{display:flex;flex-direction:column;gap:9px;margin-top:auto;}
.ps-act-book{display:flex;align-items:center;justify-content:center;gap:8px;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:13px;border-radius:12px;box-shadow:0 6px 16px rgba(47,111,224,.3);}
...
.ps-fab svg{position:relative;z-index:1;width:54px;height:54px;background:var(--brand);color:#fff;padding:16px;border-radius:50%;box-shadow:0 8px 20px rgba(47,111,224,.4);margin-top:-26px;}
...
.ps-field-lbl{font-size:12.5px;font-weight:700;color:#46566a;margin:14px 0 8px;}
.ps-pick{display:flex;gap:9px;flex-wrap:wrap;}
.ps-pick button{font-size:13px;font-weight:600;padding:9px 15px;border-radius:11px;border:1px solid var(--line);background:var(--card);color:#36465a;}
...
.bk-thumb.c2{background:linear-gradient(150deg,#6fb0ec,var(--brand));}
.bk-thumb.c3{background:linear-gradient(150deg,var(--brand),#5e8fd6);}
...
.bk-svcbar{display:flex;align-items:center;justify-content:space-between;gap:10px;background:#e7eefc;border:1px solid #cfe0f7;border-radius:12px;padding:10px 13px;margin-bottom:16px;}
...
.bk-deposit-row{display:flex;align-items:center;justify-content:space-between;margin-top:16px;padding:12px 14px;background:#e7eefc;border:1px solid #cfe0f7;border-radius:12px;}
...
.ps-search-lbl{font-size:12px;font-weight:700;color:#46566a;margin-bottom:9px;}
...
.ps-toast{position:fixed;left:50%;bottom:96px;transform:translateX(-50%);background:#16202e;color:#fff;font-size:13px;font-weight:600;padding:11px 18px;border-radius:11px;box-shadow:0 8px 24px rgba(0,0,0,.25);z-index:90;animation:pstoast .2s;}
...
.pd-head-book{display:flex;align-items:center;gap:7px;background:var(--brand);color:#fff;font-weight:700;font-size:14px;padding:11px 18px;border-radius:12px;box-shadow:0 5px 14px rgba(47,111,224,.3);}
...
.pd-avatar{width:128px;height:128px;border-radius:30px;flex:0 0 auto;background:linear-gradient(150deg,var(--brand),var(--brand-deep));color:#fff;font-family:'Fraunces';font-weight:700;font-size:56px;display:grid;place-items:center;border:6px solid var(--bg);box-shadow:0 10px 28px rgba(20,61,143,.22);}
...
.pd-book{display:flex;align-items:center;gap:7px;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px 22px;border-radius:12px;box-shadow:0 6px 16px rgba(47,111,224,.3);}
...
.pd-tabs button{font-size:14px;font-weight:600;padding:10px 16px;border-radius:10px;color:#46566a;white-space:nowrap;}
.pd-tabs button:hover{background:#f3e7e1;}
.pd-tabs button.on{background:#e7eefc;color:var(--brand-deep);font-weight:700;}
...
.pd-railcard>p{font-size:13px;line-height:1.55;color:#36465a;}
...
.pd-railbtns button:hover{background:#e7eefc;border-color:var(--brand);}
.pd-railcard.trust{background:#e7eefc;border-color:#cfe0f7;}
.pd-trust-h{display:flex;align-items:center;gap:7px;font-size:13.5px;font-weight:700;color:var(--brand-deep);}
.pd-railcard.trust p{font-size:12px;line-height:1.55;color:#54647a;margin-top:8px;}
...
.pd-filter button{font-size:13px;font-weight:600;padding:8px 16px;border-radius:20px;background:var(--card);border:1px solid var(--line);color:#36465a;}
...
.feat-tag{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--brand-deep);background:#e7eefc;padding:5px 11px;border-radius:8px;}
...
.feat-card{flex:0 0 76%;max-width:280px;background:var(--card);border:1px solid var(--line);border-radius:16px;overflow:hidden;box-shadow:0 8px 22px rgba(20,61,143,.08);display:flex;flex-direction:column;}
...
.feat-thumb.product{background:linear-gradient(150deg,#6fb0ec,var(--gold));}
...
.feat-cta{font-size:13px;font-weight:700;padding:8px 18px;border-radius:10px;color:#fff;background:var(--brand);box-shadow:0 4px 10px rgba(47,111,224,.3);}
...
.blog-card:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(20,61,143,.1);}
...
.blog-img.c2{background:linear-gradient(135deg,#6fb0ec,var(--brand-deep));}
...
.blog-body p{font-size:13px;color:#54647a;line-height:1.5;}
...
.blog-topics button{font-size:13px;font-weight:600;padding:8px 16px;border-radius:20px;background:var(--card);border:1px solid var(--line);color:#36465a;}
...
.blog-hero:hover{transform:translateY(-2px);box-shadow:0 14px 32px rgba(20,61,143,.1);}
...
.blog-hero-img.c2{background:linear-gradient(135deg,#6fb0ec,var(--brand-deep));}
...
.blog-hero-body>p{font-size:14px;color:#46566a;line-height:1.55;}
...
.author-card>p{font-size:12.5px;line-height:1.55;color:#46566a;}
...
.blog-convert{background:#e7eefc;border:1px solid #cfe0f7;border-radius:16px;padding:17px;}
.blog-convert>b{font-family:'Fraunces';font-weight:600;font-size:16px;}
.blog-convert>p{font-size:12.5px;color:#54647a;line-height:1.5;margin:6px 0 13px;}
.blog-convert-cta{display:flex;align-items:center;justify-content:center;gap:7px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:13.5px;padding:11px;border-radius:11px;box-shadow:0 6px 14px rgba(47,111,224,.28);}
...
.ab-lede{font-size:15.5px;line-height:1.6;color:#46566a;max-width:none;}
...
.ab-bio{font-size:14px;line-height:1.6;color:#46566a;margin:13px 0;}
.ab-chips{display:flex;flex-wrap:wrap;gap:7px;margin-bottom:14px;}
.ab-chips span{font-size:11.5px;font-weight:600;color:var(--brand-deep);background:#e7eefc;border:1px solid #ecd2dd;padding:5px 11px;border-radius:16px;}
...
.ab-approach-grid p{font-size:13px;line-height:1.55;color:#46566a;}
...
.ab-addr{font-size:13px;line-height:1.5;color:#36465a;margin:12px 0 8px;}
...
.ab-follow-icons button{width:40px;height:40px;border-radius:11px;display:grid;place-items:center;background:#e7eefc;border:1px solid #ecd2dd;color:var(--brand-deep);transition:background .12s,color .12s;}
...
.ab-book-m{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;margin-top:18px;background:var(--brand);color:#fff;font-weight:700;font-size:15px;padding:14px;border-radius:13px;box-shadow:0 8px 18px rgba(47,111,224,.3);}
.ab-para{font-size:14.5px;line-height:1.65;color:#46566a;max-width:none;margin-top:13px;}
...
.ab-offer-card:hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(20,61,143,.09);border-color:#cfe0f7;}
.ab-offer-card b{display:block;font-size:15.5px;font-weight:700;margin-bottom:6px;}
.ab-offer-card p{font-size:13px;line-height:1.55;color:#46566a;margin-bottom:11px;}
...
.ab-fact-v{font-size:13.5px;color:#36465a;line-height:1.45;}
.ab-grid-m{grid-template-columns:1fr;}
.ab-facts-m{grid-template-columns:1fr;}
.ab-verified{display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:700;color:var(--brand-deep);background:#e7eefc;border:1px solid #ecd2dd;padding:4px 10px;border-radius:16px;margin-top:8px;}
...
.ab-creds li{display:flex;align-items:flex-start;gap:8px;font-size:13px;line-height:1.45;color:#36465a;}
...
.ab-shot:hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(20,61,143,.14);}
...
.ab-shot.c2{background:linear-gradient(150deg,#6fb0ec,var(--brand-deep));}
...
.ab-review{position:relative;background:#e7eefc;border:1px solid #cfe0f7;border-radius:18px;padding:24px;margin:28px 0;}
...
.ab-journal-item:hover{transform:translateY(-2px);box-shadow:0 10px 22px rgba(20,61,143,.09);border-color:#cfe0f7;}
...
.faq-cats button{display:flex;align-items:center;gap:10px;width:100%;text-align:left;font-size:13px;font-weight:600;color:#36465a;padding:10px 11px;border-radius:9px;transition:background .12s,color .12s;}
.faq-cats button:hover{background:#e7eefc;color:var(--brand-deep);}
.faq-cats button svg{color:var(--brand);flex:0 0 auto;}
.faq-help{background:#e7eefc;border:1px solid #cfe0f7;border-radius:16px;padding:17px;}
.faq-help>b{font-family:'Fraunces';font-weight:600;font-size:16px;}
.faq-help>p{font-size:12.5px;color:#54647a;line-height:1.5;margin:6px 0 13px;}
.faq-help-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:13.5px;padding:11px;border-radius:11px;box-shadow:0 6px 14px rgba(47,111,224,.28);}
...
.faq-empty{background:var(--card);border:1px solid var(--line);border-radius:13px;padding:18px;font-size:13.5px;color:#46566a;}
...
.ct-wa{display:flex;align-items:center;justify-content:center;gap:9px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:15.5px;padding:15px;border-radius:13px;box-shadow:0 8px 18px rgba(47,111,224,.3);}
...
.ct-form-sub{font-size:13px;line-height:1.5;color:#46566a;margin-bottom:15px;}
...
.ct-send{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14px;padding:13px;border-radius:11px;box-shadow:0 6px 14px rgba(47,111,224,.28);}
...
.ct-map-pin{position:absolute;left:50%;top:44%;transform:translate(-50%,-50%) rotate(-45deg);display:grid;place-items:center;width:34px;height:34px;border-radius:50% 50% 50% 0;background:var(--brand);color:#fff;box-shadow:0 6px 13px rgba(20,61,143,.4);z-index:2}
...
.ct-hours-list li{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:9px 13px;font-size:12.5px;color:#46566a;}
.ct-hours-list li+li{border-top:1px solid var(--line);}
.ct-hours-list li b{font-weight:600;color:#36465a;}
.ct-hours-list li.today{background:#e7eefc;color:var(--brand-deep);}
...
.rf-list li{display:flex;align-items:flex-start;gap:10px;font-size:14px;line-height:1.55;color:#36465a;}
...
.rf-keys li span{font-size:12.5px;color:#46566a;}
...
.rf-related button{display:flex;align-items:center;justify-content:space-between;width:100%;font-size:13.5px;font-weight:600;color:#36465a;padding:11px;border-radius:10px;transition:background .12s,color .12s;}
.rf-related button:hover{background:#e7eefc;color:var(--brand-deep);}
...
.tm-intro{font-size:15px;line-height:1.6;color:#46566a;max-width:70ch;}
...
.tm-section p{font-size:14px;line-height:1.65;color:#36465a;margin-bottom:10px;}
...
.tm-contents button{text-align:left;font-size:12.5px;font-weight:600;color:#36465a;padding:8px 10px;border-radius:8px;transition:background .12s,color .12s;}
.tm-contents button:hover{background:#e7eefc;color:var(--brand-deep);}
...
.pf-shot:hover{transform:translateY(-2px);box-shadow:0 14px 30px rgba(20,61,143,.16);}
...
.pf-shot.c2{background:linear-gradient(150deg,#6fb0ec,var(--brand-deep));}
...
.art-hero.c2{background:linear-gradient(150deg,#6fb0ec,var(--brand-deep));}
...
.svc-intro{font-size:14px;color:#54647a;line-height:1.5;margin:-4px 0 24px;max-width:620px;}
...
.svc-fgroup h4{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:#6c7c92;margin-bottom:8px;}
.svc-radios{display:flex;flex-direction:column;gap:1px;}
.svc-radios button{text-align:left;font-size:13.5px;font-weight:500;padding:8px 10px;border-radius:9px;color:#36465a;}
.svc-radios button:hover{background:#f3e7e1;}
.svc-radios button.on{background:#e7eefc;color:var(--brand-deep);font-weight:700;}
.svc-book-card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:16px;box-shadow:0 8px 22px rgba(20,61,143,.06);}
...
.svc-next{font-size:13px;color:#46566a;margin:10px 0 14px;}
.svc-next b{color:var(--ink);}
.svc-book-cta{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px;border-radius:12px;box-shadow:0 6px 16px rgba(47,111,224,.3);}
...
.svc-card:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(20,61,143,.1);}
...
.svc-card-desc{font-size:12.5px;color:#54647a;line-height:1.5;margin-top:8px;flex:1;}
...
.svc-card-book{display:inline-flex;align-items:center;gap:3px;font-size:13px;font-weight:700;color:#fff;background:var(--brand);padding:8px 14px;border-radius:10px;box-shadow:0 4px 10px rgba(47,111,224,.3);}
...
.svc-deliv li{display:flex;align-items:flex-start;gap:8px;font-size:12.5px;color:#46566a;line-height:1.4;}
...
.rev-leave{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:var(--brand);color:#fff;font-weight:700;font-size:14.5px;padding:12px;border-radius:12px;box-shadow:0 6px 16px rgba(47,111,224,.3);}
...
.rev-filter-m button{display:inline-flex;align-items:center;gap:4px;font-size:12.5px;font-weight:600;padding:7px 13px;border-radius:10px;border:1px solid var(--line);background:var(--card);color:#36465a;}
...
.rev-photostrip h4{font-size:12.5px;font-weight:700;color:#46566a;margin-bottom:10px;}
...
.rev-ph.c2{background:linear-gradient(150deg,#6fb0ec,var(--brand));}
...
.rev-card-svc{display:inline-block;font-size:11px;font-weight:700;color:var(--brand-deep);background:#e7eefc;padding:3px 9px;border-radius:7px;margin:11px 0 8px;}
.rev-card-text{font-size:13.5px;line-height:1.55;color:#36465a;}
...
.rev-response p{font-size:12.5px;line-height:1.5;color:#46566a;}
.rev-form-note{display:flex;align-items:flex-start;gap:7px;font-size:12.5px;color:#54647a;line-height:1.5;background:#e7eefc;border:1px solid #cfe0f7;border-radius:11px;padding:11px 12px;margin-bottom:6px;}
...
.rev-input:focus,.rev-textarea:focus{outline:none;border-color:var(--brand);box-shadow:0 0 0 3px rgba(47,111,224,.12);}
