...
const MOCK_PRODUCT_FAQS = [
  ["How long does delivery take?", "Physical products like books and devotionals are delivered within 2 to 5 days within Lagos and 5 to 10 days elsewhere in Nigeria."],
  ["Can I return a product?", "Unopened products can be returned within 7 days in their original packaging. Contact us on WhatsApp and we will arrange a return."],
  ["Are books signed by the pastor?", "Signed copies are available on request and noted at checkout. Please allow 3 to 5 days extra for signed orders."],
  ["Can I order in bulk for my group or department?", "Yes. Message us on WhatsApp with the quantity and we will provide a quote, with group discounts available for 5 or more copies."],
...
const MOCK_SERVICE_FAQS = [
  ["Is Sunday service registration required?", "Registration is not required to attend, but it is encouraged so we can prepare enough seating and notify you of any changes."],
  ["How do I give online?", "Click Give on the home page, choose your giving type, enter your amount and pay securely. A receipt is sent to your WhatsApp or email immediately."],
  ["Can I register a family group?", "Yes. When registering for Sunday service or events, you can add the names and ages of everyone in your group, including children."],
  ["Are event fees refundable?", "Conference and retreat fees are refundable up to 14 days before the event. After that, the fee can be transferred to a future event."],
  ["How do I pay for events?", "Pay securely through Frontstore at checkout, or by bank transfer to the church. Your receipt always arrives on WhatsApp."],
];
...
const MOCK_CATS = ["Services", "Events", "Conference", "Youth", "Giving", "Ceremonies"];
              ...
              <div className="pd-listing">
                ...
                <div className="ct-wrap">
                  <div className="ct-main">
                    <p className="svc-intro">Reach us on WhatsApp for a quick reply, or send an enquiry and a member of the Covenant Life team will get back to you by email.</p>
                    {contactChannels()}
                    {contactForm()}
                  </div>
      ...
      {search && (
        <div className="ps-overlay" onClick={() => setSearch(false)}>
          <div className="ps-search-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ps-chips">{["Sunday Service", "Online Giving", "Conference", "Devotional", "Youth Retreat"].map((c: any) => (<button key={c} onClick={() => { setSearch(false); ping("Searching " + c); }}>{c}</button>))}</div>
          </div>
        </div>
      )}
...
.ps-url{display:inline-flex;align-items:center;gap:7px;font-size:12.5px;font-weight:600;color:var(--brand-deep);background:#fef3c7;padding:7px 12px;border-radius:9px;margin-top:11px;}
