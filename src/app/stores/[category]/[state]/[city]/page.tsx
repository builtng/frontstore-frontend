...
export default async function DirectoryCityPage({ params }: PageProps) {
  ...
  const content = getCityDirectoryContent(persona, state, city, stores.length);
  const canonicalUrl = `https://frontstore.ng/stores/${category}/${stateSlug}/${citySlug}`;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `${persona.name} in ${state.name}`, item: `https://frontstore.ng/stores/${category}/${stateSlug}` },
      { '@type': 'ListItem', position: 4, name: content.headline, item: canonicalUrl },
    ],
  };
      ...
      <header style={{ position: 'relative', overflow: 'hidden', padding: 'clamp(56px, 9vw, 88px) 20px clamp(40px, 7vw, 64px)', borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 760, margin: '0 auto', textAlign: 'center' }}>
          <nav aria-label="Breadcrumb" style={{ marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: 12, color: 'var(--text-faint)', flexWrap: 'wrap' }}>
            ...
            <ArrowRight size={11} />
            <Link href={`/stores/${category}/${stateSlug}`} style={{ color: 'inherit', textDecoration: 'none' }}>{persona.name} in {state.name}</Link>
            <ArrowRight size={11} />
            <span style={{ color: 'var(--text-muted)' }}>{city.name}</span>
          </nav>

          <span className="badge badge-verified animate-fade-in" style={{ marginBottom: 16 }}>
            <MapPin size={11} /> {city.name}, {state.name}
          </span>
      ...
      <main style={{ flex: 1, padding: 'clamp(32px, 5vw, 56px) 20px clamp(48px, 7vw, 72px)', maxWidth: 1120, margin: '0 auto', width: '100%' }}>
        ...
        {stores.length > 0 ? (
          ...
          <div className="card" style={{ padding: '52px 20px', textAlign: 'center', marginBottom: 48 }}>
            ...
            <p style={{ fontWeight: 700, fontSize: 15, marginBottom: 4, color: 'var(--text)' }}>No stores yet in {city.name}</p>
            <p style={{ color: 'var(--text-muted)', fontSize: 13.5 }}>
              New stores join every week — check back soon or{' '}
              <Link href="/signup" style={{ color: 'var(--primary)', fontWeight: 700 }}>create yours today</Link>.
            </p>
          </div>
        )}
        ...
        <div style={{ marginBottom: 40 }}>
          <Link href={`/stores/${category}/${stateSlug}`} className="btn btn-outline" style={{ padding: '8px 16px', fontSize: 12.5, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
            <ArrowLeft size={12} /> All {persona.name} in {state.name}
          </Link>
        </div>
        ...
        <section className="card" style={{ padding: 'clamp(22px, 4vw, 36px)', marginBottom: 40 }}>
          ...
          {content.faqs.map((faq, i) => (
            <details key={i} style={{ borderBottom: i < content.faqs.length - 1 ? '1px solid var(--border)' : 'none', padding: '13px 0' }}>
              <summary style={{ fontWeight: 700, fontSize: 14, cursor: 'pointer', color: 'var(--text)', listStyle: 'none' }}>
                {faq.question}
              </summary>
          ...
          <section>
            <div style={{ marginBottom: 18 }}>
              <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 18, fontWeight: 800, color: 'var(--text)', marginBottom: 4 }}>
                More businesses to discover in {city.name}
              </h2>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', maxWidth: 480 }}>
            <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 22, fontWeight: 800, marginBottom: 8 }}>
              Sell {persona.name.toLowerCase()} in {city.name}?
            </h3>
