...
import {
  Store, Lock, Eye, EyeOff, Loader2, ArrowRight, ShoppingBag, Check
} from 'lucide-react';
...
export default function BuyerLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      setError(null);
      const res = await fetch(`${API_URL}/v1/buyer/auth/login`, {
        method: 'POST',
      ...
      if (!res.ok) throw new Error(json.message || 'Login failed. Please check your details.');
      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        <header style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>
            Track your orders and check out faster on any store.
          </p>
        </header>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <p style={{ textAlign: 'center', fontSize: 13.5, color: 'var(--text-muted)' }}>
            New here?{' '}
            <a href="/buyer/register" style={{ color: 'var(--primary)', fontWeight: 700, textDecoration: 'underline' }}>
              Create a buyer account
            </a>
          </p>
          <p style={{ textAlign: 'center', fontSize: 13, color: 'var(--text-faint)' }}>
            Are you a seller?{' '}
            <a href="/merchant/login" style={{ color: 'var(--text-muted)', fontWeight: 700, textDecoration: 'underline' }}>
